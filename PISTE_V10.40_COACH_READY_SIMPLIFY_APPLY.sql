-- V10.40 : Je suis en place devient la transition directe du workflow simplifié.
-- À exécuter manuellement après validation ; aucune autre RPC n'est modifiée ici.
begin;
...
begin
  ...
  if (r.status='waiting' and coalesce(r.phase,'preparation')='preparation')
    ...
    update public.coaching_sessions
       set status='live', phase='coach_ready', coach_ready_at=coalesce(coach_ready_at,now())
