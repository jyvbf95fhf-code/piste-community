-- V10.41 : dry-run transactionnel du workflow Coaching simplifié.
begin;

alter table if exists public.coaching_sessions add column if not exists blind_mode text not null default 'normal';
do $$ begin
  if not exists (select 1 from pg_constraint where conname='coaching_sessions_blind_mode_v1041') then
    alter table public.coaching_sessions add constraint coaching_sessions_blind_mode_v1041 check (blind_mode in ('normal','simple_blind','full_blind')) not valid;
  end if;
end $$;
...
as $$
  select coalesce(jsonb_agg(to_jsonb(s) || jsonb_build_object(
    'workflow_version',coalesce(s.workflow_version,1),'blind_mode',coalesce(s.blind_mode,'normal'),
