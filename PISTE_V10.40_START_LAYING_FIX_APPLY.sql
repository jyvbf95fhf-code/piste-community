-- V10.40 ciblé : le démarrage de la pose réalise waiting -> live.
-- À exécuter manuellement uniquement après validation.
begin;
...
begin
  ...
  -- Reprise idempotente contrôlée : ne pas écraser l''horodatage existant.
  if r.status = 'live' and coalesce(r.phase,'preparation') = 'laying' then
    return r;
  end if;
  ...
  -- Autorise uniquement cette transition V10.40 auprès du trigger de garde.
  perform set_config('piste.v1040_transition','on',true);
  update public.coaching_sessions
     set status = 'live',
         phase = 'laying',
         laying_started_at = coalesce(laying_started_at, now())
   where id = r.id
   returning * into r;
  return r;
end
...
where routine_schema = 'public'
  and routine_name = 'start_coaching_laying';
