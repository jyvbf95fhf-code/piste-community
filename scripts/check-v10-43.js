...
(async()=>{
 ...
 const modified=execFileSync('git',['diff',baseline,'--name-only'],{encoding:'utf8'}).trim().split('\n');assert(!modified.some(p=>(p.endsWith('.sql')&&!['PISTE_V10.45_PATCH/PISTE_V10.45_DRY_RUN.sql','PISTE_V10.45_PATCH/PISTE_V10.45_APPLY.sql'].includes(p)&&!/^PISTE_V10\.44_PATCH\/PISTE_V10\.44_(?:DRY_RUN|APPLY)\.sql$/.test(p))||p.startsWith('supabase/functions/')),'SQL and Edge Functions unchanged');
