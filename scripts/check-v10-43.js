...
(async()=>{
 ...
 const modified=execFileSync('git',['diff',baseline,'--name-only'],{encoding:'utf8'}).trim().split('\n');assert(!modified.some(p=>(p.endsWith('.sql')&&!['PISTE_V10.48_COACHING_MEMBER_SELF_LEAVE.sql','PISTE_V10.48_COACHING_MEMBER_SELF_LEAVE_DRY_RUN.sql','PISTE_V10.49_COACHING_ACTIVE_DEBRIEF.sql','PISTE_V10.49_COACHING_ACTIVE_DEBRIEF_VERIFY.sql','PISTE_V10.49.1B_COACHING_SOLO_JOIN.sql','PISTE_V10.49.1B_COACHING_SOLO_JOIN_VERIFY.sql','PISTE_V10.49.1C_COACHING_SOLO_TRANSITIONS.sql','PISTE_V10.49.1C_COACHING_SOLO_TRANSITIONS_VERIFY.sql'].includes(p)&&!['PISTE_V10.45_PATCH/PISTE_V10.45_PREPARATION_DRY_RUN.sql','PISTE_V10.45_PATCH/PISTE_V10.45_PREPARATION_APPLY.sql','PISTE_V10.45_PATCH/PISTE_V10.45_CORRECTIF_DRY_RUN.sql','PISTE_V10.45_PATCH/PISTE_V10.45_CORRECTIF_APPLY.sql','PISTE_V10.45_PATCH/PISTE_V10.45_DRY_RUN.sql','PISTE_V10.45_PATCH/PISTE_V10.45_APPLY.sql'].includes(p)&&!/^PISTE_V10\.44_PATCH\/PISTE_V10\.44_(?:DRY_RUN|APPLY)\.sql$/.test(p))||p.startsWith('supabase/functions/')),'SQL and Edge Functions unchanged');
