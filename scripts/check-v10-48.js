...
for (const name of protectedFunctions) {
  if (name === 'clearCoachingRealtime') {
    const current = source(name).replace(/clearTimeout\(coachingMessageToastTimer\);coachingMessageToastTimer=null;/, 'clearTimeout(coachingMessageToastTimer);').replace(/if\(typeof coachingScenarioChannel!=='undefined'&&coachingScenarioChannel\)\{supabase\.removeChannel\(coachingScenarioChannel\);coachingScenarioChannel=null\}if\(typeof coachingScenarioState!=='undefined'\)coachingScenarioState=\{status:'absent',sessionId:null,scenario:null,reads:\[\],(?:sync:'idle',lastEvent:'—',)?error:null\}/, '').replace(/coachingScenarioGate=\{[^;]+\};/, '').replace(/if\(typeof coachingScenarioActiveHiddenAfterRead!=='undefined'\)coachingScenarioActiveHiddenAfterRead=false;if\(typeof coachingScenarioViewer!=='undefined'\)coachingScenarioViewer=\{[^;]+\};if\(typeof document!=='undefined'\)document\.body\?\.classList\.remove\('scenario-viewer-open'\);/, '').replace(/coachingAutoCenteredSessionId=null;/, '');
    assert.equal(current, source(name, old).replace(/clearTimeout\(coachingMessageToastTimer\);coachingMessageToastTimer=null;/, 'clearTimeout(coachingMessageToastTimer);'), `${name}: moteur historique modifié hors extension scénario`);
...
function withoutV1049ActiveMetrics(text) {
  return text
    ...
    .replace(/<section id="coachingScenarioDebrief"[\s\S]*?<\/section>/, '')
    .replace(/<div id="coachingScenarioGate"[\s\S]*?<\/div>/, '')
...
assert.deepEqual(
  ...
  [
    ...
    'scripts/check-v10-42-1.js','scripts/check-v10-42.js','scripts/check-v10-42-2.js',
