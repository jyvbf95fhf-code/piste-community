...
for (const name of protectedFunctions) {
  if (name === 'clearCoachingRealtime') {
    const current = source(name).replace(/clearTimeout\(coachingMessageToastTimer\);coachingMessageToastTimer=null;/, 'clearTimeout(coachingMessageToastTimer);').replace(/if\(typeof coachingScenarioChannel!=='undefined'&&coachingScenarioChannel\)\{supabase\.removeChannel\(coachingScenarioChannel\);coachingScenarioChannel=null\}if\(typeof coachingScenarioState!=='undefined'\)coachingScenarioState=\{status:'absent',sessionId:null,scenario:null,reads:\[\],sync:'idle',lastEvent:'—',error:null\}/, '').replace(/coachingScenarioGate=\{[^;]+\};/, '').replace(/coachingAutoCenteredSessionId=null;/, '');
