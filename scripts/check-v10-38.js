...
if(!/library-selection-mode/.test(css)||!/const C=['"]piste-community-v\d+['"]/.test(sw)||!/app\.js\?v=[^'"\s]+/.test(html+sw)||!/styles\.css\?v=[^'"\s]+/.test(html+sw)||!/v2\.css\?v=[^'"\s]+/.test(html+sw)||!/v2\.js\?v=[^'"\s]+/.test(html+sw)||!sw.includes('skipWaiting')||!sw.includes('clients.claim')||!sw.includes('caches.delete'))throw new Error('Cache V10.38+ incohérent.');
