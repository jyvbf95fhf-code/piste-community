...
const checks=[
 ...
 ['Traceur Démarrer le tracé',/layingActor&&phase==='preparation'.*setUiText\('startLayingBtn','Démarrer le tracé'\)/s.test(app)&&/startLayingBtn/.test(html)],
 ['démarrage pose RPC',/async function startCoachingLaying\(\).*start_coaching_laying.*startTraceurTracking/s.test(app)],
 ['Terminer le tracé uniquement après pose',/layingActor&&phase==='laying'.*setUiText\('trackReadyBtn','Terminer le tracé'\)/s.test(app)],
