...
const checks=[
 ['version centralisée',/const APP_VERSION=['"]10\.42\.(?:2|3)['"]/.test(app)&&/APP_RELEASE_NOTES/.test(app)],
 ...
 ['surface âge commune OPS/Entraînement',/terrain-disappearance-card/.test(html)&&/function isOperationalRecordMode\(mode=recordMode\)/.test(app)&&/isTerrainOlfactionMode\(\)/.test(app)&&/formatTerrainDisappearance/.test(app)],
 ['édition heure disparition commune',/saveOpsDisappearance/.test(html)&&/setTerrainDisappearanceEditing/.test(app)&&/terrainDisappearanceReference/.test(app)],
 ...
 ['draft pré-GPS',/function saveDraft\(force=false\)\{if\(!session\)return;/.test(app)&&/terrainDisappearanceAt,form:formSnapshot\(\)/.test(app)&&!/function saveDraft\(force=false\)\{if\(!gps\.start\|\|!session\)return;/.test(app)],
 ['âge Entraînement raccordé',/isTerrainOlfactionMode\(\)\?operationalTrackAgeReference\(\)/.test(app)&&/isTerrainOlfactionMode\(\)\?getOperationalTrackAgeMs\(\)/.test(app)],
 ['OPS historique piste',/isOperationalRecordMode\(mode=recordMode\)/.test(app)&&/mode==='piste'/.test(app)],
 ...
 ['cache PWA V10.42.x',/piste-community-v210(?:0|1|2|3|4)/.test(sw)&&/app\.js\?v=1042-(?:12|13|14|15|16)/.test(sw+html)&&/v2\.css\?v=20(?:67|68|69|70)/.test(sw+html)]
];
