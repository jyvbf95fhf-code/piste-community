...
ok(/syncReplay3DBaseLayerButtons/.test(app),'L’état visuel des fonds 3D doit être centralisé');
ok(/onBaseLayerChange/.test(prototypeSource+app),'Le fallback doit resynchroniser les boutons');
ok(/setLayoutProperty\('osm','visibility','none'\).*setLayoutProperty\('satellite','visibility','none'\)/.test(prototypeSource),'Les deux fonds doivent être masqués avant activation exclusive');
ok(/getBaseLayer/.test(prototypeSource+app),'Le fond actif doit être lisible');
