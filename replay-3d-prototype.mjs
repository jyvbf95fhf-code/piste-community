...
export async function createReplay3DPrototype({container,dataset,onStatus=()=>{},onReady=()=>{},onError=()=>{},onDiagnostic=()=>{}}={}){
  ...
  return {map,update:applyState,fitAll,setBaseLayer,setFollowMode:mode=>{followMode=['free','traceur','driver'].includes(mode)?mode:'free'},getFollowMode:()=>followMode,destroy(){destroyed=true;map.off('error',onMapError);map.remove()}};
}
