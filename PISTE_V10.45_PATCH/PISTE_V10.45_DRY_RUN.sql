...
begin
 ...
 without_route:=p_blind_mode='full_blind' and exists(select 1 from jsonb_array_elements(p_members) m where (m->>'user_id')::uuid=uid and m->>'role'='coach');
 if without_route then
  if p_route_id is not null then raise exception 'Le Traceur distinct pose la piste : aucun tracé du Coach en double aveugle'; end if;
 else
