...
Le parseur PostgreSQL local valide la grammaire des 29 instructions, **pas** l’exécution PL/pgSQL, les droits réellement déployés ou la concurrence en base. L’application manuelle et la recette ci-dessous restent nécessaires.

## Objets et permissions

- Table complémentaire `private.coaching_deferred_v1045` : clé `session_id` (FK vers coaching_sessions avec cascade), `search_mode`, `traceur_ready_at` et `departure_point` logistique. RLS activée, aucun privilège direct PUBLIC/anon/authenticated, aucune policy permissive.
- `create_coaching_people_session_v1045(uuid,jsonb,text,text)` : mêmes contrôles de personnes, unicité des rôles, amitiés acceptées et identité du créateur que V10.42.3. Seul le cas Coach + full_blind permet et impose `p_route_id = NULL`. Traceur et Conducteur distincts demeurent obligatoires. Les autres configurations conservent leur tracé personnel requis. Session et complément créés atomiquement, rôles non réécrits.
...
1. Coach + double aveugle, Traceur et Conducteur amis distincts : créer en immédiat puis différé, sans carte ni GPX. Vérifier Coach inchangé, planned_route vide, aucun faux Traceur. Tester aussi rôles dupliqués/manquants et non-ami → refus sans session partielle.
...
3. Traceur accepte, « Je pars tracer » : GPS réel. Sans tracé préparé, tenter « Piste tracée » avant deux points → refus explicite. Après deux points, fin de pose enregistrée ; tous les suivis GPS sont arrêtés en attente.
