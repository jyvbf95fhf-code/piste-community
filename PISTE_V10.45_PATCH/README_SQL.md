...
- `create_coaching_people_session_v1045(uuid,jsonb,text)` : mêmes contrôles de personnes, unicité des rôles, amitiés acceptées et identité du créateur que V10.42.3. Seuls les cas Coach ou Conducteur + full_blind permet et impose `p_route_id = NULL`. Traceur et Conducteur distincts demeurent obligatoires. Les autres configurations conservent leur tracé personnel requis. Session et complément créés atomiquement, rôles non réécrits.
...
Les scripts complets restent destinés à la première application V10.45. Si V10.45 est déjà appliquée, ne pas relancer APPLY complet : son précontrôle bloque la réapplication ; une mise à jour ciblée des objets V10.45 doit être préparée séparément. Aucun SQL exécuté par Codex.
