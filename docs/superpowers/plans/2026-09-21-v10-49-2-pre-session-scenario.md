...
- [ ] Task 5 — Ajouter le cycle photo `pending → Storage API → append path → ready`, cleanup Storage côté frontend, abort PostgreSQL sans suppression Storage, puis lecture serveur, « J'ai lu », verrouillage realtime, late joiner et accès débrief.
...
- La RPC historique doit rester compatible ; l'enveloppe versionnée crée le scénario `pending` sans photo. Après obtention de l'ID, le client téléverse au maximum cinq photos avec Storage API, appelle `append_coaching_scenario_photo_v10492` après chaque succès, puis finalise en `ready`. En échec, Storage API supprime les chemins connus ; l'abort serveur ne supprime que la ligne pending après confirmation. L'API refuse des chemins photo dans l'appel initial, car l'ID n'existe pas avant l'appel historique.
