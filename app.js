...
async function beginCoachingLaying(){return coachingTransitionV1040('start_coaching_laying')}
...
function renderCoachingScenario(){const box=$('coachingScenarioReveal'),s=activeCoachingSession;if(!box||!s)return;const phase=coachingPhase(s),role=myCoachingRole(s);if(role!=='driver'||!['coach_ready','driver_running'].includes(phase)){box.classList.add('hidden');return}const title=s.scenario_title||'',text=s.scenario_text||'';box.classList.remove('hidden');box.innerHTML=`<strong>${esc(title||'Scénario de départ')}</strong>${text?`<p>${esc(text)}</p>`:'<p class="muted">Aucun texte de scénario.</p>'}<button id="coachingScenarioAck" class="secondary" type="button">J’ai lu</button>`;bindClick('coachingScenarioAck',()=>{const key=`piste-coaching-scenario-ack-${s.id}`;localStorage.setItem(key,new Date().toISOString());box.classList.add('acknowledged')})}
function applyV1040RoleSurface(){const s=activeCoachingSession;if(!s)return;const role=coachingGpsRole(s),phase=coachingPhase(s),hide=id=>$(id)?.classList.add('hidden'),show=id=>$(id)?.classList.remove('hidden');['startLayingBtn','finishLayingBtn','coachReadyBtn','driverStartBtn','driverFinishBtn'].forEach(hide);if(role==='observer'){hide('coachingLayerControls');show('observerTrackChoice')}else if(role==='traceur'&&phase==='laying'){show('finishLayingBtn')}else if(role==='coach'&&s.laying_mode==='coach'&&phase==='preparation'){show('startLayingBtn')}else if(role==='coach'&&['laid','waiting_ready'].includes(phase)){show('coachReadyBtn')}else if(role==='driver'&&phase==='coach_ready'){show('driverStartBtn')}else if(role==='driver'&&phase==='driver_running'){show('driverFinishBtn')}setUiText('coachingRoleInstruction',role==='observer'?'Vous observez la session sans commande de pilotage.':role==='driver'&&phase!=='driver_running'?'En attente du Coach.':`Rôle : ${coachingRoleLabel(role)} · ${coachingPhaseLabelV1040(s)}`);renderCoachingScenario()}
