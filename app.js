...
let coachingLiveWeather=null,coachingWeatherTimer=null,coachingWeatherLoading=false,coachingTerrainPaused=false;
...
async function deleteSelectedActivities(){const keys=[...activityLibrarySelection];if(!keys.length||!confirm(`Archiver ${keys.length} activité(s) ? Elles resteront restaurables depuis la corbeille.`))return;for(const key of keys){const [type,id]=key.split(':');await archiveLibraryItem(type,id,true)}activityLibrarySelection=[];renderActivityLibrary();updateV8Home()}
...
bindClick('terrainPauseBtn',toggleTerrainPause);bindClick('terrainQuickMarkerBtn',activateTerrainQuickMarker);bindClick('terrainBlackScreenBtn',()=>openFakeLock('coaching'));bindClick('terrainPlusBtn',toggleTerrainPlus);
