...
async function renderCoachingMap(options){
 ...
 if(coachingLayerVisibility.trace&&trace.length){visibleMarkers.add('traceur-track');participantMarker(trace.at(-1),'traceur','Traceur','traceur-track')}
 for(const point of currentRes.data||[]){if(!coachingCanSeeLiveOwner(activeCoachingSession,point.owner_id))continue;visibleMarkers.add(point.owner_id);participantMarker(point,coachingMemberRole(point.owner_id),coachingParticipantName({user_id:point.owner_id}),point.owner_id)}
