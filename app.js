...
async function openFakeLock(context='record'){
 if(context==='record'&&(!gps.start||gps.paused))return;
 if(context==='coaching'&&(!activeCoachingSession||activeCoachingSession.status!=='live'||!isCoachingGpsTracking()))return;
 fakeLockContext=context;fakeLockPreviousFocus=document.activeElement;$('fakeLockScreen').classList.remove('hidden');document.body.classList.add('fake-lock-active');
 const keptAwake=await requestWakeLock();$('fakeLockWakeStatus').textContent=keptAwake?'Écran maintenu actif':'Maintien d’écran indisponible : ne verrouille pas l’iPhone';$('fakeLockWakeStatus').className=keptAwake?'good':'bad';updateFakeLock();$('fakeUnlockBtn')?.focus({preventScroll:true})
}
function closeFakeLock({returnToMap=true}={}){
 const screen=$('fakeLockScreen'),wasOpen=screen&&!screen.classList.contains('hidden'),returnToCoaching=wasOpen&&returnToMap&&fakeLockContext==='coaching'&&activeCoachingSession?.status==='live',focusTarget=fakeLockPreviousFocus;
 screen?.classList.add('hidden');$('fakeUnlockBtn')?.classList.remove('holding','ready');document.body.classList.remove('fake-lock-active');document.documentElement.classList.remove('fake-lock-active');clearTimeout(fakeLockPressTimer);fakeLockPressTimer=null;fakeLockUnlockReady=false;fakeLockPointerId=null;fakeLockPreviousFocus=null;
 if(returnToCoaching){showPage('coachingPage');setCoachingStage('live');setTimeout(()=>{coachingMap?.invalidateSize();document.querySelector('#coachingLivePanel .coaching-map-shell')?.scrollIntoView({block:'center'});focusTarget?.focus?.({preventScroll:true})},80)}else if(wasOpen)setTimeout(()=>focusTarget?.focus?.({preventScroll:true}),0)
}
function completeFakeUnlock(){fakeLockUnlockReady=true;fakeLockBlockUntil=Date.now()+900;$('fakeUnlockBtn')?.classList.add('ready');closeFakeLock()}
function beginFakeUnlock(event){event.preventDefault();event.stopPropagation();if(fakeLockPressTimer)return;fakeLockUnlockReady=false;const button=$('fakeUnlockBtn');if(event.pointerId!==undefined){fakeLockPointerId=event.pointerId;try{button.setPointerCapture(event.pointerId)}catch{}}button.classList.add('holding');fakeLockPressTimer=setTimeout(completeFakeUnlock,1400)}
function finishFakeUnlock(event){event.preventDefault();event.stopPropagation();if(fakeLockUnlockReady)return;if(event.pointerId!==undefined&&fakeLockPointerId!==null&&event.pointerId!==fakeLockPointerId)return;cancelFakeUnlock(event)}
function cancelFakeUnlock(event){event?.preventDefault();event?.stopPropagation();$('fakeUnlockBtn')?.classList.remove('holding','ready');clearTimeout(fakeLockPressTimer);fakeLockPressTimer=null;fakeLockUnlockReady=false;fakeLockPointerId=null}
...
function stopCoachingPresence(){if(coachingPresenceWatch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(coachingPresenceWatch);coachingPresenceWatch=null;coachingPresenceLastPointAt=0;if($('coachingGpsState'))$('coachingGpsState').textContent=coachingGpsError?'GPS : '+coachingGpsError:'Position non partagée';if(fakeLockContext==='coaching'&&!$('fakeLockScreen')?.classList.contains('hidden'))closeFakeLock();updateCoachingPrimaryActions()}
...
async function handleCoachingSessionChange(payload){if(!activeCoachingSession||payload.new?.id!==activeCoachingSession.id)return;Object.assign(activeCoachingSession,payload.new);if(['ended','cancelled'].includes(payload.new.status)){stopCoachingPresence();stopTraceurTracking();closeFakeLock({returnToMap:false});clearVerifiedActiveCoaching(payload.new.id);updateCoachingPhase();updateCoachingPrimaryActions();setCoachingStage('debrief');await calculateCoachingDebrief();setUiText('coachingLiveStatus',payload.new.status==='ended'?'Session terminée par l’organisateur • débrief disponible':'Session annulée par l’organisateur')}else if(payload.new.status==='live'){verifiedActiveCoachingSession=activeCoachingSession;saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();updateCoachingPhase();updateCoachingPrimaryActions();setCoachingStage('live')}}
...
function updateCoachingPrimaryActions(){
 ...
 setCoachingPrimaryAction('coachingFakeLockBtn','coachingFakeLockReason','Activer l’écran noir',live&&gpsAvailable&&gpsCapable&&tracking,live?(tracking?'Le déverrouillage s’effectue par appui long ; une sortie de secours reste disponible.':'Démarrez d’abord le suivi GPS.'):'L’écran noir est disponible uniquement pendant une session active.');
...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingAutoMetrics=null;coachingGpsError='';const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
...
function participantMarker(point,role,label){const colors={driver:'#27b7ff',solo:'#27b7ff',coach:'#a986ff',traceur:'#58d6a4'},letters={driver:'D',solo:'D',coach:'C',traceur:'T'},color=colors[role]||'#91a09d',isDriver=['driver','solo'].includes(role),heading=Number.isFinite(Number(point.heading_deg))?Number(point.heading_deg):0,html=isDriver?`<span class="driver-heading" style="--participant-color:${color};--heading:${heading}deg"><i>➤</i></span>`:`<span style="--participant-color:${color}">${letters[role]||'O'}</span>`;if(isDriver&&Number(point.accuracy_m)>0)coachingLayers.push(L.circle([point.lat,point.lon],{radius:Number(point.accuracy_m),color,weight:1,fillColor:color,fillOpacity:.08,interactive:false}).addTo(coachingMap));return L.marker([point.lat,point.lon],{zIndexOffset:900,icon:L.divIcon({className:'participant-map-icon',html,iconSize:[42,42],iconAnchor:[21,21]})}).addTo(coachingMap).bindTooltip(`${label} • ${new Date(point.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`)}
...
async function renderCoachingMap(){
 ...
 if(coachingLayerVisibility.actual)for(const [userId,userPoints] of liveGroups){const memberRole=coachingMemberRole(userId);if(blindDriver&&userId!==session.user.id)continue;const color=['driver','solo'].includes(memberRole)?'#2498f0':memberRole==='coach'?'#a986ff':memberRole==='traceur'?'#58d6a4':'#91a09d',weight=['driver','solo'].includes(memberRole)?5:3;if(userPoints.length>1)coachingLayers.push(L.polyline(userPoints.map(p=>[p.lat,p.lon]),{color,weight,opacity:.95,lineCap:'round',lineJoin:'round',className:`map-route-line map-route-${memberRole}`}).addTo(coachingMap));if(userPoints.length){const last=userPoints.at(-1),estimated=Number.isFinite(Number(last.heading_deg))?Number(last.heading_deg):headingBetween(userPoints.at(-2),last);coachingLayers.push(participantMarker({...last,heading_deg:estimated},memberRole,coachingRoleLabel(memberRole)))}}
 if(coachingLayerVisibility.trace&&trace.length&&!blindDriver)coachingLayers.push(participantMarker(trace.at(-1),'traceur','Traceur'));
 ...
 const all=[...route,...trace,...allLive],ownLatest=liveGroups.get(session.user.id)?.at(-1);if(!oldView&&ownLatest&&['driver','solo'].includes(role)){coachingMap.setView([ownLatest.lat,ownLatest.lon],17);if(Number.isFinite(Number(ownLatest.heading_deg)))coachingMap.setBearing?.(Number(ownLatest.heading_deg))}else if(all.length&&!oldView)coachingMap.fitBounds(L.latLngBounds(all.map(p=>[p.lat,p.lon])),{padding:[30,30]});
 updateCoachingDeparture();coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true;cancelCoachingLongPress()});installCoachingLongPress();
}
...
function startCoachingPresence(){
 ...
 coachingGpsError='';coachingPresenceLastPointAt=0;$('coachingGpsState').textContent='Recherche de la position…';
 coachingPresenceWatch=navigator.geolocation.watchPosition(pos=>{if(Date.now()-coachingPresenceLastPointAt<5000||pos.coords.accuracy>55)return;coachingPresenceLastPointAt=Date.now();const p={session_id:activeCoachingSession.id,owner_id:session.user.id,lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:pos.coords.accuracy,heading_deg:Number.isFinite(Number(pos.coords.heading))?Number(pos.coords.heading):null,speed_mps:Number.isFinite(Number(pos.coords.speed))?Number(pos.coords.speed):null,recorded_at:new Date(pos.timestamp).toISOString()};coachingOwnPosition=p;updateCoachingDeparture();updateFakeLock();supabase.from('coaching_live_points').insert(p).then(({error})=>{$('coachingGpsState').textContent=error?'Position non envoyée : '+error.message:`Position partagée • précision ${Math.round(p.accuracy_m)} m`;if(!error)scheduleCoachingMapRender()}).catch(()=>{$('coachingGpsState').textContent='Hors réseau — position en attente'})},err=>{coachingGpsError=err.message||'autorisation refusée';stopCoachingPresence()},{enableHighAccuracy:true,maximumAge:1000,timeout:15000});updateCoachingPrimaryActions()
}
async function startActiveCoaching(){
 if(!activeCoachingSession||!await ensureCoachingStarted())return;
 $('coachingLivePanel').classList.remove('waiting-room');$('coachingWaitingActions').classList.add('hidden');setCoachingStage('live');setCoachingPanel('team');updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();await renderCoachingMap()
}
async function toggleCoachingGpsTracking(){const s=activeCoachingSession,role=coachingGpsRole(s);if(!s||s.status!=='live'||!navigator.geolocation||!coachingGpsCapable(s)){updateCoachingPrimaryActions();return}if(isCoachingGpsTracking(s)){coachingGpsError='';if(role==='traceur')stopTraceurTracking();else stopCoachingPresence();closeFakeLock();return}coachingGpsError='';if(role==='traceur')await startTraceurTracking();else startCoachingPresence();updateCoachingPrimaryActions()}
...
async function loadSavedCoachingDebrief(){if(!activeCoachingSession)return;const {data,error}=await supabase.from('coaching_debriefs').select('*').eq('session_id',activeCoachingSession.id).maybeSingle();if(error||!data)return;const form=$('coachingDebriefForm');if(form){for(const key of ['strengths','improvement_area','coach_notes','statistics_notes'])if(form.elements[key])form.elements[key].value=data[key]||''}const el=$('coachingAutoDebrief');if(el&&data.publication_status==='published')el.insertAdjacentHTML('beforeend',`<div class="saved-debrief"><small class="section-kicker">DÉBRIEF PUBLIÉ</small>${data.strengths?`<h4>Points forts</h4><p>${esc(data.strengths)}</p>`:''}${data.improvement_area?`<h4>Axe de progression</h4><p>${esc(data.improvement_area)}</p>`:''}${data.coach_notes?`<h4>Notes du coach</h4><p>${esc(data.coach_notes)}</p>`:''}${data.statistics_notes?`<h4>Données complémentaires</h4><p>${esc(data.statistics_notes)}</p>`:''}<small>Mis à jour le ${new Date(data.updated_at).toLocaleString('fr-FR')}</small></div>`)}
...
if(window.PointerEvent){$('fakeUnlockBtn').addEventListener('pointerdown',beginFakeUnlock);$('fakeUnlockBtn').addEventListener('pointerup',finishFakeUnlock);$('fakeUnlockBtn').addEventListener('pointercancel',cancelFakeUnlock)}
else{$('fakeUnlockBtn').addEventListener('touchstart',beginFakeUnlock,{passive:false});$('fakeUnlockBtn').addEventListener('touchend',finishFakeUnlock,{passive:false});$('fakeUnlockBtn').addEventListener('touchcancel',cancelFakeUnlock,{passive:false})}
...
$('fakeUnlockEmergency').onclick=()=>closeFakeLock();
window.addEventListener('blur',()=>{if(!$('fakeLockScreen')?.classList.contains('hidden'))closeFakeLock()});
document.addEventListener('visibilitychange',()=>{if(document.hidden&&!$('fakeLockScreen')?.classList.contains('hidden'))closeFakeLock()});
