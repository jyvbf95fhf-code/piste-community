...
async function openCoachingSession(id){
 ...
 let s=coachingSessions.find(x=>x.id===id);if(!s){const {data}=await supabase.from('coaching_sessions').select('*,coaching_members(role,user_id)').eq('id',id).single();s=data}if(!s)return;
 activeCoachingSession=s;coachingAutoMetrics=null;const role=isSoloCoaching(s)?'solo':myCoachingRole(s);
 $('coachingLivePanel').classList.remove('hidden');$('coachingRole').textContent=coachingRoleLabel(role).toUpperCase();$('coachingLiveTitle').textContent=s.name||'Session coachée';$('coachingLiveCode').textContent=s.invite_code;$('coachingLiveStatus').textContent=`${coachingStatusLabel(s.status)} • indices ${s.visibility_mode==='all'?'partagés':s.visibility_mode==='progressive'?'progressifs':'réservés au coach'}`;
 ...
 updateCoachingPreflight();setCoachingStage(s.status==='ended'?'debrief':'live');await renderCoachingMap();await loadCoachingMessages();
...
async function renderCoachingMap(){
 ...
 setCoachingReplayData(trace,points,annotations);updateCoachingParticipants(liveGroups,trace);
...
async function ensureCoachingStarted(){if(!activeCoachingSession)return false;if(activeCoachingSession.status==='live')return true;const {error}=await supabase.rpc('start_coaching_session',{p_session_id:activeCoachingSession.id});if(error){alert('Départ impossible : '+error.message);return false}activeCoachingSession.status='live';$('coachingLiveStatus').textContent='En direct • positions GPS synchronisées';return true}
