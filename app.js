...
function leafletB2DistanceMeters(a,b){try{if(!a||!b)return null;return Math.round(hav({lat:Number(a.lat),lon:Number(a.lon)},{lat:Number(b.lat),lon:Number(b.lon)}))}catch{return null}}
...
function leafletB2DebugMapSnapshot(map,marker,position,extra={}){
 ...
 try{centerNear=!!center&&!!position&&Math.hypot((center.lat-position.lat)*111000,(center.lng-position.lng)*111000*Math.cos(center.lat*Math.PI/180))<100;markerNear=!!latlng&&!!position&&Math.hypot((latlng.lat-position.lat)*111000,(latlng.lng-position.lng)*111000*Math.cos(latlng.lat*Math.PI/180))<100}catch{}
