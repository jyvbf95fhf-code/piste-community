...
function coachingActiveSurfaceModel(s,phase=coachingPhase(s),role=myCoachingRole(s)){
  const rawRole=role,displayRole=rawRole==='solo'?'driver':rawRole,knownRole=['driver','traceur','observer','coach','solo'].includes(rawRole),active=!!s&&s.status==='live'&&phase!=='completed',finishTarget=Number(s?.workflow_version)>=2?'driver':'legacy';
  const capabilities=knownRole?coachingMemberCapabilities({role:rawRole},s):[],visibility=knownRole?coachingDataVisibility(s,displayRole):{planned:false,trace:false,live:false,markers:false,participants:false};
...
async function submitCoachingFinishHold(sessionId){
 ...
 try{
  ...
  confirmed=true;await refreshActiveCoachingSession(sessionId);
...
function applyV1040RoleSurface(){
 ...
 else if(v1040&&role==='driver'&&phase==='driver_running')show('driverFinishBtn');
