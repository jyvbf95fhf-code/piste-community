...
function updateTerrainCommonStatus(){
 ...
 const reference=recordMode==='operational'?operationalTrackAgeReference():(activeCoachingSession?.track_finished_at||activeCoachingSession?.created_at||null);
 setUiText('terrainLiveAge',reference?`Âge de piste : ${formatExactDuration(TerrainEngine.ageMs(reference))}`:'Âge de piste : —');
 if(recordMode==='operational')setUiText('opsAgeDisplay',reference?`Âge actuel de la piste : ${formatExactDuration(TerrainEngine.ageMs(reference))}`:'Âge actuel de la piste : —');
...
function operationalTrackAgeReference(){const form=$('pisteForm'),value=form?.elements?.disparition_at?.value||form?.elements?.depart_at?.value||gps.terrainStartedAt||gps.start;return value?new Date(value):null}
