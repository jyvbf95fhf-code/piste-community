...
const PisteTerrainEngine={
 ...
 createMap(id,options={}){if(this.maps.has(id))this.destroyMap(id);const map=L.map(id,{...this.mapOptions,...options}),root=$(id);if(root&&!root.nextElementSibling?.classList.contains('trace-palette-legend'))root.insertAdjacentHTML('afterend',traceLegendHtml());addCleanBaseLayers(map);this.maps.set(id,{map,layers:new Map(),baseLayer:'osm'});terrainDebugRefresh();return map},
 ...
 setBaseLayer(id,name){const entry=this.maps.get(id);if(!entry)return false;const map=entry.map,known={osm:'OpenStreetMap',topo:'Carte terrain'};const controls=map._layers?Object.values(map._layers):[];const tile=controls.find(layer=>layer instanceof L.TileLayer&&((name==='osm'&&layer._url.includes('openstreetmap'))||(name==='topo'&&layer._url.includes('opentopomap'))));if(tile&&!map.hasLayer(tile))tile.addTo(map);for(const layer of controls)if(layer instanceof L.TileLayer&&layer!==tile&&map.hasLayer(layer))map.removeLayer(layer);entry.baseLayer=name;terrainDebugRefresh();return Boolean(tile)},
...
function terrainDebugSnapshot(){const snapshot=PisteTerrainEngine.snapshot();return{engine:'PisteTerrainEngine',mode:PISTE_TERRAIN_ENGINE_MODE,provider:'leaflet',maps:snapshot.maps,instances:snapshot.instances,pdfSnapshot:window.PisteMapSnapshotProvider?.snapshotDebug?.()||null,online:navigator.onLine!==false,viewport:{width:window.innerWidth,height:window.innerHeight},lastAction:terrainDebugState.lastAction}};
...
function initPlanner(route=null){
 setTimeout(()=>{
  ...
  }else plannerMap=PisteTerrainEngine.createMap('plannerMap',{zoomControl:true}).setView([48.3,7.45],9);
