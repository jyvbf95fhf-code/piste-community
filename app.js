...
async function savePlanner(mode='copy',afterSave='library'){
 ...
 if(afterSave==='start'&&saved){selectedTrainingRoute=saved;beginNewPiste('training',{preservePreparedRoute:true});setTimeout(()=>applySelectedTrainingRoute(),150);return}
...
function beginNewPiste(mode='piste',options={}){
 recordMode=mode;terrainDisappearanceAt=null;TerrainEngine.configure(mode==='training'?'training':'operational');if(mode!=='training'&&!options.preservePreparedRoute)selectedTrainingRoute=null;resetGpsUI();updateTerrainCommonStatus();showPage('recordPage');$('pisteForm').reset();$('pisteForm').elements.date.value=today();if($('opsDisappearanceAt'))$('opsDisappearanceAt').value='';if(!activeOperationalCallId)$('operationalCallBanner')?.classList.add('hidden');
