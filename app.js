...
const leafletB2Debug={panel:null,current:null,sessions:new Map()};
...
function leafletB2DebugMapSnapshot(map,marker,position,extra={}){if(!LEAFLET_B2_DEBUG_ENABLED)return;const container=map?.getContainer?.(),rect=container?.getBoundingClientRect?.(),element=marker?.getElement?.(),style=element&&typeof getComputedStyle==='function'?getComputedStyle(element):null,latlng=marker?.getLatLng?.(),center=map?.getCenter?.();let near=false;try{near=!!center&&!!position&&Math.hypot((center.lat-position.lat)*111000,(center.lng-position.lon)*111000*Math.cos(center.lat*Math.PI/180))<100}catch{}leafletB2DebugLog('map-state',{...extra,mapExists:!!map,mapLoaded:!!map?._loaded,containerConnected:!!container?.isConnected,containerVisible:!!container&&!!rect&&rect.width>0&&rect.height>0,mapSize:rect?Math.round(rect.width)+'x'+Math.round(rect.height):'0x0',mapCenterNearOwnPosition:near,zoom:map?.getZoom?.()??null,markerExists:!!marker,markerAttached:!!marker&&marker._map===map,markerHasLayer:!!map?.hasLayer?.(marker),markerElement:!!element,markerElementConnected:!!element?.isConnected,markerDisplay:style?.display||'—',markerVisibility:style?.visibility||'—',markerOpacity:style?.opacity||'—',markerTransform:style?.transform||'—',markerZIndex:style?.zIndex||element?.style?.zIndex||'—',markerPane:element?.parentElement?.className||'—',markerSize:element?Math.round(element.getBoundingClientRect().width)+'x'+Math.round(element.getBoundingClientRect().height):'0x0',markerLatLngValid:Number.isFinite(Number(latlng?.lat))&&Number.isFinite(Number(latlng?.lng))})}
function leafletB2DebugEnsurePanel(){if(!LEAFLET_B2_DEBUG_ENABLED||leafletB2Debug.panel)return;const create=()=>{if(leafletB2Debug.panel||!document.body)return;const panel=document.createElement('aside');panel.id='leafletB2DebugPanel';panel.setAttribute('aria-label','DEBUG LEAFLET BLOC 2');panel.style.cssText='position:fixed;left:8px;right:8px;bottom:max(8px,env(safe-area-inset-bottom));z-index:10000;max-height:46dvh;overflow:auto;padding:9px 10px;border:1px solid #6ec9e8;border-radius:10px;background:rgba(8,16,24,.95);color:#edf7fb;font:10px/1.3 ui-monospace,monospace;box-shadow:0 5px 20px #0008';document.body.append(panel);leafletB2Debug.panel=panel;leafletB2DebugRender()};if(document.body)create();else document.addEventListener('DOMContentLoaded',create,{once:true})}
function leafletB2DebugRender(){if(!leafletB2Debug.panel)return;const s=leafletB2Debug.current||{},fmt=v=>v===undefined||v===null||v===''?'—':String(v),yes=v=>v?'oui':'non',fields=[['Session',s.sessionId],['Rôle',s.role],['Map',yes(s.mapExists)],['Map loaded',yes(s.mapLoaded)],['Conteneur DOM',yes(s.containerConnected)],['Conteneur visible',yes(s.containerVisible)],['Taille',s.mapSize],['Centre proche',yes(s.mapCenterNearOwnPosition)],['Zoom',s.zoom],['Marker',yes(s.markerExists)],['Marker map courante',yes(s.markerAttached)],['hasLayer',yes(s.markerHasLayer)],['Marker DOM',yes(s.markerElement)],['Marker DOM connecté',yes(s.markerElementConnected)],['display',s.markerDisplay],['visibility',s.markerVisibility],['opacity',s.markerOpacity],['transform',s.markerTransform],['z-index',s.markerZIndex],['pane',s.markerPane],['taille marker',s.markerSize],['LatLng valide',yes(s.markerLatLngValid)],['Own position',yes(s.ownPosition)],['Marker proche own',yes(s.markerNearOwnPosition)],['invalidateSize',yes(s.invalidateSizeCalled)],['setLatLng',yes(s.setLatLngCalled)],['addTo',yes(s.addToCalled)],['recentrage',yes(s.recenterCalled)],['Dernier événement',s.lastEvent]];const rows=fields.map(([k,v])=>'<div><b>'+k+'</b> : '+fmt(v)+'</div>').join('');const compare=[...leafletB2Debug.sessions.values()].slice(-2).map(row=>'<tr><td>'+fmt(row.sessionId)+'</td><td>'+yes(row.mapExists)+'</td><td>'+fmt(row.mapSize)+'</td><td>'+yes(row.markerAttached)+'</td><td>'+yes(row.markerHasLayer)+'</td><td>'+yes(row.markerElementConnected)+'</td><td>'+yes(row.containerVisible)+'</td><td>'+yes(row.mapCenterNearOwnPosition)+'</td><td>'+yes(row.invalidateSizeCalled)+'</td><td>'+yes(row.recenterCalled)+'</td></tr>').join('');leafletB2Debug.panel.innerHTML='<strong>DEBUG LEAFLET BLOC 2</strong>'+rows+'<hr style="border:0;border-top:1px solid #2f5361"><div><b>Comparaison A/B</b></div><table style="width:100%;font-size:9px"><thead><tr><th>Session</th><th>map</th><th>size</th><th>attach</th><th>layer</th><th>DOM</th><th>visible</th><th>center</th><th>size</th><th>recenter</th></tr></thead><tbody>'+compare+'</tbody></table>'}
