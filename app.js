...
let coachingSessionEndHandledId=null,coachingV1040FinishInFlight=false,coachingV10423TransitionInFlight=false;
...
async function openCoachingDebriefOnce(id,statusText){if(activeCoachingSession?.id!==id)return false;if(coachingDriverTrackPending()){await showCoachingDriverTrackFinish(id);return false}if(coachingSessionEndHandledId===id)return false;coachingSessionEndHandledId=id;stopCoachingPresence();stopTraceurTracking();await closeFakeLock({returnToMap:false,reacquireWake:false});clearCoachingRealtime();clearVerifiedActiveCoaching(id);$('coachingLivePanel')?.classList.remove('waiting-room');updateCoachingPhase();updateCoachingPrimaryActions();updateCoachingDebriefAccess();setCoachingStage('debrief');refreshCoachingMapLayout();setUiText('coachingLiveStatus',statusText);await calculateCoachingDebrief();await loadSavedCoachingDebrief();$('coachingDebriefStage')?.scrollIntoView({block:'start',behavior:'smooth'});return true}
async function handleCoachingSessionChange(payload){if(!activeCoachingSession||payload.new?.id!==activeCoachingSession.id)return;const id=payload.new.id,previousPhase=coachingPhase(activeCoachingSession);applySafeCoachingRealtimeStatus(payload.new);const realtimeStatus=activeCoachingSession.status,realtimePhase=coachingPhase(activeCoachingSession);await refreshActiveCoachingSession(id);const nextPhase=coachingPhase(activeCoachingSession),role=myCoachingRole(activeCoachingSession),owner=isCoachingOwner(activeCoachingSession),justCompleted=previousPhase!=='completed'&&(realtimePhase==='completed'||nextPhase==='completed');if(nextPhase!==previousPhase&&['waiting_ready','coach_ready'].includes(nextPhase)){const traceur=activeCoachingSession.coaching_members?.find(m=>m.role==='traceur');coachingToast(role==='driver'?(coachingSearchPendingV1045(activeCoachingSession)?'Piste tracée — le Traceur choisit la suite de la recherche.':activeCoachingSession.search_mode==='deferred'&&!activeCoachingSession.traceur_ready_at?'Piste tracée — en attente du retour du Traceur.':'Piste prête — vous pouvez démarrer.'):`${traceur?coachingParticipantName(traceur):'Le Traceur'} a terminé de tracer la piste.`);}if(justCompleted&&role==='driver')coachingToast('Parcours terminé');else if(justCompleted&&(role==='coach'||owner))coachingToast('Parcours du Conducteur terminé');syncCoachingPanelStatus();if(nextPhase==='completed'){await openCoachingDebriefOnce(id,'Parcours terminé • Débrief disponible');return}if(['ended','cancelled'].includes(realtimeStatus)){await openCoachingDebriefOnce(id,realtimeStatus==='ended'?'Session terminée par l’organisateur • débrief disponible':'Session annulée par l’organisateur');return}if(activeCoachingSession.status==='live'){verifiedActiveCoachingSession=activeCoachingSession;saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();setCoachingStage('live')}updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();refreshCoachingMapLayout();}
...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingSessionEndHandledId=null;coachingAutoMetrics=null;coachingGpsError='';coachingGpsReady=false;if(Number(s.workflow_version)>=2){const refreshed=await refreshActiveCoachingSession(s.id);if(refreshed)s=activeCoachingSession}requestCoachingPreviewLocation();$('coachingDebriefForm')?.reset();$('coachingDriverFeedbackForm')?.reset();setUiText('coachingDebriefState','Aucun brouillon chargé.');setUiText('coachingDriverFeedbackState','Aucun retour enregistré.');const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
 ...
 updateCoachingPreflight();updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();updateCoachingPrimaryActions();setCoachingPanel('team');setCoachingStage(s.status==='ended'||coachingPhase(s)==='completed'?'debrief':s.status==='waiting'?'room':'live');renderCoachingLiveWeather();updateCoachingTerrainStatus();fetchCoachingLiveWeather();scheduleCoachingLiveWeather();await renderCoachingMap();refreshCoachingMapLayout();await loadCoachingMessages();if(s.status==='ended'||coachingPhase(s)==='completed'){await openCoachingDebriefOnce(s.id,'Parcours terminé • Débrief disponible');return true}
 coachingChannel=supabase.channel(`coaching-${s.id}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'coaching_sessions',filter:`id=eq.${s.id}`,select:['id','status','phase','driver_finished_at','started_at','ended_at','updated_at']},handleCoachingSessionChange).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_live_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_trace_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_markers',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_messages',filter:`session_id=eq.${s.id}`},payload=>handleIncomingCoachingMessage(payload)).subscribe();
