...
function updateCoachingPrimaryActions(){
 ...
 if(Number(s?.workflow_version)>=2){hideLegacyCoachingPrimaryActions();if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();const phase=coachingPhase(s),completed=phase==='completed'||!!s.driver_finished_at;const message=completed?'Parcours terminé • Débrief disponible.':role==='driver'?(phase==='preparation'?'En attente : le Traceur prépare la piste.':phase==='laying'?'Pose en cours : le Traceur trace actuellement la piste.':phase==='waiting_ready'||phase==='coach_ready'?'La piste est prête — vous pouvez démarrer.':phase==='driver_running'?'Parcours en cours.':''):isCurrentUserLayingActor(s)?(phase==='preparation'?'Appuyez sur « Je pars tracer » pour démarrer la pose et le GPS.':phase==='laying'?'Pose en cours — terminez puis validez « Piste tracée ».':''):'';setUiText('coachingRoleInstruction',message);return}
...
$('openTerrainHomeBtn').onclick=e=>{e.preventDefault();beginNewPiste('training')};
