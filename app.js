...
function gpxCoordinate(element){const lat=Number(element.getAttribute('lat')),lon=Number(element.getAttribute('lon'));if(!Number.isFinite(lat)||!Number.isFinite(lon)||Math.abs(lat)>90||Math.abs(lon)>180)return null;const rawTime=gpxText(element,'time');if(!rawTime)return{lat,lon};const timeMs=new Date(rawTime).getTime();if(!Number.isFinite(timeMs))throw new Error('Le fichier GPX contient une date ou une heure invalide.');return{lat,lon,recorded_at:new Date(timeMs).toISOString()}}
...
function parseGpx(text,manualOrigin={}){
 ...
 const raw=nodes.map(gpxCoordinate).filter(Boolean),currentTime=new Date().getTime(),futurePoint=raw.find(point=>point.recorded_at&&new Date(point.recorded_at).getTime()>currentTime);if(futurePoint)throw new Error('Le fichier GPX contient une heure future à corriger.');const firstTimed=raw.find(point=>point.recorded_at),originAt=firstTimed?.recorded_at||parseExplicitGpxOrigin(manualOrigin),originSource=firstTimed?'gpx_embedded_time':'gpx_manual_time',deduped=raw.filter((p,i)=>!i||p.lat!==raw[i-1].lat||p.lon!==raw[i-1].lon);if(deduped.length<2)throw new Error('La trace GPX ne contient pas assez de positions différentes.');if(!firstTimed)deduped[0]={...deduped[0],recorded_at:originAt};
