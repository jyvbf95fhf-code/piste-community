...
function coachingCanSeeOdor(s=activeCoachingSession,role=myCoachingRole(s)){if(!s||!['driver','solo','traceur','observer','coach'].includes(role)||!coachingLayerVisibility.odor)return false;const normalizedRole=role==='solo'?'driver':role,surface=typeof coachingActiveSurfaceModel==='function'?coachingActiveSurfaceModel(s,typeof coachingPhase==='function'?coachingPhase(s):s.phase,role):{odorVisible:!!coachingDataVisibility(s,normalizedRole)?.trace};if(!surface.odorVisible)return false;return getLocalOdorPreference(session?.user?.id,s.id)}
