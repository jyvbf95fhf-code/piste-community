...
function isCurrentUserLayingActor(s=activeCoachingSession){if(!s)return false;const role=myCoachingRole(s);if(s.visibility_version===3)return role==='traceur'||isSoloCoaching(s);return(role==='traceur'&&s.laying_mode==='traceur')||(isCoachingOwner(s)&&role==='coach'&&s.laying_mode==='coach')}
function coachingGpsCapable(s=activeCoachingSession){return ['driver','solo','traceur','coach'].includes(coachingGpsRole(s))}
function isCoachingGpsTracking(s=activeCoachingSession){return (coachingPhase(s)==='laying'&&coachingGpsRole(s)==='solo')||coachingGpsRole(s)==='traceur'?traceurWatch!==null:coachingPresenceWatch!==null}
...
async function resumeSoloCoachingPhaseGps(s=activeCoachingSession){if(!isSoloCoaching(s)||s.status!=='live')return false;const phase=coachingPhase(s);if(phase==='laying')return startTraceurTracking();if(phase==='driver_running'){startCoachingPresence();return true}return false}
...
function coachingSurfaceActionTarget(action,s){
 const targets={pause:'#terrainPauseBtn',blackScreen:'#terrainBlackScreenBtn',messages:'#terrainMessagesBtn',plus:'#terrainPlusBtn',startDriver:'#driverStartBtn',startLaying:'#startLayingBtn',finishLaying:'#finishLayingBtn',trackReady:'#trackReadyBtn',finish:Number(s?.workflow_version)>=2?'#driverFinishBtn':'#terrainFinishBtn'};
...
async function submitCoachingFinishHold(sessionId){
 ...
 let confirmed=false;
 try{
  const {data,error}=await supabase.rpc('finish_coaching_track_v1049',{p_session_id:sessionId});
...
function applyV1040RoleSurface(){
 const s=activeCoachingSession;if(!s)return;const member=myCoachingMember(s),role=member?.role||null,phase=coachingPhase(s),surface=coachingActiveSurfaceModel(s,phase,role),owner=isCoachingOwner(s),v1040=Number(s.workflow_version)>=2,completed=phase==='completed',informativeCompleted=completed||!!s.driver_finished_at,layingActor=isCurrentUserLayingActor(s),hide=id=>$(id)?.classList.add('hidden'),show=id=>$(id)?.classList.remove('hidden');
 ...
 else if(v1040&&layingActor&&phase==='preparation'){setUiText('startLayingBtn',role==='solo'?'Départ de piste':'Je pars tracer');show('startLayingBtn')}
 else if(v1040&&layingActor&&phase==='laying'){setUiText('trackReadyBtn',role==='solo'?'Fin de pose':'Piste tracée');show('trackReadyBtn')}
 else if(v1040&&(role==='driver'||role==='solo')&&['waiting_ready','coach_ready'].includes(phase)){if(role==='solo')setUiText('driverStartBtn','Démarrer le relevé');show('driverStartBtn')}
