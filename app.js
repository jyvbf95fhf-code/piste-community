...
async function openCoachingSession(id,{resume=false}={}){
 ...
 updateCoachingPreflight();updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();updateCoachingPrimaryActions();setCoachingPanel('team');setCoachingStage(globalPhase==='active'?(s.status==='waiting'?'room':coachingDriverTrackPending(s)?'finish':'live'):'debrief');renderCoachingLiveWeather();updateCoachingTerrainStatus();fetchCoachingLiveWeather();scheduleCoachingLiveWeather();if(typeof loadCoachingScenario==='function')await loadCoachingScenario(s.id);await renderCoachingMap();await resumeSoloCoachingPhaseGps(s);refreshCoachingMapLayout();await loadCoachingMessages();if(globalPhase!=='active'){await openCoachingDebriefOnce(s.id,'Piste terminée • Débrief disponible');return true}
 coachingChannel=supabase.channel(`coaching-${s.id}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'coaching_sessions',filter:`id=eq.${s.id}`,select:['id','status','phase','debrief_status','driver_finished_at','track_finished_at','started_at','ended_at','updated_at']},handleCoachingSessionChange).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_live_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_trace_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_markers',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_messages',filter:`session_id=eq.${s.id}`},payload=>handleIncomingCoachingMessage(payload)).subscribe();
 if(typeof subscribeCoachingScenarioRealtime==='function')subscribeCoachingScenarioRealtime(s.id);
