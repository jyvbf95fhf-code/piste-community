...
let coachingScenarioState={status:'absent',sessionId:null,scenario:null,reads:[],sync:'idle',lastEvent:'—',error:null};
...
const preReleaseCoachingDebug={events:[],submitClicks:0,lastException:null,lastSupabaseError:null,startedAt:Date.now(),timer:null};
const coachingMapDebug={tracePointsCount:0,livePointsCount:0,currentPositionsCount:0,traceurLastPointAvailable:false,driverLastPointAvailable:false,participantMarkersCount:0,markerRoles:[],traceurTrackPresent:false,driverMarkerPresent:false,actualVisible:null,traceVisible:null,traceurCanSeeLiveOwner:null,driverCanSeeLiveOwner:null,participantMarkerCalled:{traceur:false,driver:false},markerAddedToMap:false,markerElementPresent:false,markerRemovedDuringCleanup:false,lastRenderError:null,renderInFlight:false,markerHasLayer:false,markerMapMatches:false,markerPaneClass:null,markerDisplay:null,markerVisibility:null,markerOpacity:null,markerZIndex:null};
function coachingMapDebugReset(){const layers=typeof coachingLayerVisibility!=='undefined'?coachingLayerVisibility:{};Object.assign(coachingMapDebug,{tracePointsCount:0,livePointsCount:0,currentPositionsCount:0,traceurLastPointAvailable:false,driverLastPointAvailable:false,participantMarkersCount:0,markerRoles:[],traceurTrackPresent:false,driverMarkerPresent:false,actualVisible:!!layers.actual,traceVisible:!!layers.trace,traceurCanSeeLiveOwner:null,driverCanSeeLiveOwner:null,participantMarkerCalled:{traceur:false,driver:false},markerAddedToMap:false,markerElementPresent:false,markerRemovedDuringCleanup:false,lastRenderError:null,renderInFlight:true,markerHasLayer:false,markerMapMatches:false,markerPaneClass:null,markerDisplay:null,markerVisibility:null,markerOpacity:null,markerZIndex:null})}
function coachingMapDebugEvent(name,details=''){preReleaseDebugEvent(name,details)}
function coachingMapDebugRefreshMarkers(){coachingMapDebug.participantMarkersCount=coachingParticipantMarkers.size;coachingMapDebug.markerRoles=[...coachingParticipantMarkers.values()].map(marker=>marker._pisteRole).filter(Boolean);coachingMapDebug.traceurTrackPresent=coachingParticipantMarkers.has('traceur-track');coachingMapDebug.driverMarkerPresent=[...coachingParticipantMarkers.values()].some(marker=>['driver','solo'].includes(marker._pisteRole))}
function preReleaseDebugSanitize(value){let text;if(value instanceof Error)text=value.message;else if(value&&typeof value==='object'){try{text=value.message||value.error_description||JSON.stringify({code:value.code,details:value.details,hint:value.hint,message:value.message})}catch{text='Erreur inconnue'}}else text=String(value??'');return text.replace(/[0-9a-f]{8}-[0-9a-f-]{27,}/gi,'[id]').replace(/[A-Fa-f0-9]{8,}/g,'[id]').replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g,'[email]').replace(/https?:\/\/\S+/gi,'[url]').replace(/(token|secret|password|apikey|access_token)[^\s,;]*/gi,'$1=[redacted]');}
function preReleaseDebugMembers(members=[]){const current=session?.user?.id;return members.map(member=>({role:member?.role||'—',currentUser:!!current&&member?.user_id===current}));}
function preReleaseDebugRecordError(error,kind='exception'){const clean=preReleaseDebugSanitize(error);if(kind==='supabase')preReleaseCoachingDebug.lastSupabaseError=clean;else preReleaseCoachingDebug.lastException=clean;if(typeof coachingMapDebug!=='undefined'&&coachingMapDebug.renderInFlight)coachingMapDebug.lastRenderError=clean;preReleaseDebugRefresh()}
function preReleaseDebugEvent(name,details=''){preReleaseCoachingDebug.events.push({at:new Date().toLocaleTimeString('fr-FR'),name,details:preReleaseDebugSanitize(details)});if(preReleaseCoachingDebug.events.length>50)preReleaseCoachingDebug.events.splice(0,preReleaseCoachingDebug.events.length-50);preReleaseDebugRefresh()}
function preReleaseDebugSnapshot(){const wizard=coachingWizard||{},roleSelect=$('coachingWizardCreatorRole'),legacyRole=$('coachingCreatorRole'),submit=$('coachingWizardSubmit');let finalValidation={ok:false,message:'indisponible'},participantValidation={ok:false,message:'indisponible'},wizardCanPrepare='indisponible',legacyCanPrepare='indisponible';try{finalValidation=validCoachingWizard()}catch(error){finalValidation={ok:false,message:preReleaseDebugSanitize(error)};preReleaseCoachingDebug.lastException=preReleaseDebugSanitize(error)}try{participantValidation=validCoachingWizardParticipants()}catch(error){participantValidation={ok:false,message:preReleaseDebugSanitize(error)};preReleaseCoachingDebug.lastException=preReleaseDebugSanitize(error)}try{wizardCanPrepare=coachingWizardCanPrepareTrack()}catch(error){wizardCanPrepare=`ERROR: ${preReleaseDebugSanitize(error)}`}try{legacyCanPrepare=coachingCanPrepareRouteV1045()}catch(error){legacyCanPrepare=`ERROR: ${preReleaseDebugSanitize(error)}`}const withoutRoute=coachingWizardWithoutPreparedRoute(),scenario=wizard.scenario||{};coachingMapDebugRefreshMarkers();const layers=typeof coachingLayerVisibility!=='undefined'?coachingLayerVisibility:{};return {step:`${wizard.currentStep||1}/6`,sessionType:wizard.sessionType||'—',mode:wizard.mode||'—',creatorRoleStored:wizard.creatorRole||'—',creatorRoleSelect:roleSelect?.value||'—',legacyCreatorRoleSelect:legacyRole?.value||'—',participantsCount:Array.isArray(wizard.participants)?wizard.participants.length:0,participantRoles:preReleaseDebugMembers(wizard.participants),invitationsCount:Array.isArray(wizard.invitations)?wizard.invitations.length:0,invitationRoles:preReleaseDebugMembers(wizard.invitations),trackMethod:wizard.trackPreparation?.method||'—',routePresent:!!wizard.trackPreparation?.routeId,draftPresent:!!wizard.trackPreparation?.draft,valid:`${finalValidation.ok?'OK':'KO'}${finalValidation.message?` — ${finalValidation.message}`:''}`,participantsValid:`${participantValidation.ok?'OK':'KO'}${participantValidation.message?` — ${participantValidation.message}`:''}`,busy:!!wizard.busy,createInFlight:!!coachingCreateInFlight,submit:`${submit?.hidden?'caché':'visible'} / ${submit?.disabled?'disabled':'enabled'}`,withoutRoute,canPrepareWizard:wizardCanPrepare,canPrepareLegacy:legacyCanPrepare,rpcRole:wizard.sessionType==='solo'?'solo':wizard.creatorRole||'—',routeSent:!withoutRoute&&!!wizard.trackPreparation?.routeId,clicks:preReleaseCoachingDebug.submitClicks,scenarioEnabled:!!scenario.enabled,scenarioStatus:coachingScenarioState.status==='absent'?'absent':(coachingScenarioState.status||'pending'),scenarioPhotoCount:Array.isArray(scenario.photos)?scenario.photos.length:(coachingScenarioState.scenario?.photo_paths||[]).length,scenarioEditorRole:coachingScenarioEditorRole(activeCoachingSession)||scenarioEditorRoleForWizard(),scenarioCanEdit:!!coachingScenarioCanEdit(activeCoachingSession)||(!activeCoachingSession&&scenario.enabled&&scenarioEditorRoleForWizard()!=='—'),scenarioRead:!!coachingScenarioReadByCurrentUser(),scenarioLocked:!!coachingScenarioState.scenario?.locked_at,scenarioSync:coachingScenarioState.sync,scenarioLastEvent:coachingScenarioState.lastEvent,scenarioGateRequired:!!coachingScenarioGate.required,scenarioGateRole:coachingScenarioGate.role||'—',scenarioGateVisible:!!coachingScenarioGate.visible,scenarioSessionAlreadyOpen:!!coachingScenarioGate.sessionOpen,scenarioRealtimeActive:!!coachingScenarioGate.realtimeActive,scenarioReadServer:!!coachingScenarioGate.readServer,scenarioGateBlockedTerrain:!!coachingScenarioGate.blockedTerrain,scenarioActiveSessionHiddenAfterRead:!!coachingScenarioActiveHiddenAfterRead,scenarioAvailableInDebrief:!!coachingScenarioState.scenario,scenarioAvailableInHistory:!!coachingScenarioState.scenario,scenarioViewerOpen:!!coachingScenarioViewer.open,scenarioViewerZoomLevel:coachingScenarioViewer.open?coachingScenarioViewer.scale:1,scenarioViewerPhotoIndex:coachingScenarioViewer.open?coachingScenarioViewer.index+1:0,gpsMap:{...coachingMapDebug,coachingLayerVisibility:{actual:!!layers.actual,trace:!!layers.trace}}};}
function preReleaseDebugEnsurePanel(){let panel=$('preReleaseCoachingDebugPanel');if(panel)return panel;const button=document.createElement('button');button.id='preReleaseCoachingDebugToggle';button.type='button';button.className='pre-release-debug-toggle';button.textContent='Debug';button.setAttribute('aria-expanded','false');const wrap=document.createElement('aside');wrap.id='preReleaseCoachingDebugPanel';wrap.className='pre-release-debug-panel hidden';wrap.setAttribute('aria-label','Diagnostic temporaire Coaching');wrap.innerHTML='<div class="pre-release-debug-head"><strong>Debug Coaching</strong><button type="button" data-debug-action="clear">Effacer le journal</button></div><pre data-debug-snapshot></pre><div class="pre-release-debug-actions"><button type="button" data-debug-action="copy">Copier le diagnostic</button><button type="button" data-debug-action="close">Fermer</button></div><pre data-debug-events></pre>';document.body.append(button,wrap);button.onclick=()=>{const open=wrap.classList.toggle('hidden')===false;button.setAttribute('aria-expanded',String(open));preReleaseDebugRefresh()};wrap.querySelector('[data-debug-action="close"]').onclick=()=>{wrap.classList.add('hidden');button.setAttribute('aria-expanded','false')};wrap.querySelector('[data-debug-action="clear"]').onclick=()=>{preReleaseCoachingDebug.events=[];preReleaseDebugRefresh()};wrap.querySelector('[data-debug-action="copy"]').onclick=async()=>{const text=preReleaseDebugText();try{await navigator.clipboard?.writeText(text);preReleaseDebugEvent('DIAGNOSTIC_COPIED')}catch(error){preReleaseDebugRecordError(error)}};window.addEventListener('error',event=>preReleaseDebugRecordError(event.error||event.message));window.addEventListener('unhandledrejection',event=>preReleaseDebugRecordError(event.reason));preReleaseCoachingDebug.timer=window.setInterval(preReleaseDebugRefresh,700);return wrap}
function preReleaseDebugText(){const snapshot=preReleaseDebugSnapshot();return `Snapshot\n${JSON.stringify(snapshot,null,2)}\n\nDernière exception : ${preReleaseCoachingDebug.lastException||'—'}\nDernière erreur Supabase : ${preReleaseCoachingDebug.lastSupabaseError||'—'}\nRéseau : ${navigator.onLine?'online':'offline'}\n\nÉvénements\n${preReleaseCoachingDebug.events.map(event=>`${event.at} ${event.name}${event.details?` — ${event.details}`:''}`).join('\n')}`}
function preReleaseDebugRefresh(){const panel=$('preReleaseCoachingDebugPanel');if(!panel||panel.classList.contains('hidden'))return;const snapshot=preReleaseDebugSnapshot();const snapshotEl=panel.querySelector('[data-debug-snapshot]'),eventsEl=panel.querySelector('[data-debug-events]');if(snapshotEl)snapshotEl.textContent=`${JSON.stringify(snapshot,null,2)}\nDernière exception : ${preReleaseCoachingDebug.lastException||'—'}\nDernière erreur Supabase : ${preReleaseCoachingDebug.lastSupabaseError||'—'}\nRéseau : ${navigator.onLine?'online':'offline'}`;if(eventsEl)eventsEl.textContent=preReleaseCoachingDebug.events.map(event=>`${event.at} ${event.name}${event.details?` — ${event.details}`:''}`).join('\n')||'Aucun événement';}
function setupPreReleaseCoachingDebug(){preReleaseDebugEnsurePanel();window.addEventListener('online',preReleaseDebugRefresh);window.addEventListener('offline',preReleaseDebugRefresh)}
...
function coachingScenarioRecord(event,error=''){coachingScenarioState.lastEvent=event;coachingScenarioState.sync=error?'KO':'OK';coachingScenarioState.error=error||null;preReleaseDebugEvent(`SCENARIO_${event}`,error);preReleaseDebugRefresh()}
...
function restoreCoachingWizardDraftForSave(draft){
 ...
 plannerPoints=draft.route.map(point=>({...point}));plannerWaypoints=(draft.waypoints||[]).map(point=>({...point}));plannerRedoStack=[];plannerRoutingMode=draft.routing_mode||'trail';plannerTrackOrigin=resolveTrackOrigin(null,draft);plannerOdorModel={...(draft.odor_model||plannerOdorModel)};
...
async function saveCoachingWizardDraft(){
 ...
 preReleaseDebugEvent('ROUTE_SAVE_START');return new Promise(resolve=>{savePlanner('copy','library',{stayOnPage:true,onSaved:saved=>{if(saved)preReleaseDebugEvent('ROUTE_SAVE_OK');else preReleaseDebugEvent('ROUTE_SAVE_FAIL');resolve(saved||null)},onError:()=>{preReleaseDebugEvent('ROUTE_SAVE_FAIL');resolve(null)}})})
}
...
async function finalizeCoachingScenario(sessionId,scenario={}){
 ...
 try{
  ...
  preReleaseDebugRecordError(error,'supabase');coachingScenarioRecord('UPLOAD_ERROR',error?.message||'erreur');
  ...
  if(cleanupError){coachingScenarioState.status='pending';coachingScenarioState.scenario={session_id:sessionId,photo_paths:paths,upload_status:'pending'};preReleaseDebugRecordError(cleanupError,'supabase');coachingScenarioRecord('CLEANUP_ERROR',cleanupError?.message||'nettoyage impossible');throw new Error(`Le scénario reste en attente : le nettoyage des photos a échoué. Réessayez. ${cleanupError?.message||''}`)}
...
async function submitCoachingWizard(){
 const debugState=typeof preReleaseCoachingDebug==='object'?preReleaseCoachingDebug:{submitClicks:0},debugEvent=typeof preReleaseDebugEvent==='function'?preReleaseDebugEvent:()=>{},debugError=typeof preReleaseDebugRecordError==='function'?preReleaseDebugRecordError:()=>{};
 debugState.submitClicks++;debugEvent('SUBMIT_CLICK');if(coachingWizard.busy)return false;debugEvent('FINAL_VALIDATION_START');const valid=validCoachingWizard();if(!valid.ok){debugEvent('FINAL_VALIDATION_FAIL',valid.message);coachingWizard.error=valid.message;renderCoachingWizard();return false}debugEvent('FINAL_VALIDATION_OK');
 coachingWizard.busy=true;coachingWizard.error=null;renderCoachingWizard();let created=null;
 try{
  if(coachingWizard.createdSessionId){debugEvent('OPEN_SESSION_START');const opened=await openCoachingSession(coachingWizard.createdSessionId);if(opened!==false){debugEvent('OPEN_SESSION_OK');coachingWizard.active=false}else debugEvent('OPEN_SESSION_FAIL');debugEvent('SUBMIT_DONE');return true}
  const preparation=coachingWizard.trackPreparation,withoutRoute=typeof coachingWizardWithoutPreparedRoute==='function'?coachingWizardWithoutPreparedRoute():false;
  ...
  const routeId=coachingWizard.sessionType==='solo'?null:(withoutRoute?null:preparation.routeId);debugEvent('CREATE_COACHING_START',`withoutRoute=${withoutRoute?'oui':'non'}`);created=await createCoaching({validated:true,routeId,members:coachingWizardMembers(),blindMode:coachingWizard.mode,withoutRoute,scenario:coachingWizard.scenario,onCreated:data=>{coachingWizard.createdSessionId=data?.id||null}});
  if(!created){debugEvent('CREATE_COACHING_RESULT','KO');debugEvent('SUBMIT_DONE');return false}debugEvent('SESSION_CREATED');coachingWizard.createdSessionId=created.id;coachingWizard.active=false;debugEvent('CREATE_COACHING_RESULT','OK');debugEvent('SUBMIT_DONE');return true
 }catch(error){debugError(error);debugEvent('CREATE_COACHING_RESULT',`KO — ${error.message||'erreur'}`);debugEvent('SUBMIT_DONE');coachingWizard.error=error.message||'Création impossible.';renderCoachingWizard();return false}
...
let fieldMarkers=[],pendingFieldMarker=null,operationalLiveWeather=null,operationalWeatherHistory=[],operationalWeatherTimer=null,operationalWeatherLoading=false,operationalWeatherOdorLayers=[],operationalCorridorVisible=true,operationalAgeTimer=null,terrainDisappearanceAt=null,activityLibraryView='list',activityLibraryFilters={type:'all',status:'active',favorite:false,query:''},activityLibrarySelection=[],activityLibrarySelectionMode=false,currentActivityDetail=null,reportCurrentModel=null,reportPhotoUrls=[];
...
function updateOdorPreview(){plannerOdorModel=readOdorForm();plannerOdorLayers.forEach(x=>x.remove());plannerOdorLayers=[];const shared=sharedOlfactionEngine({referenceTrack:plannerPoints,referenceTime:$('weatherPlannedAt')?.value?new Date($('weatherPlannedAt').value):new Date(),currentTime:new Date(),liveWeather:plannerOdorModel,module:'training',terrainContext:{environment:plannerOdorModel.environment}}),g=plannerMap?addOdorLayers(plannerMap,plannerPoints,plannerOdorModel,plannerOdorLayers):null,summary=$('odorSummary');if(summary)summary.textContent=!plannerOdorModel.enabled?'Simulation désactivée.':g?`Couloir olfactif estimé · conditions ${shared.conditionLevel} · dérive indicative ${Math.round(g.drift)} m. Estimation d’aide à l’interprétation.`:'Ajoute au moins deux points pour afficher le couloir.'}
...
function togglePlannerFollow(){if(plannerFollowWatch!==null){stopPlannerFollow();return}if(!navigator.geolocation){$('plannerLocationStatus').textContent='Géolocalisation non disponible.';return}document.querySelector('[data-planner-mode="follow"]')?.classList.add('active');$('plannerLocationStatus').textContent='Marche GPS en direct : le tracé suit ta position.';plannerFollowWatch=navigator.geolocation.watchPosition(appendPlannerGpsPoint,plannerLocationError,{enableHighAccuracy:true,maximumAge:1000,timeout:20000})}
function stopPlannerFollow(){if(plannerFollowWatch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(plannerFollowWatch);plannerFollowWatch=null;document.querySelector('[data-planner-mode="follow"]')?.classList.remove('active')}
...
function redrawPlanner(fit=true){
 ...
 if(plannerPoints.length){
  plannerLine=L.polyline(plannerPoints.map(p=>[p.lat,p.lon]),{weight:4,color:plannerImportedGpx?TRACE_PALETTE.external:TRACE_PALETTE.planned,opacity:1,lineCap:'round',lineJoin:'round',className:'map-route-line map-route-planned'}).addTo(plannerMap);
...
async function createCoaching(options={}){
 const debugEvent=typeof preReleaseDebugEvent==='function'?preReleaseDebugEvent:()=>{},debugError=typeof preReleaseDebugRecordError==='function'?preReleaseDebugRecordError:()=>{};
 ...
 try{
  const members=options.members||coachingCreationMembers(),scenario=options.scenario&&options.scenario.enabled?options.scenario:{enabled:false,text:'',photos:[]},payload={p_route_id:withoutRoute?null:route.id,p_members:members,p_blind_mode:options.blindMode||$('coachingVisibility')?.value||'normal'};if(scenario.enabled){payload.p_scenario_enabled=true;payload.p_scenario_text=String(scenario.text||'');payload.p_photo_paths=[]}debugEvent('RPC_CREATE_START',JSON.stringify({blind_mode:payload.p_blind_mode,route_id_present:!!payload.p_route_id,members:typeof preReleaseDebugMembers==='function'?preReleaseDebugMembers(members):members.map(member=>({role:member?.role||'—'})),scenario_enabled:!!scenario.enabled,scenario_photo_count:scenario.photos?.length||0}));
  ...
  if(error){debugError(error,'supabase');debugEvent('RPC_CREATE_FAIL',error.message);setUiText('coachingCreateMsg','Création impossible : '+error.message);return false}if(!data?.id){debugEvent('RPC_CREATE_FAIL','session absente');setUiText('coachingCreateMsg','Création impossible : session absente');return false}debugEvent('RPC_CREATE_OK');
  ...
  debugEvent('LOAD_COACHING_HUB_START');try{await loadCoachingHub();debugEvent('LOAD_COACHING_HUB_OK')}catch(error){debugError(error);throw error}debugEvent('OPEN_SESSION_START');try{const opened=await openCoachingSession(data.id);if(opened===false){debugEvent('OPEN_SESSION_FAIL');return false}debugEvent('OPEN_SESSION_OK')}catch(error){debugError(error);debugEvent('OPEN_SESSION_FAIL',error.message);throw error}coachingFriendInvites=[];renderCoachingFriendInvites();markCoachingSyncV10423();setUiText('coachingCreateMsg','Session créée. Les participants peuvent rejoindre le départ.');
  return data
 }catch(e){debugError(e);setUiText('coachingCreateMsg','Création impossible : '+e.message);return false}finally{coachingCreateInFlight=false;if(button)button.disabled=false}
}
...
function closeScenarioPhotoViewer(){const viewer=$('coachingScenarioPhotoViewer');if(viewer)viewer.classList.add('hidden');document.body.classList.remove('scenario-viewer-open');if(coachingScenarioViewer.open)preReleaseDebugEvent('SCENARIO_PHOTO_VIEW_CLOSE');coachingScenarioViewer={...coachingScenarioViewer,open:false,scale:1,panX:0,panY:0,touchStartX:null,touchStartY:null,touchStartDistance:null}}
...
async function renderCoachingScenarioGate(){const box=$('coachingScenarioGate'),decision=scenarioGateDecision();if(!box||!activeCoachingSession||!decision.scenario||!decision.required){if(box)box.classList.add('hidden');if(coachingScenarioGate.visible){coachingScenarioGate.visible=false;preReleaseDebugEvent('SCENARIO_GATE_CLOSE')}return}const photos=await scenarioPhotoUrls(Array.isArray(decision.scenario.photo_paths)?decision.scenario.photo_paths:[]);coachingScenarioGate.visible=true;box.classList.remove('hidden');preReleaseDebugEvent(decision.role==='observer'?'SCENARIO_OBSERVER_GATE_OPEN':'SCENARIO_GATE_OPEN');const pending=!decision.ready;box.innerHTML=`<div class="scenario-gate-card"><small class="section-kicker">SCÉNARIO</small><h2>Scénario</h2>${decision.scenario.scenario_text?`<p>${esc(decision.scenario.scenario_text)}</p>`:'<p class="muted">Aucun texte de scénario.</p>'}${photos.length?`<div class="scenario-photos">${photos.map((url,index)=>`<img data-scenario-photo-index="${index}" src="${esc(url)}" alt="Photo du scénario ${index+1}">`).join('')}</div>`:''}<p class="small muted">${pending?'Scénario en préparation. La validation sera disponible dès que les photos seront prêtes.':'Prenez connaissance du scénario avant de commencer.'}</p>${decision.role==='observer'?'<button id="coachingScenarioObserverClose" class="primary" type="button">Fermer le scénario</button>':`<button id="coachingScenarioGateRead" class="primary" type="button" ${pending||coachingScenarioGate.busy?'disabled':''}>${coachingScenarioGate.busy?'Validation…':pending?'En attente du scénario':'J’ai lu le scénario'}</button>`}<p id="coachingScenarioGateError" class="msg" aria-live="polite">${coachingScenarioState.error?esc(coachingScenarioState.error):''}</p></div>`;bindScenarioPhotoViewer(box,photos);bindClick('coachingScenarioObserverClose',()=>{coachingScenarioGate.visible=false;coachingScenarioGate.required=false;coachingScenarioActiveHiddenAfterRead=true;box.classList.add('hidden');renderCoachingScenario();preReleaseDebugEvent('SCENARIO_OBSERVER_GATE_CLOSE')});bindClick('coachingScenarioGateRead',markCoachingScenarioRead)}
async function loadCoachingScenario(sessionId){if(!sessionId)return null;coachingScenarioState.sync='pending';coachingScenarioRecord('LOAD_START');const {data,error}=await supabase.rpc('get_coaching_scenario_v10492',{p_session_id:sessionId});if(error){coachingScenarioState.sync='KO';coachingScenarioState.error=error.message;coachingScenarioRecord('LOAD_ERROR',error.message);await renderCoachingScenarioGate();return null}const scenario=data?.scenario||null;coachingScenarioState={...coachingScenarioState,sessionId,scenario,reads:Array.isArray(data?.reads)?data.reads:[],status:scenario?.upload_status||'absent',sync:'OK',error:null};coachingScenarioGate.readServer=coachingScenarioReadByCurrentUser()||!!scenario?.locked_at;if(['driver','solo'].includes(scenarioGateRole())&&coachingScenarioGate.readServer)coachingScenarioActiveHiddenAfterRead=true;coachingScenarioRecord('LOAD_OK');renderCoachingScenarioDebrief();renderCoachingScenario();await renderCoachingScenarioGate();applyV1040RoleSurface();return coachingScenarioState}
...
async function openCoachingSession(id,{resume=false}={}){
 ...
 if(typeof loadCoachingScenario==='function')loadCoachingScenario(s.id).catch(error=>{preReleaseDebugRecordError(error,'supabase');preReleaseDebugEvent('SCENARIO_GATE_READ_ERROR',error?.message||String(error))});await renderCoachingMap();await resumeSoloCoachingPhaseGps(s);refreshCoachingMapLayout();await loadCoachingMessages();if(globalPhase!=='active'){await openCoachingDebriefOnce(s.id,'Piste terminée • Débrief disponible');return true}
...
function updateDriverMarkerOrientations(){for(const marker of coachingParticipantMarkers.values()){if(!['driver','solo'].includes(marker._pisteRole))continue;const relative=markerRelativeHeading(marker._pisteHeading),element=marker.getElement()?.querySelector('.driver-heading');if(element)element.style.setProperty('--heading',`${relative}deg`)}}
	function participantMarker(point,role,label,key=label){const colors={driver:TRACE_PALETTE.conducteur,solo:TRACE_PALETTE.conducteur,coach:'#a986ff',traceur:TRACE_PALETTE.traceur},color=colors[role]||'#91a09d',isDriver=['driver','solo'].includes(role),heading=normalizeHeading(point.heading_deg)??0,debugRole=isDriver?'driver':role==='traceur'?'traceur':null;if(debugRole)coachingMapDebug.participantMarkerCalled[debugRole]=true;ensureCoachingParticipantPane();let marker=coachingParticipantMarkers.get(key);if(!marker){marker=L.marker([point.lat,point.lon],{pane:'coachingParticipantPane',zIndexOffset:900,icon:participantIcon(role,color,heading)}).addTo(coachingMap);coachingParticipantMarkers.set(key,marker);coachingMapDebugEvent(debugRole==='traceur'?'TRACEUR_MARKER_CREATE':debugRole==='driver'?'DRIVER_MARKER_CREATE':'MARKER_CREATE',key)}else{marker.setLatLng([point.lat,point.lon]);if(marker._pisteRole!==role)marker.setIcon(participantIcon(role,color,heading));coachingMapDebugEvent(debugRole==='traceur'?'TRACEUR_MARKER_UPDATE':debugRole==='driver'?'DRIVER_MARKER_UPDATE':'MARKER_UPDATE',key)}const element=marker.getElement?.(),style=element?getComputedStyle(element):null,attached=!!coachingMap?.hasLayer?.(marker),mapMatches=marker._map===coachingMap;Object.assign(coachingMapDebug,{markerAddedToMap:attached,markerElementPresent:!!element,markerHasLayer:attached,markerMapMatches:mapMatches,markerPaneClass:marker.getPane?.()?.className||null,markerDisplay:style?.display||null,markerVisibility:style?.visibility||null,markerOpacity:style?.opacity||null,markerZIndex:style?.zIndex||null});coachingMapDebugEvent(attached?'MARKER_LAYER_ATTACHED':'MARKER_LAYER_ATTACH_FAILED',`${key} pane=${coachingMapDebug.markerPaneClass||'—'} element=${!!element}`);marker._pisteRole=role;marker._pisteHeading=heading;marker.bindTooltip(`${label} • ${new Date(point.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`);if(isDriver&&Number(point.accuracy_m)>0)coachingLayers.push(L.circle([point.lat,point.lon],{radius:Number(point.accuracy_m),color,weight:1,fillColor:color,fillOpacity:.08,interactive:false}).addTo(coachingMap));coachingMapDebugRefreshMarkers();updateDriverMarkerOrientations();return marker}
...
async function renderCoachingMap(options){
 ...
 coachingMapDebugReset();coachingMapDebugEvent('MAP_RENDER_START',activeCoachingSession.id);
 const id=activeCoachingSession.id;
 if(coachingGlobalPhase(activeCoachingSession)!=='active'){
  ...
  coachingDebriefData=data;coachingLayers.forEach(layer=>{try{layer.remove()}catch{}});coachingLayers=[];for(const [key,marker] of coachingParticipantMarkers){coachingMapDebug.markerRemovedDuringCleanup=true;coachingMapDebugEvent('MARKER_REMOVE',key);marker.remove()}coachingParticipantMarkers.clear();coachingPreviewMarker?.remove();coachingPreviewMarker=null;coachingPreviewAccuracyCircle?.remove();coachingPreviewAccuracyCircle=null;
  const newMap=!coachingMap;if(newMap){coachingMap=createPisteMap('coachingMap').setView([48.3,7.45],9);addCleanBaseLayers(coachingMap);coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true;cancelCoachingLongPress()})}
  renderDebriefOverlay(data,{fitViewport:newMap&&!options.preserveViewport});coachingMapDebug.renderInFlight=false;coachingMapDebugEvent('MAP_RENDER_DONE','debrief');return;
 }
 ...
 const mapErrors=[liveRes,traceRes,markerRes,memberRes,currentRes].map(result=>result?.error).filter(Boolean);if(mapErrors.length){coachingMapDebug.lastRenderError=preReleaseDebugSanitize(mapErrors[0]);preReleaseDebugRecordError(mapErrors[0],'supabase')}
 const allLive=visibility.live?(liveRes.data||[]).filter(p=>coachingCanSeeLiveOwner(activeCoachingSession,p.owner_id)):[],trace=visibility.trace?(traceRes.data||[]):[],annotations=visibility.markers?(markerRes.data||[]):[];coachingMapDebug.tracePointsCount=trace.length;coachingMapDebug.livePointsCount=allLive.length;coachingMapDebug.currentPositionsCount=(currentRes.data||[]).length;const traceurMember=activeCoachingSession.coaching_members?.find(m=>m.role==='traceur'),driverMember=activeCoachingSession.coaching_members?.find(m=>['driver','solo'].includes(m.role));coachingMapDebug.traceurCanSeeLiveOwner=traceurMember?coachingCanSeeLiveOwner(activeCoachingSession,traceurMember.user_id):null;coachingMapDebug.driverCanSeeLiveOwner=driverMember?coachingCanSeeLiveOwner(activeCoachingSession,driverMember.user_id):null;coachingMapDebug.traceurLastPointAvailable=!!trace.at(-1);coachingMapDebug.driverLastPointAvailable=allLive.some(p=>['driver','solo'].includes(coachingMemberRole(p.owner_id)));coachingMapDebugEvent('MAP_DATA_LOADED',`trace=${trace.length},live=${allLive.length},current=${coachingMapDebug.currentPositionsCount}`);if(memberRes.data)activeCoachingSession.coaching_members=memberRes.data.map(m=>({...activeCoachingSession.coaching_members?.find(old=>old.user_id===m.user_id),...m}));
 ...
 coachingLayers.forEach(layer=>{try{layer.remove()}catch{}});coachingLayers=[];const newMap=!coachingMap;if(newMap){coachingMap=createPisteMap('coachingMap').setView([48.3,7.45],9);addCleanBaseLayers(coachingMap);ensureCoachingParticipantPane();coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true;cancelCoachingLongPress()});coachingMap.on('rotate rotateend zoomend',updateDriverMarkerOrientations);installCoachingLongPress()}
 ...
 for(const [key,marker] of coachingParticipantMarkers)if(!visibleMarkers.has(key)){coachingMapDebug.markerRemovedDuringCleanup=true;coachingMapDebugEvent('MARKER_REMOVE',key);marker.remove();coachingParticipantMarkers.delete(key)}
 ...
 updateDriverMarkerOrientations();updateCoachingDeparture();coachingMapDebugRefreshMarkers();coachingMapDebug.renderInFlight=false;coachingMapDebugEvent('MAP_RENDER_DONE','active');
}
...
function missionDebriefHtml(source){const d=source.debrief||{},section=(title,text)=>text?`<article class="mission-contribution"><h3>${esc(title)}</h3><p>${esc(text)}</p></article>`:'';if(source.type==='coaching')return section('Retour du Conducteur',d.driver_notes)+section('Analyse du Coach',[d.coach_notes,d.strengths,d.improvement_area,d.statistics_notes].filter(Boolean).join('\n\n'))||'<p>Aucune contribution accessible.</p>';return section('Notes de terrain',[source.row.observation,source.row.comportement,source.row.points_positifs,source.row.difficultes,source.row.axes_travail].filter(Boolean).join('\n\n'))||'<p>Aucune note enregistrée.</p>'}
...
function renderMissionDossier(source){
 ...
 $('missionSummary').innerHTML=`<h3>INFORMATIONS GÉNÉRALES</h3>${missionInfo(general)}${missionInfo(missionWeather(source))}<button class="primary" data-mission-go-map>Voir la carte</button>${source.type==='coaching'&&!['ended','cancelled'].includes(r.status)?'<button class="secondary" data-mission-resume>Reprendre la piste</button>':''}`;
...
function renderMissionMap(source){
 ...
 const layers={},all=[];missionMap=createPisteMap('missionMap').setView([48.3,7.45],8);addCleanBaseLayers(missionMap);
...
function missionReportModel(source){const model=buildProfessionalReportModel(source);model.map.gpx=source.gpx||[];if(source.operationalCall)model.sections.push(reportSection('operational_context','Contexte opérationnel','Fiche d’appel autorisée',[source.operationalCall.circumstances,source.operationalCall.terrain_notes,source.operationalCall.last_known_label].filter(Boolean).join('\n')));model.sections=model.sections.map(s=>s.key==='timeline'?{...s,text:missionTimeline(source).map(e=>`${source.type==='coaching'?coachingTimestampV1045(e.at):dateTimeFr(e.at)} — ${e.title}${e.detail?' : '+e.detail:''}`).join('\n')||'Aucun événement horodaté.'}:s);model.sections.push(reportSection('driver_contribution','Retour du Conducteur','Contribution enregistrée',source.debrief?.driver_notes||'Aucun retour accessible.'));model.photos=model.photos.filter(p=>missionPhotoUrl(p.data));return model}
function renderMissionReport(source){
 const model=missionReportModel(source),root=$('missionReport').querySelector('.mission-report-preview');root.innerHTML=`<article class="mission-paper"><h2>${esc(model.title)}</h2>${model.sections.map(s=>`<section><h3>${esc(s.title)}</h3><p>${esc(s.text)}</p></section>`).join('')}<img src="${reportMapCanvas(model.map).toDataURL('image/jpeg',.9)}" alt="Carte des traces autorisées">${model.photos.map(p=>`<figure><img src="${p.data}" alt="${esc(p.caption)}"><figcaption>${esc(p.caption)}</figcaption></figure>`).join('')}</article><button class="primary" data-mission-pdf>Exporter en PDF</button><p class="mission-export-state" role="status"></p>`;
...
async function reportActivitySource(type,id){
 ...
 let actual=[],trace=[],planned=[],markers=[],debrief=null,members=[],operationalCall=null,gpx=[];
 if(type==='coaching'){
 ...
 return{row,type,actual,trace,planned,reference,markers,debrief,members,metrics,operationalCall,gpx};
}
...
async function openLibraryItem(type,id){if(type==='coaching'){showPage('coachingPage');return openCoachingSession(id)}return openMissionDossier(type,id)}
...
function renderActivityLibrary(){
 ...
 setTimeout(()=>{if(globalMap){globalMap.remove();globalMap=null}globalMap=createPisteMap('activityLibraryMap').setView([48.3,7.45],8);addCleanBaseLayers(globalMap);const layers=[];rows.forEach(x=>{const track=libraryTrack(x);if(!Array.isArray(track)||track.length<2)return;const line=L.polyline(track.map(p=>[p.lat,p.lon]),{weight:4,color:activityColor(x._type),opacity:1}).addTo(globalMap).bindPopup(`<b>${esc(libraryName(x))}</b><br>${esc(libraryTypeMeta(x._type).label)}`);layers.push(line)});if(layers.length)globalMap.fitBounds(L.featureGroup(layers).getBounds(),{padding:[24,24]})},60);
}
...
async function loadPublicShareFromUrl(){
 ...
 if(track.length>1){$('publicShareMap').classList.remove('hidden');setTimeout(()=>{const map=createPisteMap('publicShareMap'),line=L.polyline(track.map(p=>[p.lat,p.lon]),{weight:5,color:activityColor(type),opacity:1}).addTo(map);addCleanBaseLayers(map);map.fitBounds(line.getBounds(),{padding:[28,28]})},80)}
 return true;
}
...
function initLiveMap(force=false){
 ...
 liveLine=L.polyline([],{weight:5,color:TRACE_PALETTE.conducteur,opacity:1}).addTo(liveMap);
...
function redrawLiveRecordingMap(){
 if(!liveMap)return;
 if(!liveLine)liveLine=L.polyline([],{weight:5,color:TRACE_PALETTE.conducteur,opacity:1}).addTo(liveMap);
 ...
 if(gps.startPoint)liveMarker=L.marker([gps.startPoint.lat,gps.startPoint.lon],{icon:traceMarkerIcon('D')}).addTo(liveMap).bindPopup('Départ');
...
$('plannerAdvancedToggle').onclick=()=>{const area=$('plannerAdvancedArea'),open=area.classList.toggle('hidden')===false;$('plannerAdvancedToggle').setAttribute('aria-expanded',String(open));$('plannerAdvancedToggle').textContent=open?'Masquer les outils avancés':'Outils avancés';setTimeout(()=>plannerMap?.invalidateSize(),80)};
...
async function markCoachingScenarioRead(){const id=activeCoachingSession?.id;if(!id||coachingScenarioState.status!=='ready')return false;coachingScenarioGate.busy=true;preReleaseDebugEvent('SCENARIO_GATE_READ_START');await renderCoachingScenarioGate();const {data,error}=await supabase.rpc('mark_coaching_scenario_read_v10492',{p_session_id:id});if(error){coachingScenarioGate.busy=false;coachingScenarioRecord('READ_ERROR',error.message);preReleaseDebugEvent('SCENARIO_GATE_READ_ERROR',error.message);coachingToast(`Lecture du scénario impossible : ${error.message}`);await renderCoachingScenarioGate();return false}await loadCoachingScenario(id);const confirmed=coachingScenarioReadByCurrentUser()||!!coachingScenarioState.scenario?.locked_at;if(!confirmed){coachingScenarioGate.busy=false;preReleaseDebugEvent('SCENARIO_GATE_READ_ERROR','confirmation serveur absente');await renderCoachingScenarioGate();return false}coachingScenarioGate.busy=false;coachingScenarioGate.readServer=true;coachingScenarioGate.required=false;coachingScenarioGate.visible=false;coachingScenarioActiveHiddenAfterRead=true;renderCoachingScenario();preReleaseDebugEvent('SCENARIO_GATE_READ_OK');preReleaseDebugEvent('SCENARIO_GATE_CLOSE');await renderCoachingScenarioGate();return !!data}
...
window.coachingPhase=coachingPhase;window.coachingDbVisibility=coachingDbVisibility;window.refreshActiveCoachingSessionData=refreshActiveCoachingSessionData;
setupV1040Coaching();
boot();
