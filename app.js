...
async function beginCoachingLaying(){const s=activeCoachingSession;if(coachingV1040ActionBusy)return false;if(!s){coachingToast('Session Coaching non chargée. Actualisez la session avant de commencer la pose.');return false}if(Number(s.workflow_version)<2||coachingPhase(s)!=='preparation'){coachingToast('La pose ne peut commencer que depuis une session V10.40 en préparation.');applyV1040RoleSurface();return false}coachingV1040ActionBusy=true;try{const ok=await coachingTransitionV1040('start_coaching_laying');if(ok&&activeCoachingSession)await startTraceurTracking();return ok}finally{coachingV1040ActionBusy=false}}
...
function layingHoldCancel(){if(coachingV1040HoldTimer){clearTimeout(coachingV1040HoldTimer);coachingV1040HoldTimer=null}coachingV1040HoldState=null;if(coachingV1040HoldButton){coachingV1040HoldButton.classList.remove('holding');coachingV1040HoldButton=null}}
function layingHoldStart(button,action,event=null){layingHoldCancel();coachingV1040HoldButton=button;coachingV1040HoldState={startedAt:performance.now(),pointerId:event?.pointerId??null,startX:event?.clientX??event?.touches?.[0]?.clientX??null,startY:event?.clientY??event?.touches?.[0]?.clientY??null};button.classList.add('holding');coachingV1040HoldTimer=setTimeout(()=>{coachingV1040HoldTimer=null;button.classList.remove('holding');coachingV1040HoldState=null;coachingV1040HoldButton=null;Promise.resolve(action()).catch(error=>coachingToast(error.message||'Action impossible'))},2000)}
