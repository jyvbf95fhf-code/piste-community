...
let coachingSessionEndHandledId=null,coachingV1040FinishInFlight=false,coachingV10423TransitionInFlight=false,coachingDebriefOpenPromises=new Map(),coachingDebriefClosePromises=new Map(),coachingDebriefStep='summary';
...
function liveOdorModel(trace,points,s=activeCoachingSession,role=myCoachingRole(s)){if(!coachingCanSeeOdor(s,role))return null;const base=s?.odor_model||{},w=coachingLiveWeather?.status==='ready'?coachingLiveWeather:null,ref=s?.track_started_at||s?.track_finished_at||null,result=sharedOlfactionEngine({referenceTrack:trace.length>1?trace:s?.planned_route||[],referenceTime:ref,currentTime:new Date(),liveWeather:w||base,module:'coaching'});return {...base,enabled:true,wind_direction_deg:w?.wind_direction_deg??base.wind_direction_deg??0,wind_speed_kmh:w?.wind_speed_kmh??base.wind_speed_kmh??5,gust_kmh:w?.wind_gusts_kmh??base.gust_kmh,age_hours:result.trackAgeSeconds===null?Number(base.age_hours)||0:result.trackAgeSeconds/3600,source:w?.source||base.source||'historique',fetched_at:w?.fetched_at||base.fetched_at}}
function addLiveOdorCorridor(trace,points){const model=liveOdorModel(trace,points);if(!model)return null;const route=trace.length>1?trace:(activeCoachingSession?.planned_route||[]);if(route.length<2)return null;return addOdorLayers(coachingMap,route,model,coachingLayers)}
...
function setCoachingDebriefMode(active){
 ...
 if(active&&canvas){
  if(mapShell&&mapShell.parentElement!==canvas)canvas.appendChild(mapShell);
  if(layers&&layers.parentElement!==canvas)canvas.appendChild(layers);
 }else{
  if(mapShell&&mapSlot)mapSlot.after(mapShell);
  if(layers&&layerSlot)layerSlot.after(layers);
 }
 return !!active;
}
...
function coachingDebriefWeatherSummary(){const w=coachingLiveWeather;if(!w||w.status==='unavailable')return '';const when=w.fetched_at?` · actualisée à ${new Date(w.fetched_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}`:'';const values=[w.temperature_c!=null?`Température ${fmt(w.temperature_c,1)} °C`:null,w.humidity_pct!=null?`Humidité ${fmt(w.humidity_pct,0)} %`:null,w.wind_speed_kmh!=null?`Vent ${fmt(w.wind_speed_kmh,1)} km/h`:null,w.wind_gusts_kmh!=null?`Rafales ${fmt(w.wind_gusts_kmh,1)} km/h`:null,w.precipitation_mm!=null?`Pluie ${fmt(w.precipitation_mm,1)} mm`:null].filter(Boolean);return values.length?`<div class="debrief-weather-summary"><small class="section-kicker">MÉTÉO DE SESSION</small><span>${esc(values.join(' · '))}${esc(when)}</span></div>`:''}
