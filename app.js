...
async function markCoachingTrackReady(){return coachingTransitionV1040('mark_coaching_track_ready')}
...
function setupV1040Coaching(){bindClick('trackReadyBtn',markCoachingTrackReady);bindClick('coachReadyBtn',markCoachReady);bindClick('driverStartBtn',startDriverRun);bindClick('driverFinishBtn',finishDriverRun);bindClick('refreshCoachingSessionBtn',refreshActiveCoachingSessionData);bindClick('generateCoachingAi',generateCoachingAiDebrief);bindClick('coachingHelpBtn',()=>{const box=$('coachingHelpPanel')||$('coachingHelp');if(box)box.classList.toggle('hidden');else coachingToast('Aide Coaching : rôles, modes, phases et limites du couloir olfactif estimé.')});const toggle=$('observerTrackToggle');if(toggle)toggle.addEventListener('change',()=>{if(activeCoachingSession)localStorage.setItem(`piste-observer-track-${activeCoachingSession.id}`,String(toggle.checked));if(coachingMap)renderCoachingMap()});if(activeCoachingSession)applyV1040RoleSurface()}
