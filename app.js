...
async function startSoloRun(){const s=activeCoachingSession;if(!s||!isSoloCoaching(s)||coachingPhase(s)!=='preparation')return false;const {data,error}=await supabase.rpc('start_solo_run',{p_session_id:s.id});if(error){coachingToast(`Départ impossible : ${error.message}`);return false}await refreshActiveCoachingSession(s.id);if(!activeCoachingSession||coachingPhase(activeCoachingSession)!=='driver_running'){coachingToast('Le départ Solo n’a pas été confirmé par le serveur.');return false}updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await startCoachingGpsTracking();await renderCoachingMap();refreshCoachingMapLayout();return !!data}
