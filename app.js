...
function finishHoldStart(e){if(e){e.preventDefault();e.stopPropagation()}const b=$('terrainFinishBtn'),s=activeCoachingSession,v2=Number(s?.workflow_version)>=2,canFinish=!!s&&s.status==='live'&&isCoachingOwner(s)&&(!v2||coachingPhase(s)==='completed'||!!s.driver_finished_at);if(!b||!canFinish||coachingFinishArmed)return;coachingFinishArmed=true;b.classList.add('holding');b.style.setProperty('--finish-progress','0%');const started=Date.now();coachingFinishTimer=setInterval(()=>{const pct=Math.min(100,((Date.now()-started)/2000)*100);b.style.setProperty('--finish-progress',`${pct}%`);if(pct>=100){finishHoldCancel();if(v2)finishCoachingSessionV1040();else finishActiveCoaching()}},50)}
...
async function handleCoachingSessionChange(payload){if(!activeCoachingSession||payload.new?.id!==activeCoachingSession.id)return;const id=payload.new.id,status=payload.new.status,previousPhase=coachingPhase(activeCoachingSession);applySafeCoachingRealtimeStatus(payload.new);await refreshActiveCoachingSession(id);const nextPhase=coachingPhase(activeCoachingSession),role=myCoachingRole(activeCoachingSession),owner=isCoachingOwner(activeCoachingSession),justCompleted=nextPhase!==previousPhase&&nextPhase==='completed';if(nextPhase!==previousPhase&&nextPhase==='coach_ready'&&role==='driver')coachingToast('La piste est prête — vous pouvez préparer votre départ');if(justCompleted&&role==='driver')coachingToast('Parcours terminé');else if(justCompleted&&(role==='coach'||owner))coachingToast('Parcours du Conducteur terminé');syncCoachingPanelStatus();if(['ended','cancelled'].includes(status)){stopCoachingPresence();stopTraceurTracking();await closeFakeLock({returnToMap:false,reacquireWake:false});clearVerifiedActiveCoaching(id);updateCoachingPhase();updateCoachingPrimaryActions();setCoachingStage('debrief');refreshCoachingMapLayout();await calculateCoachingDebrief();await loadSavedCoachingDebrief();setUiText('coachingLiveStatus',status==='ended'?'Session terminée par l’organisateur • débrief disponible':'Session annulée par l’organisateur')}else{if(status==='live'){verifiedActiveCoachingSession=activeCoachingSession;saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();setCoachingStage('live')}updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();refreshCoachingMapLayout();if(justCompleted&&(role==='coach'||owner))setUiText('coachingLiveStatus','Parcours du Conducteur terminé • vous pouvez terminer la session')}}
...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingAutoMetrics=null;coachingGpsError='';coachingGpsReady=false;if(Number(s.workflow_version)>=2){const refreshed=await refreshActiveCoachingSession(s.id);if(refreshed)s=activeCoachingSession}$('coachingDebriefForm')?.reset();setUiText('coachingDebriefState','Aucun brouillon chargé.');const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
...
async function renderCoachingMap(){
 ...
 const fullRoute=activeCoachingSession.planned_route||[],role=myCoachingRole(activeCoachingSession),dbVisibility=coachingDbVisibility(activeCoachingSession,role),route=dbVisibility==='hidden'?[]:fullRoute;
 ...
 if(coachingLayerVisibility.trace&&trace.length>1&&dbVisibility!=='hidden')coachingLayers.push(L.polyline(trace.map(p=>[p.lat,p.lon]),{color:'#58d6a4',weight:4,dashArray:'5 7',opacity:.9,lineCap:'round',className:'map-route-line map-route-trace'}).addTo(coachingMap));
 const blindDriver=dbVisibility==='hidden';
 ...
 if(coachingLayerVisibility.trace&&trace.length&&dbVisibility!=='hidden'){visibleMarkers.add('traceur-track');participantMarker(trace.at(-1),'traceur','Traceur','traceur-track')}
 ...
 if(coachingLayerVisibility.markers&&dbVisibility!=='hidden')(activeCoachingSession.planned_markers||[]).filter(w=>markerVisible(w,activeCoachingSession,points)).forEach(w=>{const d=SCENARIO_MARKERS[w.type]||SCENARIO_MARKERS.note;coachingLayers.push(L.marker([w.lat,w.lon],{icon:L.divIcon({className:'scenario-map-icon',html:`<span>${d.icon}</span>`,iconSize:[34,34],iconAnchor:[17,17]})}).addTo(coachingMap).bindPopup(`<b>${esc(d.label)}</b>${w.note?`<br>${esc(w.note)}`:''}`))});
 if(coachingLayerVisibility.markers&&dbVisibility!=='hidden')annotations.forEach(w=>{const d=LIVE_MARKERS[w.marker_type]||LIVE_MARKERS.note;coachingLayers.push(L.marker([w.lat,w.lon],{icon:L.divIcon({className:'live-map-icon',html:`<span>${d.icon}</span>`,iconSize:[32,32],iconAnchor:[16,16]})}).addTo(coachingMap).bindPopup(`<b>${esc(d.label)}</b>${w.note?`<br>${esc(w.note)}`:''}`))});
...
function drawCoachingReplay(){
 ...
 const before=(rows,timeKey='recorded_at')=>rows.filter(p=>new Date(p[timeKey]).getTime()<=coachingReplay.currentAt),trace=before(coachingReplay.trace),driver=before(coachingReplay.driver),route=coachingDbVisibility(activeCoachingSession,myCoachingRole(activeCoachingSession))==='hidden'?[]:(activeCoachingSession?.planned_route||[]);
 ...
 if(coachingLayerVisibility.trace&&trace.length>1&&coachingDbVisibility(activeCoachingSession,myCoachingRole(activeCoachingSession))!=='hidden')coachingLayers.push(L.polyline(trace.map(p=>[p.lat,p.lon]),{color:'#58d6a4',weight:5,dashArray:'5 7',opacity:.95}).addTo(coachingMap));
...
async function finishCoachingSessionV1040(){const s=activeCoachingSession;if(!s||!isCoachingOwner(s)){alert('Seul l’organisateur peut terminer la session.');return false}if(Number(s.workflow_version)<2)return finishActiveCoaching();if(s.status!=='live'||(coachingPhase(s)!=='completed'&&!s.driver_finished_at)){coachingToast('Le parcours du Conducteur doit être terminé avant de clôturer la session.');return false}if(!confirm('Terminer la session et ouvrir le débrief ?'))return false;const id=s.id,{error}=await supabase.rpc('finish_coaching_session',{p_session_id:id});if(error){coachingToast(`Fin impossible : ${error.message}`);return false}await refreshActiveCoachingSession(id);stopCoachingPresence();stopTraceurTracking();await closeFakeLock({returnToMap:false,reacquireWake:false});clearVerifiedActiveCoaching(id);$('coachingLivePanel')?.classList.remove('waiting-room');$('coachingDebriefForm')?.classList.remove('hidden');updateCoachingPhase();updateCoachingPrimaryActions();setCoachingStage('debrief');refreshCoachingMapLayout();setUiText('coachingLiveStatus','Session terminée • débrief prêt à être complété');await calculateCoachingDebrief();await loadSavedCoachingDebrief();return true}
...
function coachingDbVisibility(s,role){
  const mode=Number(s?.workflow_version)>=2?(s?.blind_mode||'normal'):(s?.visibility_mode||'all'),r=role||myCoachingRole(s);
  if(mode==='full_blind'||mode==='coach')return r==='traceur'||(r==='coach'&&s?.laying_mode==='coach')?'visible':'hidden';
  if(mode==='simple_blind'||mode==='progressive'){if(r==='driver')return 'hidden';if(r==='observer'){const key=`piste-observer-track-${s?.id||'session'}`;return localStorage.getItem(key)==='true'?'visible':'hidden'}return 'visible'}
  return 'visible';
}
...
function applyV1040RoleSurface(){const s=activeCoachingSession;if(!s)return;const role=coachingGpsRole(s),phase=coachingPhase(s),owner=isCoachingOwner(s),v1040=Number(s.workflow_version)>=2,completed=phase==='completed'||!!s.driver_finished_at,layingActor=isCurrentUserLayingActor(s),hide=id=>$(id)?.classList.add('hidden'),show=id=>$(id)?.classList.remove('hidden');['startCoachingLive','toggleCoachingGps','coachingFakeLockBtn','endCoachingLive','terrainFinishBtn','cancelCoachingWaiting','deleteCoachingWaiting','cancelCoachingSession','deleteCoachingSession','startLayingBtn','finishLayingBtn','coachReadyBtn','trackReadyBtn','driverStartBtn','driverFinishBtn'].forEach(hide);show('terrainBlackScreenBtn');if(v1040&&completed){if(owner){setUiText('terrainFinishBtn','Terminer la session');show('terrainFinishBtn')}}else if(role==='observer'){hide('coachingLayerControls');show('observerTrackChoice');hide('leaveCoachingSession')}else if(v1040&&layingActor&&phase==='preparation'){setUiText('startLayingBtn','Je pars tracer');show('startLayingBtn')}else if(v1040&&layingActor&&phase==='laying'){setUiText('trackReadyBtn','Piste prête');show('trackReadyBtn')}else if(v1040&&role==='driver'&&['waiting_ready','coach_ready'].includes(phase))show('driverStartBtn');else if(v1040&&role==='driver'&&phase==='driver_running')show('driverFinishBtn');else if(!v1040&&role==='traceur'){if(phase==='preparation')show('startLayingBtn');if(phase==='laying')show('finishLayingBtn')}else if(!v1040&&role==='coach'&&s.laying_mode==='coach'){if(phase==='preparation')show('startLayingBtn');if(phase==='laying')show('finishLayingBtn');if(['laid','waiting_ready'].includes(phase))show('coachReadyBtn')}else if(!v1040&&role==='coach'&&['laid','waiting_ready'].includes(phase))show('coachReadyBtn');if(owner&&!v1040&&phase==='driver_running')show('terrainFinishBtn');if(owner&&s.status==='waiting'){show('cancelCoachingWaiting');show('deleteCoachingWaiting')}setUiText('coachingRoleInstruction',completed?(role==='driver'?'Parcours terminé.':owner||role==='coach'?'Parcours du Conducteur terminé. Vous pouvez terminer la session et ouvrir le débrief.':'Parcours du Conducteur terminé.'):role==='observer'?'Vous observez la session sans commande de pilotage.':role==='driver'&&phase==='preparation'?'En attente : le Coach prépare la piste.':role==='driver'&&phase==='laying'?'Pose de la piste en cours.':role==='driver'&&['waiting_ready','coach_ready'].includes(phase)?'La piste est prête — vous pouvez démarrer.':role==='driver'&&phase==='driver_running'?'Parcours en cours.':layingActor&&phase==='preparation'?'Appuyez sur « Je pars tracer » pour démarrer la pose et le GPS.':layingActor&&phase==='laying'?'Pose en cours — terminez puis validez « Piste prête ».':`Rôle : ${coachingRoleLabel(role)} · ${coachingPhaseLabelV1040(s)}`);renderCoachingScenario()}
