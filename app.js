...
async function createCoaching(){const route=trainingRoutes.find(r=>r.id===$('coachingRouteSelect').value),creatorRole=$('coachingCreatorRole')?.value||'coach',button=$('createCoachingSession');if(!route){$('coachingCreateMsg').textContent='Choisis d’abord un tracé préparé.';return}const payload={owner_id:session.user.id,dog_id:$('coachingDogSelect')?.value||null,route_id:route.id,name:$('coachingSessionName').value.trim()||route.name,status:'waiting',planned_route:route.route||[],planned_markers:route.waypoints||[],odor_model:route.odor_model||{},visibility_mode:$('coachingVisibility').value,invite_code:coachingCode(),expires_at:new Date(Date.now()+7*864e5).toISOString()};button.disabled=true;$('coachingCreateMsg').textContent='Création de la salle de préparation…';try{const {data,error}=await supabase.from('coaching_sessions').insert(payload).select().single();if(error||!data?.id){$('coachingCreateMsg').textContent='Création impossible : '+(error?.message||'identifiant de session absent.');return}const member=await supabase.from('coaching_members').insert({session_id:data.id,user_id:session.user.id,role:creatorRole});if(member.error){await supabase.from('coaching_sessions').delete().eq('id',data.id).eq('owner_id',session.user.id);$('coachingCreateMsg').textContent='Création impossible : '+member.error.message;return}clearActiveCoachingRef(data.id);await loadCoachingHub();const opened=await openCoachingSession(data.id);$('coachingCreateMsg').textContent=opened?`Session prête — partagez le code ${data.invite_code}.`:'Session créée, mais son ouverture a échoué. Utilisez « Préparer » dans Mes sessions.'}catch(error){$('coachingCreateMsg').textContent='Création impossible : '+(error?.message||'erreur inattendue.')}finally{button.disabled=false}}
...
function coachingPrimaryLabel(role,status,owner=false){if(status==='ended')return 'Session terminée';if(status==='waiting')return owner?'Démarrer la session':'En attente de l’organisateur';if(role==='driver'||role==='solo')return 'Ouvrir mon suivi GPS';if(role==='traceur')return 'Partager ma position';if(role==='coach')return 'Partager ma position';return 'Suivre la session'}
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingAutoMetrics=null;const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=s.owner_id===session.user.id;if(s.status==='live'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
 ...
 $('coachingLivePanel').classList.toggle('waiting-room',s.status==='waiting');$('startCoachingLive').textContent=coachingPrimaryLabel(role,s.status,owner);$('startCoachingLive').classList.toggle('hidden',s.status==='waiting'?!owner:role==='observer'||s.status==='ended');$('startCoachingLive').disabled=s.status==='waiting'&&!owner;$('endCoachingLive').classList.toggle('hidden',!['coach','driver','solo'].includes(role)||s.status!=='live');$('coachAnnotationTools').classList.toggle('hidden',!['coach','driver','solo'].includes(role)||s.status==='ended');$('traceurTools').classList.toggle('hidden',role!=='traceur'||s.status==='ended');$('calculateCoachingDebrief').classList.toggle('hidden',s.status==='waiting');$('coachingDebriefForm').classList.toggle('hidden',s.status==='waiting'||!['coach','driver','solo'].includes(role));$('coachingAutoDebrief').classList.add('hidden');
 ...
 updateCoachingPreflight();updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();setCoachingPanel('team');setCoachingStage(s.status==='ended'?'debrief':s.status==='waiting'?'room':'live');if(s.status==='live')startCoachingPresence();await renderCoachingMap();await loadCoachingMessages();
...
async function startActiveCoaching(){
 ...
 $('coachingLivePanel').classList.remove('waiting-room');$('coachingWaitingActions').classList.add('hidden');$('startCoachingLive').disabled=false;setCoachingStage('live');updateCoachingPhase();updateCoachingPreparationDetails();startCoachingPresence();await renderCoachingMap();
