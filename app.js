...
function updateDriverMarkerOrientations(){for(const marker of coachingParticipantMarkers.values()){if(!['driver','solo'].includes(marker._pisteRole))continue;const relative=markerRelativeHeading(marker._pisteHeading),element=marker.getElement()?.querySelector('.driver-heading');if(element)element.style.setProperty('--heading',`${relative}deg`)}}
	function participantMarker(point,role,label,key=label){const colors={driver:TRACE_PALETTE.conducteur,solo:TRACE_PALETTE.conducteur,coach:'#a986ff',traceur:TRACE_PALETTE.traceur},color=colors[role]||'#91a09d',isDriver=['driver','solo'].includes(role),heading=normalizeHeading(point.heading_deg)??0,debugRole=isDriver?'driver':role==='traceur'?'traceur':null;if(debugRole)coachingMapDebug.participantMarkerCalled[debugRole]=true;let marker=coachingParticipantMarkers.get(key);if(!marker){marker=L.marker([point.lat,point.lon],{zIndexOffset:900,icon:participantIcon(role,color,heading)}).addTo(coachingMap);coachingParticipantMarkers.set(key,marker);coachingMapDebug.markerAddedToMap=true;coachingMapDebugEvent(debugRole==='traceur'?'TRACEUR_MARKER_CREATE':debugRole==='driver'?'DRIVER_MARKER_CREATE':'MARKER_CREATE',key)}else{marker.setLatLng([point.lat,point.lon]);if(marker._pisteRole!==role)marker.setIcon(participantIcon(role,color,heading));coachingMapDebugEvent(debugRole==='traceur'?'TRACEUR_MARKER_UPDATE':debugRole==='driver'?'DRIVER_MARKER_UPDATE':'MARKER_UPDATE',key)}marker._pisteRole=role;marker._pisteHeading=heading;marker.bindTooltip(`${label} • ${new Date(point.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`);coachingMapDebug.markerElementPresent=!!marker.getElement?.();if(isDriver&&Number(point.accuracy_m)>0)coachingLayers.push(L.circle([point.lat,point.lon],{radius:Number(point.accuracy_m),color,weight:1,fillColor:color,fillOpacity:.08,interactive:false}).addTo(coachingMap));coachingMapDebugRefreshMarkers();updateDriverMarkerOrientations();return marker}
