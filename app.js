...
function setCoachingDebriefStep(step='summary'){
 const allowed=['summary','map','observations','complete'],next=allowed.includes(step)?step:'summary';coachingDebriefStep=next;
...
function renderAutoDebrief(m,usesTrace){const el=$('coachingAutoDebrief');if(!el)return;el.classList.remove('hidden');const concordance=m.concordance||{},trace=m.traceur||{},driver=m.driver||{},value=(number,unit='')=>number===null||number===undefined?'Non calculable':`${fmt(number,1)}${unit}`,age=m.track_age_ms===null?'Non calculable':formatExactDuration(m.track_age_ms),duration=driver.duration_ms===null||driver.duration_ms===undefined?'Non calculable':formatExactDuration(driver.duration_ms),odorKpis=`<div><strong>Couloir estimé</strong><span>interprétation indicative, sans pourcentage d’odeur</span></div>`;el.innerHTML=`<div class="card-title-row"><div><small class="section-kicker">DÉBRIEF AUTOMATIQUE</small><h3>Traceur / conducteur</h3></div><span>${usesTrace?'Comparaison sur GPS réels':'Indice non calculable · trace réelle manquante'}</span></div><div class="debrief-kpis debrief-primary-kpis"><div><strong>${value(concordance.indexPct,' %')}</strong><span>Indice de concordance</span></div><div><strong>${value(concordance.maxDeviationM,' m')}</strong><span>Écart maximal</span></div><div class="age-kpi"><strong>${age}</strong><span>Âge piste au départ</span></div><div><strong>${duration}</strong><span>Durée du parcours</span></div></div><div class="debrief-role-grid"><article class="traceur"><small>TRACEUR</small><strong>${value(trace.distance_km,' km')}</strong><span>Distance Traceur</span><em>${trace.points||0} points GPS</em></article><article class="driver"><small>CONDUCTEUR</small><strong>${value(driver.distance_km,' km')}</strong><span>Distance Conducteur</span><em>${driver.points||0} points GPS</em></article></div><div class="debrief-kpis debrief-detail-kpis"><div><strong>${value(concordance.meanDeviationM,' m')}</strong><span>Écart moyen</span></div><div><strong>${value(concordance.maxDeviationM,' m')}</strong><span>Écart maximal brut</span></div><div><strong>${esc(concordance.quality?.label||'Qualité non calculable')}</strong><span>qualité des données GPS</span></div><div><strong>${m.stops}</strong><span>arrêts (${fmt(m.stopped_min,1)} min)</span></div><div><strong>${m.objects_visited}/${m.objects_total}</strong><span>objets approchés</span></div><div><strong>${m.losses}</strong><span>pertes annotées</span></div><div><strong>${m.recoveries}</strong><span>reprises annotées</span></div>${odorKpis}</div>${coachingDebriefWeatherSummary()}<p class="small muted">L’Indice de concordance est un indicateur géométrique progressif calculé dans les deux sens. Il contextualise les écarts et ne constitue pas une vérité scientifique. Les résultats sont recalculés depuis les points GPS bruts.</p>`}
