...
function updateCoachingPrimaryActions(){
 ...
 if(Number(s?.workflow_version)>=2){hideLegacyCoachingPrimaryActions();if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();const phase=coachingPhase(s),completed=phase==='completed'||!!s.driver_finished_at;const message=completed?(role==='driver'?'Parcours terminé.':owner||role==='coach'?'Parcours du Conducteur terminé. Vous pouvez terminer la session et ouvrir le débrief.':'Parcours du Conducteur terminé.'):role==='driver'?(phase==='preparation'?'En attente : le Traceur prépare la piste.':phase==='laying'?'Pose en cours : le Traceur trace actuellement la piste.':phase==='waiting_ready'||phase==='coach_ready'?'La piste est prête — vous pouvez démarrer.':phase==='driver_running'?'Parcours en cours.':''):isCurrentUserLayingActor(s)?(phase==='preparation'?'Appuyez sur « Je démarre la piste » pour démarrer la pose et le GPS.':phase==='laying'?'Pose en cours — terminez puis validez « Piste tracée ».':''):'';setUiText('coachingRoleInstruction',message);return}
...
async function finishDriverRun(){const ok=await coachingTransitionV1040('finish_driver_run');if(ok){stopCoachingPresence();setCoachingStage('debrief');await loadSavedCoachingDebrief()}return ok}
...
function applyV1040RoleSurface(){
 ...
 if(v1040&&completed){if(owner){setUiText('terrainFinishBtn','Terminer la session');show('terrainFinishBtn')}}
 ...
 else if(v1040&&layingActor&&phase==='preparation'){setUiText('startLayingBtn','Je démarre la piste');show('startLayingBtn')}
 ...
 setUiText('coachingRoleInstruction',informativeCompleted?(role==='driver'?'Parcours terminé.':owner||role==='coach'?'Parcours du Conducteur terminé. Vous pouvez terminer la session et ouvrir le débrief.':'Parcours du Conducteur terminé.'):role==='observer'?'Vous observez la session sans commande de pilotage.':role==='driver'&&phase==='preparation'?'En attente : le Traceur prépare la piste.':role==='driver'&&phase==='laying'?'Pose de la piste en cours.':role==='driver'&&['waiting_ready','coach_ready'].includes(phase)?'La piste est prête — vous pouvez démarrer.':role==='driver'&&phase==='driver_running'?'Parcours en cours.':layingActor&&phase==='preparation'?'Appuyez sur « Je démarre la piste » pour démarrer la pose et le GPS.':layingActor&&phase==='laying'?'Pose en cours — terminez puis validez « Piste tracée ».':`Rôle : ${coachingRoleLabel(role)} · ${coachingPhaseLabelV1040(s)}`);updateCoachingDebriefAccess();renderCoachingScenario()
}
