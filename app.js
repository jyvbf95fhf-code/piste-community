...
async function calculateCoachingDebrief(){if(!activeCoachingSession)return null;const id=activeCoachingSession.id,data=await loadCoachingDebriefData(id);if(activeCoachingSession?.id!==id)return null;coachingDebriefData=data;await renderCoachingMap({debriefData:data,preserveViewport:true});const points=data.driverPoints,trace=data.traceurPoints,reference=trace,driverMetrics=computeCoachingMetrics(points,reference,[],data.markers,activeCoachingSession.odor_model||{},trace),traceDuration=trace.length>1?Math.max(0,new Date(trace.at(-1).recorded_at)-new Date(trace[0].recorded_at)):0,driverDuration=points.length>1?Math.max(0,new Date(points.at(-1).recorded_at)-new Date(points[0].recorded_at)):0;coachingAutoMetrics={...driverMetrics,driver:{distance_km:Number((routeDistance(points)/1000).toFixed(3)),duration_ms:driverDuration,started_at:points[0]?.recorded_at||null,ended_at:points.at(-1)?.recorded_at||null,points:points.length},traceur:{distance_km:Number((routeDistance(trace)/1000).toFixed(3)),duration_ms:traceDuration,started_at:trace[0]?.recorded_at||null,ended_at:trace.at(-1)?.recorded_at||null,points:trace.length},track_age_ms:coachingTimingV1045(activeCoachingSession).ageAtStartMs};renderAutoDebrief(coachingAutoMetrics,trace.length>1);return coachingAutoMetrics}
...
function renderAutoDebrief(m,usesTrace){const el=$('coachingAutoDebrief');if(!el)return;el.classList.remove('hidden');const odorKpis=`<div><strong>Couloir estimé</strong><span>interprétation indicative, sans pourcentage d’odeur</span></div>`,ageKpis=m.track_age_ms===null?'':`<div class="age-kpi"><strong>${formatExactDuration(m.track_age_ms)}</strong><span>vieillissement au départ</span></div>`,trace=m.traceur||{},driver=m.driver||{};el.innerHTML=`<div class="card-title-row"><div><small class="section-kicker">DÉBRIEF AUTOMATIQUE</small><h3>Traceur / conducteur</h3></div><span>${usesTrace?'Comparaison sur GPS réels':'Tracé réel du Traceur indisponible'}</span></div><div class="debrief-role-grid"><article class="traceur"><small>TRACEUR</small><strong>${fmt(trace.distance_km,2)} km</strong><span>${formatExactDuration(trace.duration_ms)}</span><em>${trace.points||0} points GPS</em></article><article class="driver"><small>CONDUCTEUR</small><strong>${fmt(driver.distance_km,2)} km</strong><span>${formatExactDuration(driver.duration_ms)}</span><em>${driver.points||0} points GPS</em></article></div><div class="debrief-kpis">${ageKpis}<div><strong>${fmt(m.planned_km,2)}</strong><span>km référence réelle</span></div><div><strong>${m.average_deviation_m} m</strong><span>écart moyen conducteur</span></div><div><strong>${m.max_deviation_m} m</strong><span>écart maximal</span></div><div><strong>${m.stops}</strong><span>arrêts (${fmt(m.stopped_min,1)} min)</span></div><div><strong>${m.objects_visited}/${m.objects_total}</strong><span>objets approchés</span></div><div><strong>${m.losses}</strong><span>pertes annotées</span></div><div><strong>${m.recoveries}</strong><span>reprises annotées</span></div>${odorKpis}</div><p class="small muted">Les deux distances et durées sont recalculées à partir des points GPS horodatés, même après la fin de la session.</p>`}
