...
function renderCoachingActiveBanner(metrics){
 metrics=metrics||{};const {activeMs,activeKm,trackAgeMs,pauseState}=metrics,valid=value=>value!==null&&value!==undefined&&Number.isFinite(Number(value))&&Number(value)>=0;
 const banner=$('coachingTerrainStatus'),values=[
  ...
  ['coachingTrackAge',trackAgeMs,valid(trackAgeMs)?formatOperationalTrackAge(Number(trackAgeMs)):'—','Début réel du traçage non renseigné'],
 ];
...
function updateCoachingTerrainStatus(next={}){
 const s=activeCoachingSession;if(!s)return;const role=myCoachingRole(s),startedAt=s.driver_started_at||(role==='traceur'?s.laying_started_at:null),finishedAt=s.driver_finished_at||(role==='traceur'?s.track_finished_at:null),started=startedAt?new Date(startedAt).getTime():NaN,finished=finishedAt?new Date(finishedAt).getTime():Date.now(),activeMs=Number.isFinite(started)&&Number.isFinite(finished)?Math.max(0,finished-started):null,origin=s.track_started_at?new Date(s.track_started_at).getTime():NaN,ageEnd=s.driver_finished_at?new Date(s.driver_finished_at).getTime():Date.now(),trackAgeMs=Number.isFinite(origin)&&Number.isFinite(ageEnd)&&ageEnd>=origin?ageEnd-origin:null,hasKm=Object.prototype.hasOwnProperty.call(next,'activeKm');
 ...
 if(typeof renderCoachingActiveBanner==='function')return renderCoachingActiveBanner({activeMs,activeKm,trackAgeMs,pauseState});
...
function applySafeCoachingRealtimeStatus(row){if(!activeCoachingSession||!row)return;for(const key of ['status','phase','driver_finished_at','started_at','ended_at','updated_at'])if(Object.prototype.hasOwnProperty.call(row,key))activeCoachingSession[key]=row[key]}
...
function updateCoachingLiveMetrics(groups,trace=[]){const started=activeCoachingSession?.started_at?new Date(activeCoachingSession.started_at).getTime():0,role=myCoachingRole(activeCoachingSession),driver=([...groups.entries()].find(([userId])=>['driver','solo'].includes(coachingMemberRole(userId)))?.[1]||[]).filter(p=>new Date(p.recorded_at).getTime()>=started),mine=role==='traceur'?trace:driver,activeKm=mine.length>1?routeDistance(mine)/1000:null;updateCoachingTerrainStatus({activeKm})}
