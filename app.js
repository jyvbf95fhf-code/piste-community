...
let fieldMarkers=[],pendingFieldMarker=null,operationalLiveWeather=null,operationalWeatherHistory=[],operationalWeatherTimer=null,operationalWeatherLoading=false,operationalWeatherOdorLayers=[],operationalCorridorVisible=true,operationalAgeTimer=null,activityLibraryView='list',activityLibraryFilters={type:'all',status:'active',favorite:false,query:''},activityLibrarySelection=[],activityLibrarySelectionMode=false,currentActivityDetail=null,reportCurrentModel=null,reportPhotoUrls=[];
...
function updateTerrainCommonStatus(){
 ...
 const reference=recordMode==='operational'?operationalTrackAgeReference():(activeCoachingSession?.track_finished_at||activeCoachingSession?.created_at||null),ageMs=reference?TerrainEngine.ageMs(reference):null;
 setUiText('terrainLiveAge',reference?`Âge de piste : ${formatExactDuration(ageMs)}`:'Âge de piste : —');
...
function operationalTrackAgeReference(){const form=$('pisteForm'),opsValue=$('opsDisappearanceAt')?.value,value=opsValue||form?.elements?.disparition_at?.value||form?.elements?.depart_at?.value||gps.terrainStartedAt||gps.start;const date=value?new Date(value):null;return date&&Number.isFinite(date.getTime())?date:null}
function operationalTrackAgeHours(){const t=operationalTrackAgeReference()?.getTime();return Number.isFinite(t)?Math.max(0,(Date.now()-t)/36e5):0}
