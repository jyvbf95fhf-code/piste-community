...
let coachingWorkMode='team',coachingLastServerSyncAt=0,coachingSyncTimer=null,coachingV10423ResumeInFlight=false;
...
function setCoachingWorkMode(mode){
 ...
 const title=document.querySelector('.coaching-invites .section-kicker');if(title)title.textContent=mode==='duo'?'À DEUX':mode==='team'?'ÉQUIPE':'SEUL';
...
function isCurrentUserLayingActor(s=activeCoachingSession){if(!s)return false;const role=myCoachingRole(s);return(role==='traceur'&&s.laying_mode==='traceur')||(isCoachingOwner(s)&&role==='coach'&&s.laying_mode==='coach')}
...
function updateCoachingPrimaryActions(){
 ...
 if(Number(s?.workflow_version)>=2){hideLegacyCoachingPrimaryActions();if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();const phase=coachingPhase(s),completed=phase==='completed'||!!s.driver_finished_at;const message=completed?(role==='driver'?'Parcours terminé.':owner||role==='coach'?'Parcours du Conducteur terminé. Vous pouvez terminer la session et ouvrir le débrief.':'Parcours du Conducteur terminé.'):role==='driver'?(phase==='preparation'?'En attente : le Coach prépare la piste.':phase==='laying'?'Pose en cours : le Coach trace actuellement la piste.':phase==='waiting_ready'||phase==='coach_ready'?'La piste est prête — vous pouvez démarrer.':phase==='driver_running'?'Parcours en cours.':''):isCurrentUserLayingActor(s)?(phase==='preparation'?'Appuyez sur « Je pars tracer » pour démarrer la pose et le GPS.':phase==='laying'?'Pose en cours — terminez puis validez « Piste prête ».':''):'';setUiText('coachingRoleInstruction',message);return}
...
async function requestCoachingPreviewLocation(){
 ...
 navigator.geolocation.getCurrentPosition(pos=>{coachingPreviewPosition={lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:Number(pos.coords.accuracy)||0,recorded_at:new Date(pos.timestamp||Date.now()).toISOString()};updateCoachingDeparture();updateCoachingTerrainStatus();renderCoachingMap();},err=>{coachingPreviewPosition=null;coachingGpsError=err?.code===1?'autorisation de géolocalisation refusée':'position GPS indisponible';updateCoachingDeparture();updateCoachingTerrainStatus();updateCoachingPrimaryActions();},{enableHighAccuracy:true,maximumAge:5000,timeout:15000});
 return true;
}
async function openCoachingSession(id,{resume=false}={}){
 requestCoachingPreviewLocation();clearCoachingRealtime();coachingKeepViewport=false;
...
async function renderCoachingMap(){
 ...
 const visibility=coachingDataVisibility(activeCoachingSession),allLive=visibility.live?(liveRes.data||[]):[],trace=visibility.trace?(traceRes.data||[]):[],annotations=visibility.markers?(markerRes.data||[]):[];if(memberRes.data)activeCoachingSession.coaching_members=memberRes.data;
...
$('openTerrainHomeBtn').onclick=e=>{e.preventDefault();showPage('coachingPage');setCoachingStage('prepare');setCoachingWorkMode('solo')};
...
document.querySelectorAll('[data-coaching-work-mode]').forEach(b=>b.onclick=()=>setCoachingWorkMode(b.dataset.coachingWorkMode));
...
async function coachingTransitionV1040(rpc){if(coachingV10423TransitionInFlight)return false;coachingV10423TransitionInFlight=true;try{const s=activeCoachingSession;if(!s||!session?.user)return false;const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){coachingToast(`Action impossible : ${error.message}`);return false}await refreshActiveCoachingSession(s.id);updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();return !!data||!error}finally{coachingV10423TransitionInFlight=false}}
async function markCoachingTrackReady(){const s=activeCoachingSession;if(isCurrentUserLayingActor(s)&&coachingPhase(s)==='laying')stopTraceurTracking();return coachingTransitionV1040('mark_coaching_track_ready')}
...
async function finishDriverRun(){const ok=await coachingTransitionV1040('finish_driver_run');if(ok)stopCoachingPresence();return ok}
...
function applyV1040RoleSurface(){
 ...
 else if(role==='observer'){show('observerTrackChoice');const toggle=$('observerTrackToggle'),canChoose=completed||coachingBlindMode(s)==='normal';if(toggle){toggle.disabled=!canChoose;toggle.checked=canChoose&&localStorage.getItem(`piste-observer-track-${s.id}`)==='true'}}
 else if(v1040&&layingActor&&phase==='preparation'){setUiText('startLayingBtn','Je pars tracer');show('startLayingBtn')}
 else if(v1040&&layingActor&&phase==='laying'){setUiText('trackReadyBtn','Piste prête');show('trackReadyBtn')}
 ...
 setUiText('coachingRoleInstruction',informativeCompleted?(role==='driver'?'Parcours terminé.':owner||role==='coach'?'Parcours du Conducteur terminé. Vous pouvez terminer la session et ouvrir le débrief.':'Parcours du Conducteur terminé.'):role==='observer'?'Vous observez la session sans commande de pilotage.':role==='driver'&&phase==='preparation'?'En attente : le Coach prépare la piste.':role==='driver'&&phase==='laying'?'Pose de la piste en cours.':role==='driver'&&['waiting_ready','coach_ready'].includes(phase)?'La piste est prête — vous pouvez démarrer.':role==='driver'&&phase==='driver_running'?'Parcours en cours.':layingActor&&phase==='preparation'?'Appuyez sur « Je pars tracer » pour démarrer la pose et le GPS.':layingActor&&phase==='laying'?'Pose en cours — terminez puis validez « Piste prête ».':`Rôle : ${coachingRoleLabel(role)} · ${coachingPhaseLabelV1040(s)}`);updateCoachingDebriefAccess();renderCoachingScenario()
}
