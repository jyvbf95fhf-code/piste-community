...
function updateCoachingPhase(){const el=$('coachingPhase'),s=activeCoachingSession;if(!el||!s)return;const completed=coachingPhase(s)==='completed';el.innerHTML=completed?'PARCOURS TERMINÉ<small>Débrief disponible</small>':esc(coachingPhaseLabel(s));el.className='coaching-phase '+(completed?'completed':s.status||'waiting')}
...
async function openCoachingDebriefOnce(id,statusText){if(coachingSessionEndHandledId===id)return false;coachingSessionEndHandledId=id;stopCoachingPresence();stopTraceurTracking();await closeFakeLock({returnToMap:false,reacquireWake:false});clearCoachingRealtime();clearVerifiedActiveCoaching(id);$('coachingLivePanel')?.classList.remove('waiting-room');updateCoachingPhase();updateCoachingPrimaryActions();updateCoachingDebriefAccess();setCoachingStage('debrief');refreshCoachingMapLayout();setUiText('coachingLiveStatus',statusText);await calculateCoachingDebrief();await loadSavedCoachingDebrief();return true}
...
async function saveCoachingDriverFeedback(e){e?.preventDefault();const s=activeCoachingSession,form=$('coachingDriverFeedbackForm');if(!s||!form||!hasCoachingCapability('drive',s)||!(s.status==='ended'||coachingPhase(s)==='completed'))return alert('Le retour Conducteur est disponible après le parcours.');const f=new FormData(form),now=new Date().toISOString(),payload={session_id:s.id,owner_id:s.owner_id,driver_notes:f.get('driver_notes')||null,last_editor_id:session.user.id,updated_at:now},{error}=await supabase.from('coaching_debriefs').upsert(payload,{onConflict:'session_id'});if(error){alert('Retour non enregistré : '+error.message);return}setUiText('coachingDriverFeedbackState','Retour du Conducteur enregistré.')}
