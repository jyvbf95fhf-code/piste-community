...
function preReleaseDebugRecordError(error,kind='exception'){const clean=preReleaseDebugSanitize(error);if(kind==='supabase')preReleaseCoachingDebug.lastSupabaseError=clean;else preReleaseCoachingDebug.lastException=clean;preReleaseDebugRefresh()}
...
async function renderCoachingMap(options){
 ...
 if(coachingGlobalPhase(activeCoachingSession)!=='active'){
  ...
  coachingDebriefData=data;coachingLayers.forEach(layer=>{try{layer.remove()}catch{}});coachingLayers=[];for(const marker of coachingParticipantMarkers.values())marker.remove();coachingParticipantMarkers.clear();coachingPreviewMarker?.remove();coachingPreviewMarker=null;coachingPreviewAccuracyCircle?.remove();coachingPreviewAccuracyCircle=null;
