...
let fieldMarkers=[],pendingFieldMarker=null,operationalLiveWeather=null,operationalWeatherTimer=null,operationalWeatherLoading=false,operationalWeatherOdorLayers=[],activityLibraryView='list',activityLibraryFilters={type:'all',status:'active',favorite:false,query:''},activityCompare=[],activityLibrarySelection=[],activityLibrarySelectionMode=false,currentActivityDetail=null,reportCurrentModel=null,reportPhotoUrls=[];
...
function activityLibraryCard(x){
 ...
 return `<article class="library-card activity-open ${archived?'archived':''} ${selected?'selected':''}" data-id="${x.id}" data-type="${x._type}" data-key="${key}">
  ...
  <div class="library-actions"><button class="primary openLibraryItem" type="button">Ouvrir</button><details class="library-actions-menu"><summary>••• Plus d’actions</summary>${owned?`<button class="secondary manageLibraryItem" type="button">Modifier</button><button class="secondary duplicateLibraryItem" type="button">Dupliquer</button><button class="secondary archiveLibraryItem" type="button">${archived?'Désarchiver':'Archiver'}</button>${visibility==='public'?'<button class="secondary copyLibraryLink" type="button">Copier le lien</button>':''}<button class="danger-button deleteLibraryItem" type="button">Supprimer</button>`:'<button class="danger-button deleteLibraryItem" type="button">Quitter</button>'}</details></div></div></article>`;
}
...
async function deleteSelectedActivities(){const keys=[...activityLibrarySelection];if(!keys.length||!confirm(`Archiver ${keys.length} activité(s) ? Elles resteront restaurables depuis la corbeille.`))return;for(const key of keys){const [type,id]=key.split(':');await archiveLibraryItem(type,id,true)}setLibrarySelectionMode(false);updateV8Home()}
