...
let coachingSessionEndHandledId=null,coachingV1040FinishInFlight=false;
...
async function savePlanner(mode='copy',afterSave='library'){
 ...
 if(afterSave==='start'&&saved){selectedTrainingRoute=saved;beginNewPiste('training',{preservePreparedRoute:true,preserveDisappearance:true});setTimeout(()=>applySelectedTrainingRoute(),150);return}
...
async function createCoaching(){const routeSelect=$('coachingRouteSelect'),route=trainingRoutes.find(r=>r.id===routeSelect?.value),button=$('createCoachingSession'),blindMode=$('coachingVisibility')?.value||'normal';if(!route){setUiText('coachingCreateMsg','Choisis un tracé existant ou crée-en un sur la carte.');return}if(button)button.disabled=true;setUiText('coachingCreateMsg',pendingCreatedCoachingSession?'Récupération de la session déjà créée…':'Création de la salle de préparation…');try{if(pendingCreatedCoachingSession){await recoverCreatedCoachingSession();return}const now=new Date(),technicalName=`Coaching · ${now.toLocaleDateString('fr-FR')} · ${now.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}`,payload={owner_id:session.user.id,route_id:route.id,name:technicalName,status:'waiting',workflow_version:2,blind_mode:blindMode,laying_mode:$('coachingLayingMode')?.value||'coach',planned_route:route.route||[],planned_markers:route.waypoints||[],odor_model:route.odor_model||{},visibility_mode:'all',invite_code:coachingCode(),expires_at:new Date(Date.now()+7*864e5).toISOString()},{data,error}=await supabase.from('coaching_sessions').insert(payload).select('id,invite_code,name,status,workflow_version,phase,blind_mode,laying_mode,started_at').single();if(error||!data?.id){setUiText('coachingCreateMsg','Création impossible : '+(error?.message||'identifiant de session absent.'));return}pendingCreatedCoachingSession={id:data.id,code:data.invite_code,role:'coach'};const scenario={title:$('coachingScenarioTitle')?.value.trim()||'',text:$('coachingScenarioText')?.value.trim()||'',photo_selected:!!$('coachingScenarioPhoto')?.files?.length};try{localStorage.setItem(`piste-coaching-scenario-${data.id}`,JSON.stringify(scenario))}catch{}if(scenario.photo_selected)setUiText('coachingCreateMsg','Session créée. La photo reste locale tant qu’aucun stockage durable n’est configuré.');const inviteFailures=await inviteCoachingFriends(data.id);if(inviteFailures.length)setUiText('coachingCreateMsg',`Session créée, mais ${inviteFailures.length} invitation(s) n’ont pas pu être envoyées. Le code ${data.invite_code} reste utilisable.`);await recoverCreatedCoachingSession();coachingFriendInvites=[];renderCoachingFriendInvites()}catch(error){if(pendingCreatedCoachingSession)setUiText('coachingCreateMsg',`Session ${pendingCreatedCoachingSession.code} créée avec le statut En attente, mais son affichage a échoué : ${error?.message||'erreur inattendue.'}`);else setUiText('coachingCreateMsg','Création impossible : '+(error?.message||'erreur inattendue.'))}finally{if(button)button.disabled=false}}
...
async function refreshActiveCoachingSession(id){const {data,error}=await supabase.rpc('get_my_coaching_sessions',{p_session_id:id});const fresh=!error&&Array.isArray(data)?data[0]:null;if(!fresh||activeCoachingSession?.id!==id)return false;activeCoachingSession={...activeCoachingSession,...fresh};const index=coachingSessions.findIndex(x=>x.id===id);if(index>=0)coachingSessions[index]=activeCoachingSession;else coachingSessions.push(activeCoachingSession);if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();return true}
...
function updateCoachingParticipants(groups,trace){const el=$('coachingParticipants');if(!el)return;const members=activeCoachingSession?.coaching_members||[],rows=members.map(member=>{const points=groups.get(member.user_id)||[],last=points.at(-1),owner=member.user_id===activeCoachingSession.owner_id?' • organisateur':'',label=coachingPresenceLabelV1040(member,last);return `<span class="participant-chip ${member.role}"><i></i>${esc(coachingRoleLabel(member.role))}${owner}<small>${label}${last?` · ${new Date(last.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}`:''}</small></span>`});if(trace.length&&!members.some(m=>m.role==='traceur')){const last=trace.at(-1);rows.push(`<span class="participant-chip traceur"><i></i>Traceur <small>${coachingPresenceLabelV1040(null,last)} · ${new Date(last.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</small></span>`)}el.innerHTML=rows.join('')||'<span class="participant-chip waiting"><i></i>Aucun participant</span>'}
...
function updateCoachingDebriefAccess(){const s=activeCoachingSession,role=myCoachingRole(s),ready=!!s&&(s.status==='ended'||coachingPhase(s)==='completed'),coach=ready&&(isCoachingOwner(s)||role==='coach'),driver=ready&&role==='driver';$('calculateCoachingDebrief')?.classList.toggle('hidden',!ready);$('coachingDebriefForm')?.classList.toggle('hidden',!coach);$('coachingDriverFeedbackForm')?.classList.toggle('hidden',!driver)}
...
async function coachingTransitionV1040(rpc){const s=activeCoachingSession;if(!s||!session?.user)return false;const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){coachingToast(`Action impossible : ${error.message}`);return false}await refreshActiveCoachingSession(s.id);updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();return !!data||!error}
