...
async function submitCoachingWizard(){
 ...
 try{
  ...
  const routeId=coachingWizard.sessionType==='solo'?null:(withoutRoute?null:preparation.routeId);debugEvent('CREATE_COACHING_START',`withoutRoute=${withoutRoute?'oui':'non'}`);created=await createCoaching({validated:true,routeId,members:coachingWizardMembers(),blindMode:coachingWizard.mode,withoutRoute,onCreated:data=>{coachingWizard.createdSessionId=data?.id||null}});
...
async function createCoaching(options={}){
 ...
 try{
  const members=options.members||coachingCreationMembers(),payload={p_route_id:withoutRoute?null:route.id,p_members:members,p_blind_mode:options.blindMode||$('coachingVisibility')?.value||'normal'};debugEvent('RPC_CREATE_START',JSON.stringify({blind_mode:payload.p_blind_mode,route_id_present:!!payload.p_route_id,members:typeof preReleaseDebugMembers==='function'?preReleaseDebugMembers(members):members.map(member=>({role:member?.role||'—'}))}));
  const {data,error}=await supabase.rpc('create_coaching_people_session_v1045',payload);
...
async function openCoachingDebriefOnce(id,statusText){
 ...
 const operation=(async()=>{
  ...
  stopCoachingPresence();stopTraceurTracking();await closeFakeLock({returnToMap:false,reacquireWake:false});clearCoachingRealtime();clearVerifiedActiveCoaching(id);$('coachingLivePanel')?.classList.remove('waiting-room');updateCoachingPhase();updateCoachingPrimaryActions();updateCoachingDebriefAccess();setCoachingStage('debrief');refreshCoachingMapLayout();setUiText('coachingLiveStatus',statusText);
...
function renderCoachingScenario(){const box=$('coachingScenarioReveal'),s=activeCoachingSession;if(!box||!s)return;const phase=coachingPhase(s),role=myCoachingRole(s);if(role!=='driver'||!['coach_ready','driver_running'].includes(phase)){box.classList.add('hidden');return}let localScenario=null;try{localScenario=JSON.parse(localStorage.getItem(`piste-coaching-scenario-${s.id}`)||'null')}catch{}const title=s.scenario_title||localScenario?.title||'',text=s.scenario_text||localScenario?.text||'';box.classList.remove('hidden');box.innerHTML=`<strong>${esc(title||'Scénario de départ')}</strong>${text?`<p>${esc(text)}</p>`:'<p class="muted">Aucun texte de scénario.</p>'}${localScenario?.photo_selected&&!s.scenario_photo_url?'<p class="small muted">Photo non persistante : stockage durable non configuré.</p>':''}<button id="coachingScenarioAck" class="secondary" type="button">J’ai lu</button>`;bindClick('coachingScenarioAck',()=>{const key=`piste-coaching-scenario-ack-${s.id}`;localStorage.setItem(key,new Date().toISOString());box.classList.add('acknowledged')})}
