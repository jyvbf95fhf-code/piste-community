...
function updateCoachingTerrainStatus(){const el=$('coachingTerrainStatus');if(!el)return;const gps=coachingPreviewPosition||coachingOwnPosition,quality=!gps?'GPS —':Number(gps.accuracy_m)<=20?'GPS bon':Number(gps.accuracy_m)<=60?'GPS précis':'GPS imprécis';const startedAt=myCoachingRole(activeCoachingSession)==='traceur'?activeCoachingSession?.laying_started_at:activeCoachingSession?.driver_started_at||activeCoachingSession?.started_at;const started=startedAt?new Date(startedAt).getTime():0;const finishedAt=myCoachingRole(activeCoachingSession)==='traceur'?activeCoachingSession?.track_finished_at:activeCoachingSession?.driver_finished_at;const activeMs=started&&(finishedAt||isCoachingGpsTracking())?Math.max(0,(finishedAt?new Date(finishedAt).getTime():Date.now())-started):0;const activeClock=formatExactDuration(activeMs).replace(/^0 h /,'').replace(' min ',' min ');const age=activeCoachingSession?.track_finished_at,ageMs=coachingTimingV1045(activeCoachingSession).waitingMs;el.innerHTML=`<span>${quality}</span><span>Âge ${age&&ageMs!==null?formatExactDuration(ageMs): '—'}</span><span>${finishedAt?'Terminé':coachingTerrainPaused?'En pause':isCoachingGpsTracking()?'Actif':activeCoachingSession?.search_mode==='deferred'?'En attente':'Aperçu GPS'} ${activeClock}</span><span>Distance ${$('coachingLiveDistance')?.textContent||'0,00 km'}</span>`}
