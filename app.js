...
async function startActiveCoaching(){
 ...
 if(v1040){
  if(!await ensureCoachingStarted())return;
  $('coachingLivePanel').classList.remove('waiting-room');$('coachingWaitingActions').classList.add('hidden');setCoachingStage('live');setCoachingPanel('team');updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();await renderCoachingMap();return;
 }
