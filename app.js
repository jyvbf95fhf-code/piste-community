...
async function coachingTransitionV1040(rpc){const s=activeCoachingSession;if(!s||!session?.user)return false;const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){coachingToast(`Action impossible : ${error.message}`);return false}await refreshActiveCoachingSession(s.id);updateCoachingPhase();applyV1040RoleSurface();return !!data||!error}
async function markCoachingTrackReady(){if(coachingGpsRole()==='traceur'&&coachingPhase(activeCoachingSession)==='laying')stopTraceurTracking();return coachingTransitionV1040('mark_coaching_track_ready')}
