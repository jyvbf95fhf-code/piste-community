...
async function renderCoachingMap(options){
 ...
 const id=activeCoachingSession.id,[liveRes,traceRes,markerRes,memberRes,currentRes]=await Promise.all([supabase.from('coaching_live_points').select('owner_id,lat,lon,accuracy_m,heading_deg,speed_mps,recorded_at').eq('session_id',id).order('recorded_at'),supabase.from('coaching_trace_points').select('owner_id,lat,lon,accuracy_m,recorded_at').eq('session_id',id).order('recorded_at'),supabase.from('coaching_markers').select('*').eq('session_id',id).order('created_at'),supabase.from('coaching_members').select('role,user_id,invitation_status').eq('session_id',id),activeCoachingSession.visibility_version===3?supabase.from('coaching_current_positions').select('*').eq('session_id',id):Promise.resolve({data:[]})]);
 if(activeCoachingSession?.id!==id)return;
 const member=myCoachingMember(activeCoachingSession),surface=coachingActiveSurfaceModel(activeCoachingSession,coachingPhase(activeCoachingSession),member?.role||null),visibility=surface.mapPriority?surface.visibility:coachingDataVisibility(activeCoachingSession),allLive=visibility.live?(liveRes.data||[]).filter(p=>coachingCanSeeLiveOwner(activeCoachingSession,p.owner_id)):[],trace=visibility.trace?(traceRes.data||[]):[],annotations=visibility.markers?(markerRes.data||[]):[];if(memberRes.data)activeCoachingSession.coaching_members=memberRes.data.map(m=>({...activeCoachingSession.coaching_members?.find(old=>old.user_id===m.user_id),...m}));
 ...
 const fullRoute=activeCoachingSession.planned_route||[],role=myCoachingRole(activeCoachingSession),route=visibility.planned?fullRoute:[];
...
function drawCoachingReplay(){
 ...
 if(coachingLayerVisibility.odor&&route.length>1&&Number.isFinite(ageHours))addOdorLayers(coachingMap,route,{...(activeCoachingSession?.odor_model||{}),age_hours:ageHours},coachingLayers);
...
function applyCoachingActiveSurface(model){
 ...
 if(!model?.mapPriority){coachingActiveBannerState={activeKm:null};delete panel.dataset.mapPriority;Object.values(blocks).forEach(element=>element?.style.removeProperty('display'));odorControl?.classList.remove('hidden');odorControl?.removeAttribute('aria-hidden');return}
 ...
 if(odorControl){odorControl.classList.toggle('hidden',!model.odorVisible);odorControl.setAttribute('aria-hidden',String(!model.odorVisible))}
}
