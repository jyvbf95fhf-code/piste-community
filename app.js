...
const gpsWatchRegistry=(()=>{const watches=new Map();const start=(context,onSuccess,onError,options)=>{if(watches.has(context))return watches.get(context);void gpsPermission.getState();if(!globalThis?.navigator?.geolocation){onError?.({code:0,message:'Géolocalisation non disponible.'});return null}let id;try{id=globalThis.navigator.geolocation.watchPosition(onSuccess,onError,options)}catch(error){onError?.(error);return null}watches.set(context,id);return id};const stop=context=>{const id=watches.get(context);if(id===undefined)return false;try{globalThis.navigator.geolocation.clearWatch(id)}catch{}watches.delete(context);return true};const has=context=>watches.has(context);const get=context=>watches.get(context)??null;return{start,stop,has,get}})();
...
async function requestCoachingPreviewLocation(){
 ...
 },err=>{if(activeCoachingSession?.id!==id)return;coachingGpsError=err?.code===1?'autorisation de géolocalisation refusée':'position GPS indisponible';updateCoachingTerrainStatus()},{enableHighAccuracy:true,maximumAge:5000,timeout:15000});return coachingPreviewWatch!==null;
}
