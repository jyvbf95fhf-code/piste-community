...
async function loadCoachingHub(){
 ...
 coachingSessions=error?[]:(Array.isArray(data)?data:[]);renderCoachingSessions();
}
...
async function openCoachingSession(id){
 ...
 let s=coachingSessions.find(x=>x.id===id);if(!s){const {data}=await supabase.rpc('get_my_coaching_sessions',{p_session_id:id});s=Array.isArray(data)?data[0]:null}if(!s)return;
 activeCoachingSession=s;coachingAutoMetrics=null;const role=isSoloCoaching(s)?'solo':myCoachingRole(s);refreshActiveSessionShortcut();
 $('coachingLivePanel').classList.remove('hidden');$('coachingRole').textContent=coachingRoleLabel(role).toUpperCase();$('coachingLiveTitle').textContent=s.name||'Session coachée';$('coachingLiveCode').textContent=s.invite_code;$('coachingLiveStatus').textContent=`${coachingStatusLabel(s.status)} • ${s.visibility_mode==='all'?'mode partagé':s.visibility_mode==='progressive'?'mode progressif':'mode double aveugle'}`;
...
async function ensureCoachingStarted(){if(!activeCoachingSession)return false;if(activeCoachingSession.status==='live')return true;const {error}=await supabase.rpc('start_coaching_session',{p_session_id:activeCoachingSession.id});if(error){alert('Départ impossible : '+error.message);return false}activeCoachingSession.status='live';activeCoachingSession.started_at=new Date().toISOString();$('coachingLiveStatus').textContent='En direct • positions GPS synchronisées';return true}
...
async function startActiveCoaching(){
 ...
 $('startCoachingLive').textContent=coachingPrimaryLabel(role,'live');$('endCoachingLive').classList.toggle('hidden',!['coach','driver','solo'].includes(role));
}
async function finishActiveCoaching(){if(!activeCoachingSession||!confirm('Mettre fin à cette piste pour toute l’équipe ?'))return;const id=activeCoachingSession.id,{error}=await supabase.rpc('finish_coaching_session',{p_session_id:id});if(error){alert('Fin impossible : '+error.message);return}stopCoachingPresence();stopTraceurTracking();activeCoachingSession.status='ended';clearCoachingRealtime();closeFakeLock();refreshActiveSessionShortcut();await loadCoachingHub();await openCoachingSession(id)}
