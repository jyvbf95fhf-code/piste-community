...
function updateHomeCoachingState(mode='ready'){
 ...
 const waiting=coachingSessions.find(s=>s.search_mode==='deferred'&&s.status==='waiting'&&s.track_finished_at&&s.coaching_members?.some(m=>m.user_id===session?.user?.id&&['accepted','active'].includes(m.invitation_status)));
 ...
 if(waiting){state.textContent='Recherche différée en attente';info.textContent=waiting.name||'Session Coaching';action.textContent='Reprendre la session';action.dataset.coachingHomeAction='deferred';return}
...
async function handleHomeCoachingAction(action){
 if(action==='deferred'){const waiting=coachingSessions.find(s=>s.search_mode==='deferred'&&s.status==='waiting'&&s.track_finished_at&&s.coaching_members?.some(m=>m.user_id===session?.user?.id&&['accepted','active'].includes(m.invitation_status)));if(waiting){showPage('coachingPage');await openCoachingSession(waiting.id);return}}
...
function updateCoachingWorkflowNotice(s,role){
 ...
 else if(role==='driver'&&['waiting_ready','coach_ready','laid'].includes(phase))el.textContent=s.search_mode==='deferred'&&!s.traceur_ready_at?'Piste tracée — en attente du retour du Traceur.':s.search_mode==='deferred'?'Le Traceur est en place. Vous pouvez vous préparer à démarrer.':'Piste tracée — vous pouvez démarrer au point de départ';
...
function coachingDeferredNoPreviewV1045(s){return s?.search_mode==='deferred'&&(s.status!=='live'||coachingPhase(s)!=='driver_running')}
...
function coachingSearchLabelV1045(s){return s?.search_mode==='deferred'?'Recherche différée':'Recherche immédiate'}
...
function syncCoachingDeferredV1045(s=activeCoachingSession){
 const deferred=s?.search_mode==='deferred',phase=coachingPhase(s),pending=deferred&&phase==='waiting_ready'&&!s.traceur_ready_at&&s.status==='waiting';
 if(deferred){
 ...
 if(pending&&myCoachingRole(s)==='driver')$('driverStartBtn')?.classList.add('hidden');
...
async function createCoaching(){
 ...
 try{
  const {data,error}=await supabase.rpc('create_coaching_people_session_v1045',{p_route_id:withoutRoute?null:route.id,p_members:coachingCreationMembers(),p_blind_mode:$('coachingVisibility')?.value||'normal',p_search_mode:$('coachingSearchMode')?.value||'immediate'});
...
async function coachingTransitionV1040(rpc){if(coachingV10423TransitionInFlight)return false;coachingV10423TransitionInFlight=true;try{const s=activeCoachingSession;if(!s||!session?.user)return false;const allowed={mark_coaching_traceur_ready_v1045:['waiting_ready'],start_coaching_laying:['preparation'],mark_coaching_track_ready:['laying'],start_driver_run:['waiting_ready','coach_ready'],finish_driver_run:['driver_running']};if(allowed[rpc]&&!allowed[rpc].includes(coachingPhase(s)))return false;const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){coachingToast(`Action impossible : ${error.message}`);return false}if(data?.id===s.id){for(const key of ['status','phase','laying_started_at','track_finished_at','driver_started_at','driver_finished_at','traceur_ready_at','search_mode'])if(Object.prototype.hasOwnProperty.call(data,key))s[key]=data[key]}await refreshActiveCoachingSession(s.id);updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();return !!data||!error}finally{coachingV10423TransitionInFlight=false}}
async function markCoachingTrackReady(){const s=activeCoachingSession;if(!isCurrentUserLayingActor(s))return false;const ok=await coachingTransitionV1040('mark_coaching_track_ready');if(ok)stopTraceurTracking();return ok}
