...
function renderParticipantObservations(observations=coachingParticipantObservations){
 ...
 const form=$('coachingObservationForm'),textarea=$('coachingObservationBody');if(form){form.onsubmit=submitParticipantObservation;if(textarea)textarea.oninput=()=>{form.dataset.dirty='true'}}if(pendingDraft&&form&&textarea){textarea.value=pendingDraft.body;form.dataset.observationVersion=pendingDraft.version;form.dataset.dirty='true';setUiText('coachingObservationState','Une mise à jour distante est disponible. Votre brouillon local est conservé ; enregistrez pour détecter le conflit.')}
}
