...
const APP_RELEASE_VERSION='10.52';
const APP_BUILD_FALLBACK='source';
const APP_PREVIEW_LATEST_ORIGIN='https://stats-piste-community-v10-51-1-test.vercel.app';
let versionUpdate={current:{version:APP_RELEASE_VERSION,build:APP_BUILD_FALLBACK},available:null,availableOrigin:null,deferred:false,loading:false};
// Immutable Vercel Preview hostnames must consult the stable Preview channel.
function isImmutablePreviewHost(host=location.hostname){return /^stats-piste-community-v10-51-1-test-[a-z0-9]+\.vercel\.app$/i.test(host)}
function versionLatestOrigin(){return isImmutablePreviewHost()?APP_PREVIEW_LATEST_ORIGIN:location.origin}
function versionPayloadUrl(origin){const url=new URL('/version.json',origin);url.searchParams.set('ts',Date.now());return url.toString()}
async function fetchVersionPayload(origin){const response=await fetch(versionPayloadUrl(origin),{cache:'no-store',headers:{Accept:'application/json'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const payload=await response.json();if(!payload?.version||!payload?.build)throw new Error('version source invalide');return payload}
...
async function applyVersionUpdate(){if(!versionUpdate.available)return false;if(versionUpdateSessionActive()){versionUpdate.deferred=true;renderVersionState();return false}const targetOrigin=versionUpdate.availableOrigin||versionLatestOrigin();try{const registration=await navigator.serviceWorker?.getRegistration();await registration?.update();if(registration?.waiting)registration.waiting.postMessage({type:'SKIP_WAITING'});if(window.caches){const keys=await caches.keys();await Promise.all(keys.map(key=>caches.delete(key)))}}catch{}const targetUrl=new URL(`${location.pathname}${location.search}${location.hash}`,targetOrigin);if(targetUrl.origin===location.origin)location.reload();else location.assign(targetUrl.toString());return true}
async function checkAppVersion(){if(versionUpdate.loading)return false;versionUpdate.loading=true;renderVersionState();try{const latestOrigin=versionLatestOrigin();const [localResult,latestResult]=await Promise.allSettled([fetchVersionPayload(location.origin),fetchVersionPayload(latestOrigin)]);const local=localResult.status==='fulfilled'?localResult.value:null,remote=latestResult.status==='fulfilled'?latestResult.value:null;if(!remote)throw latestResult.reason||new Error('latest version inaccessible');const current={version:local?.version||APP_RELEASE_VERSION,build:local?.build||APP_BUILD_FALLBACK};versionUpdate.current=current;const newer=remote.version!==current.version||remote.build!==current.build;versionUpdate.available=newer?remote:null;versionUpdate.availableOrigin=newer?latestOrigin:null;renderVersionState();return true}catch{renderVersionState();return false}finally{versionUpdate.loading=false}}
...
let replayMap=null,replayPlayer=null,replayDataset=null,replayUnsubscribe=null,replayLayers=null,replayFollowMode='free',replay3dPrototype=null,replay3dUnsubscribe=null,replaySurface={panelId:'blackBoxReplay',mapId:'activityReplayMap',back:()=>setBlackBoxTab('summary')};
...
function locateCoachingUser(){const p=coachingPreviewPosition||coachingOwnPosition,lat=Number(p?.lat),lon=Number(p?.lon);if(!coachingMap||!Number.isFinite(lat)||!Number.isFinite(lon)){setUiText('coachingLiveStatus','Position GPS indisponible pour le moment.');$('recenterCoachingMap')?.classList.remove('active');return false}try{coachingMap.invalidateSize({pan:false});coachingKeepViewport=false;coachingAutoCenteredSessionId=activeCoachingSession?.id||null;const zoom=Math.max(15,coachingMap.getZoom?.()||16);coachingMap.setView([lat,lon],zoom,{animate:true,duration:.25});$('recenterCoachingMap')?.classList.add('active');setTimeout(()=>coachingMap?.invalidateSize({pan:false}),80);return true}catch(error){setUiText('coachingLiveStatus','Recentrage indisponible pour le moment.');$('recenterCoachingMap')?.classList.remove('active');return false}}
...
function togglePlannerFollow(){if(plannerFollowWatch!==null){stopPlannerFollow();return}if(!navigator.geolocation){$('plannerLocationStatus').textContent='Géolocalisation non disponible.';return}document.querySelector('[data-planner-mode="follow"]')?.classList.add('active');$('plannerDirectBtn')?.classList.add('active');$('plannerDirectBtn')?.setAttribute('aria-pressed','true');$('plannerLocationStatus').textContent='Marche GPS en direct : le tracé suit ta position.';plannerFollowWatch=navigator.geolocation.watchPosition(appendPlannerGpsPoint,plannerLocationError,{enableHighAccuracy:true,maximumAge:1000,timeout:20000})}
function stopPlannerFollow(){if(plannerFollowWatch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(plannerFollowWatch);plannerFollowWatch=null;document.querySelector('[data-planner-mode="follow"]')?.classList.remove('active');$('plannerDirectBtn')?.classList.remove('active');$('plannerDirectBtn')?.setAttribute('aria-pressed','false')}
...
function syncCoachingDeferredV1045(s=activeCoachingSession){
 ...
 if(deferred||choicePending){
  if(coachingDeferredNoPreviewV1045(s)&&coachingPreviewWatch!==null){navigator.geolocation?.clearWatch(coachingPreviewWatch);coachingPreviewWatch=null;coachingPreviewPosition=null;}
...
function isCurrentUserLayingActor(s=activeCoachingSession){if(!s)return false;const role=myCoachingRole(s),member=myCoachingMember(s),accepted=!member||['accepted','active'].includes(member.invitation_status);if(s.visibility_version===3)return role==='traceur'&&accepted;return(role==='traceur'&&s.laying_mode==='traceur'&&accepted)||(isCoachingOwner(s)&&role==='coach'&&s.laying_mode==='coach')}
...
async function requestCoachingPreviewLocation(){
 ...
 coachingPreviewWatch=navigator.geolocation.watchPosition(pos=>{
 ...
 },err=>{if(activeCoachingSession?.id!==id)return;coachingGpsError=err?.code===1?'autorisation de géolocalisation refusée':'position GPS indisponible';updateCoachingTerrainStatus()},{enableHighAccuracy:true,maximumAge:5000,timeout:15000});return true;
}
...
function ensureMissionReplayTab(){
 const nav=$('missionTabs'),report=$('missionTabReport');if(!nav||!report)return null;
 let replay=$('missionTabReplay');
 if(!replay){replay=document.createElement('button');replay.id='missionTabReplay';replay.dataset.missionTab='replay';replay.setAttribute('role','tab');replay.setAttribute('aria-controls','missionReplay');replay.textContent='Replay';replay.onclick=()=>setMissionTab('replay');}
 if(replay.parentElement!==nav||replay.previousElementSibling!==report)nav.insertBefore(replay,report.nextSibling);
 return replay;
}
...
function renderMissionDossier(source){
 ...
 $('missionDebrief').innerHTML=missionDebriefHtml(source);$('missionAnalysis').innerHTML=coachingArchive?`<h3>NOTES</h3>${missionDebriefHtml(source)}`:$('missionAnalysis').innerHTML;$('missionMapKpis').innerHTML=missionInfo([...missionMetrics(source),...missionWeather(source)]);renderMissionReplay(source);
...
function renderMissionReplay(source){
 const panel=$('missionReplay');if(!panel)return;
 const state=replayEntryState(source?.row,source?.type),dataset=replaySourceDataset(source);
 if(state.disabled){panel.innerHTML='<div class="black-box-replay"><b>Replay disponible après la fin de la piste</b><p>Cette session est encore active.</p></div>';return}
 if(!dataset.capabilities.replayAvailable){panel.innerHTML='<div class="black-box-replay"><b>Replay indisponible pour cette piste</b><p>Données temporelles incomplètes pour cette session.</p></div>';return}
 panel.innerHTML='<div class="black-box-replay"><b>Replay de la piste</b><p>Les données temporelles sont disponibles.</p><button type="button" class="primary" data-mission-replay>▶ Replay de la piste</button></div>';
 panel.querySelector('[data-mission-replay]').onclick=()=>renderReplaySurface(dataset,'missionReplay',()=>setMissionTab('map'),'missionReplayMap');
}
function setMissionTab(tab){if(tab!=='map')setMissionMapExpanded(false);if(tab!=='replay'&&replayMap)destroyReplaySurface();document.querySelectorAll('[data-mission-tab]').forEach(b=>{const active=b.dataset.missionTab===tab;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active));b.tabIndex=active?0:-1});document.querySelectorAll('[data-mission-panel]').forEach(p=>p.classList.toggle('hidden',p.dataset.missionPanel!==tab));if(missionSource?.type==='coaching')saveCoachingArchiveState(missionSource.row.id,tab);if(tab==='map'&&missionSource)renderMissionMap(missionSource)}
...
function renderMissionMap(source){
 ...
 for(const [label,points,color,dash] of [['Tracé prévu',source.planned,TRACE_PALETTE.planned,'8 6'],['Tracé Traceur',source.trace,TRACE_PALETTE.traceur,'4 7'],[source.type==='coaching'?'Parcours Conducteur':'Trace GPS',source.actual,TRACE_PALETTE.conducteur,null]]){
  ...
  group.addTo(missionMap);layers[`<i class="trace-layer-swatch" style="background:${color}" aria-hidden="true"></i>${esc(label)}`]=group;all.push(group);
 }
 const reference=referenceCandidates.map(([,points])=>Array.isArray(points)?points.filter(valid):[]).find(points=>points.length)||[];
 if(reference.length){const endpoints=L.featureGroup();L.marker([reference[0].lat,reference[0].lon],{icon:traceMarkerIcon('D')}).bindPopup('Départ').addTo(endpoints);if(reference.length>1){const last=reference.at(-1);L.marker([last.lat,last.lon],{icon:traceMarkerIcon('A')}).bindPopup('Arrivée').addTo(endpoints)}endpoints.addTo(missionMap);layers['<i class="trace-layer-swatch trace-layer-endpoints" aria-hidden="true"></i>Départ / Arrivée']=endpoints;all.push(endpoints)}
...
const EVENT_ACTIVE_WINDOW_MS=5000;
function replayEventLabel(event){const def=LIVE_MARKERS[event?.type]||{icon:'•',label:event?.type||'Événement'};return `${def.icon} ${event.label||def.label}`}
function replayEventIsActive(event,currentTime){return Number.isFinite(event?.timestamp)&&currentTime>=event.timestamp&&currentTime<=event.timestamp+EVENT_ACTIVE_WINDOW_MS}
function destroyReplaySurface(){
 replay3dUnsubscribe?.();replay3dUnsubscribe=null;replay3dPrototype?.destroy?.();replay3dPrototype=null;document.querySelector('.replay-3d-prototype')?.remove();replayUnsubscribe?.();replayUnsubscribe=null;replayPlayer?.destroy();replayPlayer=null;replayDataset=null;
 if(replayMap){try{PisteTerrainEngine.destroyMap(replaySurface.mapId)}catch{}replayMap=null}
}
...
function closeReplay3DPrototype(reason='user'){console.warn('[Replay3D] destroy',reason);replay3dUnsubscribe?.();replay3dUnsubscribe=null;replay3dPrototype?.destroy?.();replay3dPrototype=null;document.querySelector('.replay-3d-prototype')?.remove();document.querySelector('.replay-map')?.classList.remove('hidden')}
function syncReplay3DBaseLayerButtons(surface,active){surface?.querySelectorAll('[data-replay-3d-base]').forEach(button=>{const selected=button.dataset.replay3dBase===active;button.classList.toggle('active',selected);button.setAttribute('aria-pressed',String(selected))})}
async function openReplay3DPrototype(dataset){
 closeReplay3DPrototype();const surface=document.createElement('section');surface.className='replay-3d-prototype';surface.innerHTML=`<div class="replay-3d-head"><div><small class="section-kicker">PROTOTYPE 3D</small><h3>Terrain MapLibre</h3><p class="small muted" data-replay-3d-status role="status">Chargement de la 3D…</p></div><button type="button" class="secondary" data-replay-3d-close>‹ Retour 2D</button></div><div class="replay-3d-toolbar" role="group" aria-label="Fond et caméra 3D"><div class="replay-3d-baselayers" role="group" aria-label="Fond de carte 3D"><button type="button" class="active" data-replay-3d-base="classic" aria-pressed="true">Classique</button><button type="button" data-replay-3d-base="satellite" aria-pressed="false">Satellite</button></div><button type="button" class="active" data-replay-3d-follow="free">Vue libre</button>${dataset.capabilities.hasTraceur?'<button type="button" data-replay-3d-follow="traceur">Suivre Traceur</button>':''}${dataset.capabilities.hasDriver?'<button type="button" data-replay-3d-follow="driver">Suivre Conducteur</button>':''}<button type="button" data-replay-3d-fit>Vue globale</button></div><div class="replay-3d-map" data-replay-3d-map aria-label="Prototype de carte 3D"></div><p class="replay-3d-attribution">Prototype Preview · MapLibre GL JS 4.7.1 · DEM AW3D30 (JAXA) · Imagerie © Esri, Maxar, Earthstar Geographics</p>`;const panel=$(replaySurface.panelId);const mapHost=panel?.querySelector('.replay-map');mapHost?.classList.add('hidden');mapHost?.insertAdjacentElement('afterend',surface);surface.querySelector('[data-replay-3d-close]').onclick=closeReplay3DPrototype;
 try{const mod=await import('./replay-3d-prototype.mjs');replay3dPrototype=await mod.createReplay3DPrototype({container:surface.querySelector('[data-replay-3d-map]'),dataset,onStatus:message=>{surface.querySelector('[data-replay-3d-status]').textContent=message},onBaseLayerChange:active=>syncReplay3DBaseLayerButtons(surface,active),onDiagnostic:info=>{console.warn('[Replay3D] diagnostic',info);if(info?.kind==='dem')surface.querySelector('[data-replay-3d-status]').textContent='Relief temporairement indisponible';else if(info?.kind==='tile')surface.querySelector('[data-replay-3d-status]').textContent=info.sourceId==='satellite'?'Satellite indisponible · fond classique utilisé':info.sourceId==='osm'?'Fond raster temporairement indisponible':'Relief temporairement indisponible'},onError:error=>{console.warn('[Replay3D] fatal',error);surface.querySelector('[data-replay-3d-status]').textContent='3D indisponible · retour à la carte 2D';closeReplay3DPrototype('fatal:'+String(error?.phase||error?.message||'map'))}});if(!replay3dPrototype){closeReplay3DPrototype('unsupported');return}replay3dUnsubscribe=replayPlayer?.subscribe(state=>replay3dPrototype?.update(state));surface.querySelectorAll('[data-replay-3d-base]').forEach(button=>button.onclick=()=>{if(button.dataset.replay3dBase==='satellite'){syncReplay3DBaseLayerButtons(surface,'classic');surface.querySelector('[data-replay-3d-status]').textContent='Vue Satellite 3D · En cours de développement';return}syncReplay3DBaseLayerButtons(surface,replay3dPrototype?.setBaseLayer('classic')||'classic')});surface.querySelectorAll('[data-replay-3d-follow]').forEach(button=>button.onclick=()=>{replay3dPrototype?.setFollowMode(button.dataset.replay3dFollow);surface.querySelectorAll('[data-replay-3d-follow]').forEach(item=>item.classList.toggle('active',item===button))});surface.querySelector('[data-replay-3d-fit]').onclick=()=>{replay3dPrototype?.setFollowMode('free');replay3dPrototype?.fitAll();surface.querySelectorAll('[data-replay-3d-follow]').forEach(item=>item.classList.toggle('active',item.dataset.replay3dFollow==='free'))};syncReplay3DBaseLayerButtons(surface,replay3dPrototype?.getBaseLayer?.()||'classic');surface.querySelector('[data-replay-3d-status]').textContent='Prototype prêt · horloge Replay partagée';surface.scrollIntoView({behavior:'smooth',block:'start'})}catch(error){console.warn('[Replay3D] load failure',error);closeReplay3DPrototype('load:'+String(error?.message||error))}
}
function updateReplayVisual(state){
 ...
 const activeEvents=(replayDataset.events||[]).filter(event=>replayEventIsActive(event,state.currentTime));
 replayLayers?.events?.forEach((layer,index)=>layer?.setOpacity?.(activeEvents.includes(replayDataset.events[index])?1:.45));
 document.querySelectorAll(`[data-replay-event-index][data-replay-surface="${replaySurface.mapId}"]`).forEach(button=>{const event=replayDataset.events[Number(button.dataset.replayEventIndex)],active=replayEventIsActive(event,state.currentTime);button.classList.toggle('active',active);button.setAttribute('aria-current',active?'true':'false')});
 const eventPanel=$(replaySurface.prefix+'Event');if(eventPanel){eventPanel.innerHTML=activeEvents.length?`<b>ÉVÉNEMENT</b><span>${esc(replayEventLabel(activeEvents.at(-1)))}</span><small>${replayTimeLabel(activeEvents.at(-1).timestamp-(replayDataset.capabilities.startTimestamp||activeEvents.at(-1).timestamp))}</small>`:'<span class="muted">Aucun événement à cet instant.</span>'}
}
function renderReplaySurface(dataset,panelId='blackBoxReplay',back=()=>setBlackBoxTab('summary'),mapId='activityReplayMap'){
 ...
 const all=[...dataset.tracks.traceur,...dataset.tracks.driver,...dataset.tracks.planned,...(dataset.events||[]).filter(event=>Number.isFinite(Number(event.lat))&&Number.isFinite(Number(event.lon)))];
...
function activityLibraryCard(x){
 ...
 return `<article class="library-card library-dossier-card activity-open ${archived?'archived':''} ${selected?'selected':''}" data-id="${x.id}" data-type="${x._type}" data-key="${key}">
  ...
  <div class="library-card-body">
   ...
   <div class="library-actions">${replayShortcut?'<button class="primary replayShortcut" type="button">▶ Replay</button>':' '}<button class="${replayShortcut?'secondary':'primary'} openLibraryItem" type="button">Voir la piste</button><button class="library-actions-toggle" type="button" aria-expanded="false">＋ d’actions</button><div class="library-actions-panel hidden">
...
async function openReplayShortcut(type,id){const opened=await openMissionDossier(type,id);if(!opened||!missionSource)return false;setMissionTab('replay');const dataset=replaySourceDataset(missionSource);if(dataset.capabilities.replayAvailable)renderReplaySurface(dataset,'missionReplay',()=>setMissionTab('map'),'missionReplayMap');return dataset.capabilities.replayAvailable}
...
function bindLibraryCards(root){
 ...
 root.querySelectorAll('.replayShortcut').forEach(b=>b.onclick=e=>{e.stopPropagation();const c=b.closest('.activity-open');openReplayShortcut(c.dataset.type,c.dataset.id)});
...
function resetGpsUI(clear=true,{force=false}={}){
 ...
 if(gps.watch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(gps.watch);clearInterval(gps.timer);clearInterval(operationalWeatherTimer);clearInterval(operationalAgeTimer);operationalWeatherTimer=null;operationalAgeTimer=null;releaseWakeLock();
 closeFakeLock();
...
function applyV1040RoleSurface(){
 const s=activeCoachingSession;if(!s)return;const member=myCoachingMember(s),role=myCoachingRole(s),phase=coachingPhase(s),surface=coachingActiveSurfaceModel(s,phase,role),owner=isCoachingOwner(s),v1040=Number(s.workflow_version)>=2,completed=phase==='completed',informativeCompleted=completed||!!s.driver_finished_at,layingActor=isCurrentUserLayingActor(s)&&role!=='solo',hide=id=>$(id)?.classList.add('hidden'),show=id=>$(id)?.classList.remove('hidden');
