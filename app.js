...
async function submitCoachingWizard(){
 ...
 try{
  ...
  const routeId=coachingWizard.sessionType==='solo'?null:(withoutRoute?null:preparation.routeId);created=await createCoaching({validated:true,routeId,members:coachingWizardMembers(),blindMode:coachingWizard.mode,withoutRoute,onCreated:data=>{coachingWizard.createdSessionId=data?.id||null}});
...
let soloDebugCaptureBound=false;
function soloDebugPanel(show=false){let panel=$('soloDebugPanel');if(!panel){panel=document.createElement('div');panel.id='soloDebugPanel';panel.setAttribute('role','status');panel.setAttribute('aria-live','polite');Object.assign(panel.style,{position:'fixed',left:'10px',right:'10px',bottom:'calc(76px + env(safe-area-inset-bottom,0px))',zIndex:'30000',padding:'9px 10px',border:'2px solid #ffcf5a',borderRadius:'10px',background:'rgba(24,18,3,.96)',color:'#ffe8a6',font:'12px/1.35 ui-monospace,monospace',whiteSpace:'pre-wrap',pointerEvents:'none',maxHeight:'30vh',overflow:'auto'});document.body.append(panel)}panel.classList.toggle('hidden',!show);return panel}
function soloDebugStep(message){const s=activeCoachingSession;if(!isSoloCoaching(s))return;const panel=soloDebugPanel(true),rows=(panel.textContent||'').split('\n').filter(Boolean);rows.push(`${new Date().toLocaleTimeString()} ${message}`);panel.textContent=rows.slice(-8).join('\n')}
function soloDebugInteraction(event){const s=activeCoachingSession,button=$('startLayingBtn');if(!isSoloCoaching(s)||!button||coachingPhase(s)!=='preparation')return;const rect=button.getBoundingClientRect(),x=Number(event.clientX),y=Number(event.clientY),inRect=Number.isFinite(x)&&Number.isFinite(y)&&x>=rect.left&&x<=rect.right&&y>=rect.top&&y<=rect.bottom,closest=event.target?.closest?.('#startLayingBtn'),inTarget=event.target===button||button.contains(event.target);if(!inRect&&!closest&&!inTarget)return;const hit=Number.isFinite(x)&&Number.isFinite(y)?document.elementFromPoint(x,y):null;soloDebugStep(`${event.type.toUpperCase()} target=${event.target?.tagName||'?'}#${event.target?.id||''} closest=${closest?'oui':'non'} role=${myCoachingMember(s)?.role||myCoachingRole(s)} phase=${coachingPhase(s)} session=${s.id} disabled=${button.disabled} aria-disabled=${button.getAttribute('aria-disabled')} pointer=${getComputedStyle(event.target).pointerEvents} hit=${hit?.tagName||'?'}#${hit?.id||''}.${hit?.className||''}`)}
function setupSoloDebugCapture(){if(soloDebugCaptureBound)return;soloDebugCaptureBound=true;document.addEventListener('pointerdown',soloDebugInteraction,true);document.addEventListener('click',soloDebugInteraction,true)}
...
async function startSoloRun(){const s=activeCoachingSession;if(!s||!isSoloCoaching(s)||coachingPhase(s)!=='preparation')return false;soloDebugStep('START SOLO');const button=$('startLayingBtn');if(button){button.disabled=true;button.setAttribute('aria-busy','true')}try{soloDebugStep('RPC START');const {data,error}=await supabase.rpc('start_solo_run',{p_session_id:s.id});if(error){soloDebugStep(`RPC ERROR : ${error.message}`);throw error}soloDebugStep('RPC OK');await refreshActiveCoachingSession(s.id);if(!activeCoachingSession||coachingPhase(activeCoachingSession)!=='driver_running'){coachingToast('Le départ Solo n’a pas été confirmé par le serveur.');return false}updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await startCoachingGpsTracking();await renderCoachingMap();refreshCoachingMapLayout();return !!data}catch(error){coachingToast(`Départ impossible : ${error?.message||'erreur serveur'}`);return false}finally{if(button){button.disabled=false;button.removeAttribute('aria-busy')}}}
