...
function coachingDriverTrackPending(s=activeCoachingSession){
 if(!s||coachingPhase(s)!=='completed'||myCoachingRole(s)!=='driver')return false;
...
async function openCoachingDebriefOnce(id,statusText){
 ...
 const operation=(async()=>{
  ...
  if(globalPhase==='track_finished'){
   let data=null,error=null;try{({data,error}=await supabase.rpc('begin_coaching_debrief_v1049',{p_session_id:id}))}catch(cause){error=cause}
   try{await refreshActiveCoachingSession(id)}catch(cause){error=error||cause}
   if(activeCoachingSession?.id!==id)return false;globalPhase=coachingGlobalPhase(activeCoachingSession);
   if(!error&&data===true&&globalPhase==='track_finished'){activeCoachingSession.debrief_status='in_progress';globalPhase='debrief'}
   if(error&&globalPhase==='track_finished')coachingToast(`Initialisation du débrief en attente : ${error.message}`);
  }
