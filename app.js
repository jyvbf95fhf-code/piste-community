...
function coachingActiveSurfaceModel(s,phase=coachingPhase(s),role=myCoachingRole(s)){
  const rawRole=role,displayRole=rawRole==='solo'?'driver':rawRole,knownRole=['driver','traceur','observer','coach','solo'].includes(rawRole),active=!!s&&s.status==='live'&&phase!=='completed';
  ...
  if(!active)return {statusLabel:'Session indisponible',visibleBlocks:[],actions:[],mapPriority:false,odorVisible:false,capabilities,visibility};
  if(!knownRole)return {statusLabel:'Session en cours',visibleBlocks:['compactStatus','weather','map','commandBar','navigation'],actions:[],mapPriority:true,odorVisible:false,capabilities,visibility};
...
function applyCoachingActiveSurface(model){
 ...
 const actionNames=['pause','blackScreen','messages','plus','startDriver','startLaying','finishLaying','trackReady','finish'],actionTargets=Object.fromEntries(actionNames.map(action=>[action,document.querySelector(coachingSurfaceActionTarget(action,activeCoachingSession))]));
