...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingAutoMetrics=null;const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
 ...
 $('coachingLivePanel').classList.toggle('waiting-room',s.status==='waiting');$('startCoachingLive').textContent=coachingPrimaryLabel(role,s.status,owner);$('startCoachingLive').classList.toggle('hidden',s.status==='waiting'?!owner:role==='observer'||s.status==='ended');$('startCoachingLive').disabled=s.status==='waiting'&&!owner;$('coachAnnotationTools').classList.toggle('hidden',!['coach','driver','solo'].includes(role)||s.status==='ended');$('traceurTools').classList.toggle('hidden',role!=='traceur'||s.status==='ended');$('calculateCoachingDebrief').classList.toggle('hidden',s.status==='waiting');$('coachingDebriefForm').classList.toggle('hidden',s.status==='waiting'||!['coach','driver','solo'].includes(role));$('coachingAutoDebrief').classList.add('hidden');
 $('endCoachingLive').classList.toggle('hidden',!owner||s.status!=='live');$('cancelCoachingSession').classList.toggle('hidden',!owner||s.status!=='waiting');$('deleteCoachingSession').classList.toggle('hidden',!owner);$('leaveCoachingSession').classList.toggle('hidden',owner||!['waiting','live'].includes(s.status));
...
async function startActiveCoaching(){
 ...
 $('startCoachingLive').textContent=coachingPrimaryLabel(role,'live',true);$('endCoachingLive').classList.toggle('hidden',!isCoachingOwner(activeCoachingSession));
}
...
async function finishActiveCoaching(){const s=activeCoachingSession;if(!s||!isCoachingOwner(s)){alert('Seul l’organisateur peut terminer cette session.');return}if(s.status!=='live'){alert('Cette session n’est pas active.');return}if(!confirm('Terminer cette session pour toute l’équipe ? Elle restera consultable dans les sessions terminées.'))return;const id=s.id,endedAt=new Date().toISOString(),{data,error}=await supabase.from('coaching_sessions').update({status:'ended',ended_at:endedAt}).eq('id',id).eq('owner_id',session.user.id).eq('status','live').select('id').maybeSingle();if(error||!data){alert('Fin impossible : '+(error?.message||'la session a déjà changé d’état.'));return}clearVerifiedActiveCoaching(id);await returnToCoachingSessions('ended')}
...
async function saveCoachingDebrief(e){e.preventDefault();const s=activeCoachingSession;if(!s)return;const f=new FormData(e.target),{data:allPoints=[]}=await supabase.from('coaching_live_points').select('owner_id,lat,lon,accuracy_m,recorded_at').eq('session_id',s.id).order('recorded_at'),points=allPoints.filter(p=>['driver','solo'].includes(coachingMemberRole(p.owner_id)));if(!coachingAutoMetrics)await calculateCoachingDebrief();const payload={session_id:s.id,owner_id:s.owner_id,coach_id:myCoachingRole(s)==='driver'?null:session.user.id,strengths:f.get('strengths')||null,improvement_area:f.get('improvement_area')||null,coach_notes:f.get('coach_notes')||null,actual_track:points,auto_metrics:coachingAutoMetrics||{},updated_at:new Date().toISOString()},{error}=await supabase.from('coaching_debriefs').upsert(payload);if(error){alert(error.message);return}if(!isCoachingOwner(s)){alert('Débrief enregistré. Seul l’organisateur peut terminer la session.');return}if(s.status!=='live'){alert('Débrief enregistré. La session était déjà terminée.');return}const endedAt=new Date().toISOString(),{data:ended,error:endError}=await supabase.from('coaching_sessions').update({status:'ended',ended_at:endedAt}).eq('id',s.id).eq('owner_id',session.user.id).eq('status','live').select('id').maybeSingle();if(endError||!ended){alert('Débrief enregistré, mais clôture impossible : '+(endError?.message||'la session a changé d’état.'));return}clearVerifiedActiveCoaching(s.id);alert('Débrief enregistré. Les tracés, événements et mesures automatiques sont conservés.');await returnToCoachingSessions('ended')}
