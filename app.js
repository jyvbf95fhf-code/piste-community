...
async function fetchCoachingLiveWeather(){
 ...
 try{
  ...
  coachingLiveWeather=fresh;try{localStorage.setItem(key,JSON.stringify(fresh))}catch{}renderCoachingLiveWeather();if(coachingMap)renderCoachingMap();return true
 }catch(error){
  if(!coachingWeatherRequestMatches(requestId,requestedSessionId,coachingWeatherRequestId,activeCoachingSession?.id||null))return false;
