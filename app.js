...
async function ensureCoachingStarted(){if(!activeCoachingSession)return false;if(activeCoachingSession.status==='live')return true;const {error}=await supabase.rpc('start_coaching_session',{p_session_id:activeCoachingSession.id});if(error){alert('Départ impossible : '+error.message);return false}activeCoachingSession.status='live';activeCoachingSession.started_at=new Date().toISOString();verifiedActiveCoachingSession=activeCoachingSession;coachingShortcutValidated=true;saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();$('coachingLiveStatus').textContent='En direct • positions GPS synchronisées';return true}
