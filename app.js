...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingSessionEndHandledId=null;coachingAutoMetrics=null;coachingDebriefData=null;coachingGpsError='';coachingGpsReady=false;if(Number(s.workflow_version)>=2){const refreshed=await refreshActiveCoachingSession(s.id);if(refreshed)s=activeCoachingSession}const globalPhase=coachingGlobalPhase(s);if(globalPhase==='active')requestCoachingPreviewLocation();$('coachingDebriefForm')?.reset();$('coachingDriverFeedbackForm')?.reset();setUiText('coachingDebriefState','Aucun brouillon chargé.');setUiText('coachingDriverFeedbackState','Aucun retour enregistré.');const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'&&globalPhase==='active'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
...
function updateCoachingDebriefAccess(){const s=activeCoachingSession,ready=!!s&&(s.status==='ended'||coachingPhase(s)==='completed')&&!coachingDriverTrackPending(s),coach=ready&&((s.visibility_version!==3&&isCoachingOwner(s))||hasCoachingCapability('coach',s)),driver=ready&&hasCoachingCapability('drive',s);$('calculateCoachingDebrief')?.classList.toggle('hidden',!ready);$('coachingDebriefForm')?.classList.toggle('hidden',!coach);$('coachingDriverFeedbackForm')?.classList.toggle('hidden',!driver)}
