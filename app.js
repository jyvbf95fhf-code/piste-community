...
async function handleCoachingSessionChange(payload){if(!activeCoachingSession||payload.new?.id!==activeCoachingSession.id)return;Object.assign(activeCoachingSession,payload.new);if(['ended','cancelled'].includes(payload.new.status)){stopCoachingPresence();stopTraceurTracking();closeFakeLock();clearVerifiedActiveCoaching(payload.new.id);updateCoachingPhase();setCoachingStage('debrief');await calculateCoachingDebrief();setUiText('coachingLiveStatus',payload.new.status==='ended'?'Session terminée par l’organisateur • débrief disponible':'Session annulée par l’organisateur')}else if(payload.new.status==='live'){verifiedActiveCoachingSession=activeCoachingSession;saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();updateCoachingPhase();setCoachingStage('live')}}
...
async function openCoachingSession(id,{resume=false}={}){
 ...
 updateCoachingPreflight();updateCoachingPhase();updateCoachingDeparture();updateCoachingPreparationDetails();setCoachingPanel('team');setCoachingStage(s.status==='ended'?'debrief':s.status==='waiting'?'room':'live');if(s.status==='live')startCoachingPresence();await renderCoachingMap();await loadCoachingMessages();if(s.status==='ended'){await calculateCoachingDebrief();await loadSavedCoachingDebrief()}
...
async function startActiveCoaching(){
 ...
 if(role==='driver'||role==='solo'){$('startCoachingLive').textContent='Suivi GPS actif';$('startCoachingLive').disabled=true;setCoachingPanel('team');return}
