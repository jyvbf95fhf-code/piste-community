...
let coachingSessionEndHandledId=null,coachingV1040FinishInFlight=false,coachingV10423TransitionInFlight=false,coachingDebriefOpenPromises=new Map();
...
async function openCoachingDebriefOnce(id,statusText){
 ...
 const operation=(async()=>{
  ...
  if(coachingSessionEndHandledId===id){setCoachingStage('debrief');return true}
...
function updateCoachingDebriefAccess(){const s=activeCoachingSession,ready=!!s&&(s.status==='ended'||coachingPhase(s)==='completed')&&!coachingDriverTrackPending(s),personal=ready&&['in_progress','closed'].includes(s.debrief_status),coach=ready&&!personal&&((s.visibility_version!==3&&isCoachingOwner(s))||hasCoachingCapability('coach',s)),driver=ready&&!personal&&hasCoachingCapability('drive',s);$('calculateCoachingDebrief')?.classList.toggle('hidden',!ready);$('coachingParticipantObservations')?.classList.toggle('hidden',!personal);$('coachingDebriefForm')?.classList.toggle('hidden',!coach);$('coachingDriverFeedbackForm')?.classList.toggle('hidden',!driver)}
