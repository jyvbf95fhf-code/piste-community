...
function finishHoldStart(e){if(e?.type==='keydown'&&(!['Enter',' '].includes(e.key)||e.repeat))return;if(e?.button!=null&&e.button!==0)return;if(e){e.preventDefault();e.stopPropagation()}const requested=e?.currentTarget?.id||'driverFinishBtn',driver=requested==='driverFinishBtn',b=$(requested),s=activeCoachingSession,v2=Number(s?.workflow_version)>=2,canFinish=driver?coachingCanFinishTrack(s):!!s&&s.status==='live'&&isCoachingOwner(s)&&(!v2||coachingPhase(s)==='completed');if(!b||!canFinish||coachingFinishArmed||driver&&(coachingFinishSubmittingSessionId===s.id||coachingFinishSubmittedSessionId===s.id))return;coachingFinishArmed=true;coachingFinishSessionId=s.id;coachingFinishTargetId=requested;b.classList.add('holding');b.style.setProperty('--finish-progress','0%');const started=Date.now();coachingFinishTimer=setInterval(()=>{const current=activeCoachingSession,valid=current?.id===coachingFinishSessionId&&(driver?coachingCanFinishTrack(current):current.status==='live'&&isCoachingOwner(current)&&(!v2||coachingPhase(current)==='completed'));if(!valid){finishHoldCancel();return}const elapsed=Date.now()-started,pct=Math.min(100,(elapsed/2000)*100);b.style.setProperty('--finish-progress',`${pct}%`);if(elapsed>=2000){const id=coachingFinishSessionId;finishHoldCancel();if(driver){coachingFinishSubmittedSessionId=id;submitCoachingFinishHold(id)}else if(v2)finishCoachingSessionV1040();else finishActiveCoaching()}},50)}
function finishHoldCancel(e){if(e){e.preventDefault();e.stopPropagation()}const target=coachingFinishTargetId||e?.currentTarget?.id;clearInterval(coachingFinishTimer);coachingFinishTimer=null;coachingFinishArmed=false;coachingFinishSessionId=null;coachingFinishTargetId=null;const b=target?$(target):null;b?.classList.remove('holding');b?.style.removeProperty('--finish-progress')}
...
function setupCoachingFinishHold(){
 ...
 for(const event of ['pointerup','pointercancel','pointerleave','keyup','blur'])driverFinishBtn.addEventListener(event,finishHoldCancel);
