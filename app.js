...
function finishHoldStart(e){if(e){e.preventDefault();e.stopPropagation()}const b=$('terrainFinishBtn'),s=activeCoachingSession,v2=Number(s?.workflow_version)>=2,canFinish=!!s&&s.status==='live'&&isCoachingOwner(s)&&(!v2||coachingPhase(s)==='completed');if(!b||!canFinish||coachingFinishArmed)return;coachingFinishArmed=true;b.classList.add('holding');b.style.setProperty('--finish-progress','0%');const started=Date.now();coachingFinishTimer=setInterval(()=>{const pct=Math.min(100,((Date.now()-started)/2000)*100);b.style.setProperty('--finish-progress',`${pct}%`);if(pct>=100){finishHoldCancel();if(v2)finishCoachingSessionV1040();else finishActiveCoaching()}},50)}
...
async function finishDriverRun(){const id=activeCoachingSession?.id;const ok=await coachingTransitionV1040('finish_driver_run');if(ok&&activeCoachingSession?.id===id)await openCoachingDebriefOnce(id,'Parcours terminé • Débrief disponible');return ok}
