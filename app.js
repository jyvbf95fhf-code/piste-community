...
function coachingDriverTrail(groups){const started=activeCoachingSession?.started_at?new Date(activeCoachingSession.started_at).getTime():0,role=myCoachingRole(activeCoachingSession),ownOnly=role==='driver'&&!isSoloCoaching(activeCoachingSession)&&activeCoachingSession?.visibility_mode!=='all';return[...groups.entries()].filter(([userId])=>['driver','solo'].includes(coachingMemberRole(userId))&&(!ownOnly||userId===session.user.id)).flatMap(([,rows])=>rows).filter(point=>!started||new Date(point.recorded_at).getTime()>=started).sort((a,b)=>new Date(a.recorded_at)-new Date(b.recorded_at))}
...
let coachingV1040HoldTimer=null;
let coachingV1040HoldProgressTimer=null;
let coachingV1040HoldButton=null;
let coachingV1040HoldState=null;
let coachingV1040LastHoldStart=0;
...
function coachingPhaseLabelV1040(s){const labels={preparation:'Préparation',laying:'Pose en cours',laid:'Pose terminée',waiting_ready:'En attente du Coach',coach_ready:'Coach en place',driver_running:'Conducteur en piste',completed:'Terminée'};return labels[coachingPhase(s)]||'Préparation'}
function coachingDbVisibility(s,role){
  const mode=s?.visibility_mode||'all',r=role||myCoachingRole(s);
  if(mode==='coach'||mode==='full_blind')return r==='traceur'||(r==='coach'&&s?.laying_mode==='coach')?'visible':'hidden';
  if(mode==='progressive'||mode==='simple_blind'){if(r==='driver')return 'hidden';if(r==='observer'){const key=`piste-observer-track-${s?.id||'session'}`;return localStorage.getItem(key)==='true'?'visible':'hidden'}return 'visible'}
  return 'visible';
}
...
async function coachingTransitionV1040(rpc){const s=activeCoachingSession;if(!s||!session?.user){coachingHoldDebug(`Action refusée : session/utilisateur absent · ${coachingHoldContext()}`);return false}if(rpc==='start_coaching_laying')coachingHoldDebug(`RPC start_coaching_laying envoyée · ${coachingHoldContext()}`);const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){const message=error.message||String(error);coachingHoldDebug(`RPC erreur : ${message} · ${coachingHoldContext()}`);coachingToast(`Action impossible : ${message}`);return false}if(rpc==='start_coaching_laying')coachingHoldDebug(`RPC réussie · data=${JSON.stringify(data)} · ${coachingHoldContext()}`);const refreshed=await refreshActiveCoachingSession(s.id);if(rpc==='start_coaching_laying')coachingHoldDebug(`${refreshed?'Session rechargée':'Rechargement impossible'} · ${coachingHoldContext()}`);updateCoachingPhase();applyV1040RoleSurface();return !!data||!error}
async function beginCoachingLaying(){const s=activeCoachingSession;coachingHoldDebug(`beginCoachingLaying appelée · ${coachingHoldContext()} · busy=${coachingV1040ActionBusy}`);if(coachingV1040ActionBusy){coachingHoldDebug('Action refusée : action déjà en cours');return false}if(!s){coachingToast('Session Coaching non chargée. Actualisez la session avant de commencer la pose.');coachingHoldDebug('Action refusée : activeCoachingSession absente');return false}if(Number(s.workflow_version)<2||coachingPhase(s)!=='preparation'){coachingToast('La pose ne peut commencer que depuis une session V10.40 en préparation.');coachingHoldDebug(`Action refusée : workflow/phase incompatibles · ${coachingHoldContext()}`);applyV1040RoleSurface();return false}coachingV1040ActionBusy=true;try{const ok=await coachingTransitionV1040('start_coaching_laying');if(ok&&activeCoachingSession){coachingHoldDebug(`Phase : ${coachingPhase(activeCoachingSession)} · laying_started_at=${activeCoachingSession.laying_started_at??'absent'}`);coachingHoldDebug(`GPS pose demandé · ${coachingHoldContext()}`);await startTraceurTracking();if(coachingGpsError)coachingHoldDebug(`GPS pose erreur : ${coachingGpsError}`)}return ok}finally{coachingV1040ActionBusy=false}}
async function finishCoachingLaying(){if(coachingV1040ActionBusy)return false;coachingV1040ActionBusy=true;try{const ok=await coachingTransitionV1040('finish_coaching_laying');if(ok)stopTraceurTracking();return ok}finally{coachingV1040ActionBusy=false}}
...
function coachingHoldDebug(message){const el=$('coachingHoldDebug');if(el)el.textContent=message}
function coachingHoldContext(){const s=activeCoachingSession;return s?`id=${s.id} · workflow=${s.workflow_version??'absent'} · phase=${coachingPhase(s)} · rôle=${myCoachingRole(s)} · laying=${s.laying_mode??'absent'}`:'session absente'}
function layingHoldCancel(reason='Maintien annulé'){if(coachingV1040HoldTimer){clearTimeout(coachingV1040HoldTimer);coachingV1040HoldTimer=null}if(coachingV1040HoldProgressTimer){clearInterval(coachingV1040HoldProgressTimer);coachingV1040HoldProgressTimer=null}if(coachingV1040HoldState)coachingHoldDebug(`${reason} · ${Math.round(performance.now()-coachingV1040HoldState.startedAt)} ms · ${coachingHoldContext()}`);coachingV1040HoldState=null;if(coachingV1040HoldButton){coachingV1040HoldButton.classList.remove('holding');coachingV1040HoldButton=null}}
function layingHoldStart(button,action,event=null){layingHoldCancel();coachingV1040HoldButton=button;coachingV1040HoldState={startedAt:performance.now(),pointerId:event?.pointerId??null,startX:event?.clientX??event?.touches?.[0]?.clientX??null,startY:event?.clientY??event?.touches?.[0]?.clientY??null};button.classList.add('holding');coachingHoldDebug(`Appui détecté · ${coachingHoldContext()}`);coachingV1040HoldProgressTimer=setInterval(()=>{if(coachingV1040HoldState)coachingHoldDebug(`Maintien en cours : ${Math.round(performance.now()-coachingV1040HoldState.startedAt)} ms · ${coachingHoldContext()}`)},100);coachingV1040HoldTimer=setTimeout(()=>{const elapsed=Math.round(performance.now()-coachingV1040HoldState.startedAt);if(coachingV1040HoldProgressTimer){clearInterval(coachingV1040HoldProgressTimer);coachingV1040HoldProgressTimer=null}coachingV1040HoldTimer=null;button.classList.remove('holding');coachingHoldDebug(`2 secondes atteintes (${elapsed} ms) · ${coachingHoldContext()}`);coachingV1040HoldState=null;coachingV1040HoldButton=null;Promise.resolve().then(()=>{coachingHoldDebug(`beginCoachingLaying appelée · ${coachingHoldContext()}`);return action()}).catch(error=>coachingHoldDebug(`Erreur action : ${error.message||error}`))},2000)}
function bindV1040Hold(id,action){const button=$(id);if(!button||button.dataset.v1040HoldBound==='true')return;button.dataset.v1040HoldBound='true';const start=event=>{if(button.disabled||button.classList.contains('hidden'))return;const now=Date.now();if(now-coachingV1040LastHoldStart<80)return;coachingV1040LastHoldStart=now;event.preventDefault();if(event.pointerId!==undefined){try{button.setPointerCapture?.(event.pointerId)}catch{}}layingHoldStart(button,action,event)};const move=event=>{if(!coachingV1040HoldState)return;const x=event.clientX??event.touches?.[0]?.clientX,y=event.clientY??event.touches?.[0]?.clientY,dx=Number.isFinite(x)&&coachingV1040HoldState.startX!==null?x-coachingV1040HoldState.startX:0,dy=Number.isFinite(y)&&coachingV1040HoldState.startY!==null?y-coachingV1040HoldState.startY:0;if(Math.hypot(dx,dy)>20)layingHoldCancel('Maintien annulé : déplacement > 20 px')};const end=()=>layingHoldCancel('Maintien annulé : relâchement');['pointerdown','touchstart'].forEach(type=>button.addEventListener(type,start,{passive:false}));['pointerup','pointercancel','touchend','touchcancel'].forEach(type=>button.addEventListener(type,end,{passive:false}));['pointermove','pointerleave','touchmove'].forEach(type=>button.addEventListener(type,move,{passive:false}));}
...
function applyV1040RoleSurface(){const s=activeCoachingSession;if(!s)return;const role=coachingGpsRole(s),phase=coachingPhase(s),owner=isCoachingOwner(s),hide=id=>$(id)?.classList.add('hidden'),show=id=>$(id)?.classList.remove('hidden');['startCoachingLive','toggleCoachingGps','coachingFakeLockBtn','endCoachingLive','terrainFinishBtn','cancelCoachingWaiting','deleteCoachingWaiting','cancelCoachingSession','deleteCoachingSession','startLayingBtn','finishLayingBtn','coachReadyBtn','driverStartBtn','driverFinishBtn'].forEach(hide);show('terrainBlackScreenBtn');if(role==='observer'){hide('coachingLayerControls');show('observerTrackChoice');hide('leaveCoachingSession')}else if(role==='traceur'){if(phase==='preparation')show('startLayingBtn');if(phase==='laying')show('finishLayingBtn')}else if(role==='coach'&&s.laying_mode==='coach'){if(phase==='preparation')show('startLayingBtn');if(phase==='laying')show('finishLayingBtn');if(['laid','waiting_ready'].includes(phase))show('coachReadyBtn')}else if(role==='coach'&&['laid','waiting_ready'].includes(phase))show('coachReadyBtn');else if(role==='driver'&&phase==='coach_ready')show('driverStartBtn');else if(role==='driver'&&phase==='driver_running')show('driverFinishBtn');if(owner&&phase==='driver_running')show('terrainFinishBtn');if(owner&&phase==='waiting'){show('cancelCoachingWaiting');show('deleteCoachingWaiting')}setUiText('coachingRoleInstruction',role==='observer'?'Vous observez la session sans commande de pilotage.':role==='driver'&&phase!=='driver_running'?'En attente du Coach.':`Rôle : ${coachingRoleLabel(role)} · ${coachingPhaseLabelV1040(s)}`);renderCoachingScenario()}
