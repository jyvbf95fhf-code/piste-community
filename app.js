...
async function finishCoachingLaying(){const ok=await coachingTransitionV1040('finish_coaching_laying');if(ok)stopTraceurTracking();return ok}
...
function layingHoldCancel(){if(coachingV1040HoldTimer){clearTimeout(coachingV1040HoldTimer);coachingV1040HoldTimer=null}if(coachingV1040HoldButton){coachingV1040HoldButton.classList.remove('holding');coachingV1040HoldButton=null}}
function layingHoldStart(button,action){layingHoldCancel();coachingV1040HoldButton=button;button.classList.add('holding');coachingV1040HoldTimer=setTimeout(()=>{coachingV1040HoldTimer=null;button.classList.remove('holding');Promise.resolve(action()).catch(error=>coachingToast(error.message||'Action impossible'))},2000)}
function bindV1040Hold(id,action){const button=$(id);if(!button||button.dataset.v1040HoldBound==='true')return;button.dataset.v1040HoldBound='true';const start=event=>{if(button.disabled||button.classList.contains('hidden'))return;const now=Date.now();if(now-coachingV1040LastHoldStart<80)return;coachingV1040LastHoldStart=now;event.preventDefault();layingHoldStart(button,action)};['pointerdown','touchstart'].forEach(type=>button.addEventListener(type,start,{passive:false}));['pointerup','pointercancel','pointerleave','touchend','touchcancel','touchmove'].forEach(type=>button.addEventListener(type,layingHoldCancel,{passive:false}));}
