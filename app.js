...
function updateCoachingWorkflowNotice(s,role){
 ...
 else if(role==='driver'&&['waiting_ready','coach_ready','laid'].includes(phase))el.textContent='Piste tracée — vous pouvez démarrer au point de départ';
...
async function createCoaching(){
 ...
 if(!route||route.route?.length<2){setUiText('coachingCreateMsg','Choisissez un tracé préparé comportant au moins deux points.');return}
 coachingCreateInFlight=true;const button=$('createCoachingSession');if(button)button.disabled=true;
 try{
  const {data,error}=await supabase.rpc('create_coaching_people_session',{p_route_id:route.id,p_members:coachingCreationMembers(),p_blind_mode:$('coachingVisibility')?.value||'normal'});
...
async function refreshActiveCoachingSession(id){const {data,error}=await supabase.rpc('get_my_coaching_sessions',{p_session_id:id});const fresh=!error&&Array.isArray(data)?data[0]:null;if(!fresh||activeCoachingSession?.id!==id)return false;activeCoachingSession={...activeCoachingSession,...fresh};markCoachingSyncV10423();const index=coachingSessions.findIndex(x=>x.id===id);if(index>=0)coachingSessions[index]=activeCoachingSession;else coachingSessions.push(activeCoachingSession);if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();return true}
...
function shareCoachingCurrentPosition(id,position){
 const s=activeCoachingSession;if(s?.id!==id||s.visibility_version!==3||myCoachingRole(s)==='observer'||coachingPhase(s)==='completed'||Date.now()-coachingPreviewSharedAt<5000)return;
...
async function requestCoachingPreviewLocation(){
 if(!navigator.geolocation||coachingPreviewWatch!==null||!activeCoachingSession||myCoachingRole(activeCoachingSession)==='observer')return false;
...
function missionTimeline(source){
 ...
 for(const [key,title] of [['created_at','Création'],['laying_started_at','Début du traçage'],['track_finished_at','Piste tracée'],['coach_ready_at','Piste prête'],['driver_started_at','Départ Conducteur'],['driver_finished_at','Fin de parcours'],['ended_at','Clôture de la session']])add(r[key],title);
...
function missionAge(source){const r=source.row,start=Date.parse(r.driver_started_at||source.actual[0]?.recorded_at),laid=Date.parse(r.laying_started_at||source.trace[0]?.recorded_at);if(Number.isFinite(start)&&Number.isFinite(laid)&&start>=laid)return formatExactDuration(start-laid);return hasValue(r.delai_h)?`${r.delai_h} h`:null}
...
function renderMissionDossier(source){
 ...
 const timeline=missionTimeline(source);$('missionTimeline').innerHTML='<h3>CHRONOLOGIE</h3>'+(timeline.length?'<ol class="mission-timeline">'+timeline.map(e=>`<li><time>${esc(dateTimeFr(e.at))}</time><b>${esc(e.title)}</b>${e.detail?`<p>${esc(e.detail)}</p>`:''}</li>`).join('')+'</ol>':'<p>Aucun événement horodaté disponible.</p>');
...
async function coachingTransitionV1040(rpc){if(coachingV10423TransitionInFlight)return false;coachingV10423TransitionInFlight=true;try{const s=activeCoachingSession;if(!s||!session?.user)return false;const allowed={start_coaching_laying:['preparation'],mark_coaching_track_ready:['laying'],start_driver_run:['waiting_ready','coach_ready'],finish_driver_run:['driver_running']};if(allowed[rpc]&&!allowed[rpc].includes(coachingPhase(s)))return false;const {data,error}=await supabase.rpc(rpc,{p_session_id:s.id});if(error){coachingToast(`Action impossible : ${error.message}`);return false}if(data?.id===s.id){for(const key of ['status','phase','laying_started_at','track_finished_at','driver_started_at','driver_finished_at'])if(Object.prototype.hasOwnProperty.call(data,key))s[key]=data[key]}await refreshActiveCoachingSession(s.id);updateCoachingPhase();updateCoachingPreparationDetails();updateCoachingPrimaryActions();applyV1040RoleSurface();await renderCoachingMap();return !!data||!error}finally{coachingV10423TransitionInFlight=false}}
