...
let coachingLayerVisibility={planned:true,trace:true,actual:true,odor:true,markers:true},plannerDraftTimer=null,plannerContext='training',plannerPendingLatLng=null,plannerTouchTimer=null,plannerTouchOrigin=null;
...
function openPlannerLongPressMenu(latlng){plannerPendingLatLng=latlng;$('plannerLongPressMenu')?.classList.remove('hidden')}
...
function setPlannerContext(context='training'){plannerContext=['training','coaching','operational'].includes(context)?context:'training';const labels={training:['Préparer un entraînement','Enregistre le tracé puis démarre avec ton chien.'],coaching:['Préparer une session Coaching','Le tracé sera sélectionné au retour dans la préparation Coaching.'],operational:['Préparer une piste OPS','Le tracé préparé restera affiché pendant la session OPS.']},copy=labels[plannerContext];setUiText('plannerContextTitle',copy[0]);setUiText('plannerContextHelp',copy[1]);document.querySelectorAll('[data-planner-context]').forEach(button=>button.classList.toggle('active',button.dataset.plannerContext===plannerContext));setUiText('saveAndStartRoute',plannerContext==='coaching'?'💾 Enregistrer et revenir au Coaching':'▶ Démarrer avec mon chien');savePlannerDraft()}
...
$('plannerFreeMode').onclick=()=>{setPlannerRoutingMode('free');setPlannerSection('map');$('plannerAdvancedArea').classList.add('hidden');$('plannerAdvancedToggle').setAttribute('aria-expanded','false')};
$('plannerRouteMode').onclick=()=>{setPlannerRoutingMode('trail');setPlannerSection('map');$('plannerAdvancedArea').classList.add('hidden');$('plannerAdvancedToggle').setAttribute('aria-expanded','false')};
