...
function setCoachingDebriefStep(step='summary'){
 const allowed=['summary','map','observations','complete'],next=allowed.includes(step)?step:'summary';coachingDebriefStep=next;setCoachingDebriefMode(true);
...
function updateCoachingDebriefAccess(){
 const s=activeCoachingSession,access=coachingHistoricalAccess(s),ready=!!s&&(s.status==='ended'||coachingPhase(s)==='completed')&&!coachingDriverTrackPending(s),personal=ready&&access.canReadDebrief,closed=personal&&s.debrief_status==='closed',closer=canCloseCoachingDebrief(s),coach=ready&&!personal&&((s.visibility_version!==3&&isCoachingOwner(s))||hasCoachingCapability('coach',s)),driver=ready&&!personal&&hasCoachingCapability('drive',s),closure=$('coachingDebriefClosure'),closeButton=$('closeCoachingDebriefBtn');
 $('calculateCoachingDebrief')?.classList.toggle('hidden',!ready);$('coachingParticipantObservations')?.classList.toggle('hidden',!personal);$('coachingDebriefForm')?.classList.toggle('hidden',!coach);$('coachingDriverFeedbackForm')?.classList.toggle('hidden',!driver);closure?.classList.toggle('hidden',!personal);closeButton?.classList.toggle('hidden',!closer);if(closeButton)closeButton.disabled=!closer;setUiText('coachingDebriefPhaseLabel',closed?'DÉBRIEF CLÔTURÉ':personal?'DÉBRIEF EN COURS':'PISTE TERMINÉE');setUiText('coachingDebriefClosureState',closed?'Débrief clôturé. La carte, les statistiques et les observations restent consultables ; votre observation personnelle reste modifiable.':closer?'Vous pouvez clôturer le débrief, même si votre observation personnelle est vide.':'Le Conducteur ou le Coach peut clôturer le débrief. Vous pouvez enregistrer ou modifier votre observation personnelle.');
}
