...
async function renderCoachingMap(options){
 ...
 if(coachingGlobalPhase(activeCoachingSession)!=='active'){
  ...
  const newMap=!coachingMap;if(newMap){coachingMap=PISTE_TERRAIN_ENGINE_MODE==='legacy'?createPisteMap('coachingMap').setView([48.3,7.45],9):PisteTerrainEngine.createMap('coachingMap').setView([48.3,7.45],9);if(PISTE_TERRAIN_ENGINE_MODE==='legacy')addCleanBaseLayers(coachingMap);ensureCoachingParticipantPane();coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true;cancelCoachingLongPress()})}
 ...
 if(PISTE_TERRAIN_ENGINE_MODE!=='legacy')PisteTerrainEngine.clearLayer('coachingMap','coaching-layers');coachingLayers.forEach(layer=>{try{layer.remove()}catch{}});coachingLayers=[];const newMap=!coachingMap;if(newMap){coachingMap=PISTE_TERRAIN_ENGINE_MODE==='legacy'?createPisteMap('coachingMap').setView([48.3,7.45],9):PisteTerrainEngine.createMap('coachingMap').setView([48.3,7.45],9);if(PISTE_TERRAIN_ENGINE_MODE==='legacy')addCleanBaseLayers(coachingMap);ensureCoachingParticipantPane();coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true;cancelCoachingLongPress()});coachingMap.on('rotate rotateend zoomend',updateDriverMarkerOrientations);installCoachingLongPress()}
...
function initOperationalCallMap(){if(operationalCallMap){renderOperationalCallMap();return}operationalCallMap=PISTE_TERRAIN_ENGINE_MODE==='legacy'?createPisteMap('operationalCallMap',{zoomControl:true}).setView([48.3,7.45],9):PisteTerrainEngine.createMap('operationalCallMap',{zoomControl:true}).setView([48.3,7.45],9);if(PISTE_TERRAIN_ENGINE_MODE==='legacy')addCleanBaseLayers(operationalCallMap);operationalCallMap.on('click',e=>{if(!operationalCallPoint){operationalCallPoint={lat:e.latlng.lat,lon:e.latlng.lng,label:'Dernier point connu'};$('callLocationStatus').textContent='Dernier point connu positionné. Touche encore la carte pour ajouter le repère sélectionné.'}else{const type=$('callMarkerType').value,note=$('callMarkerNote').value.trim(),def=CALL_MARKERS[type]||CALL_MARKERS.note;operationalCallMarkers.push({id:crypto.randomUUID?.()||String(Date.now()),type,lat:e.latlng.lat,lon:e.latlng.lng,note:note||def.label});$('callMarkerNote').value=''}renderOperationalCallMap();renderCallMarkerList();renderOperationalCallSummary()});renderOperationalCallMap()}
