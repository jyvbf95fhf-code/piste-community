...
const GPS_B2_DEBUG_ENABLED=typeof location!=='undefined'&&/stats-piste-community-v10-51-1-test/.test(location.hostname);
const gpsB2DebugState={current:null,sessions:[],panel:null};
function gpsB2DebugEnsurePanel(){if(!GPS_B2_DEBUG_ENABLED||gpsB2DebugState.panel)return gpsB2DebugState.panel;const create=()=>{if(gpsB2DebugState.panel||!document.body)return;const panel=document.createElement('aside');panel.id='gpsB2DebugPanel';panel.setAttribute('aria-label','DEBUG GPS BLOC 2');panel.style.cssText='position:fixed;left:8px;right:8px;bottom:max(8px,env(safe-area-inset-bottom));z-index:10000;max-height:42dvh;overflow:auto;padding:9px 10px;border:1px solid #d9b85c;border-radius:10px;background:rgba(10,17,18,.94);color:#f2edcf;font:11px/1.35 ui-monospace,monospace;box-shadow:0 5px 20px #0008';document.body.append(panel);gpsB2DebugState.panel=panel;gpsB2DebugRender()};if(document.body)create();else document.addEventListener('DOMContentLoaded',create,{once:true});return gpsB2DebugState.panel}
function gpsB2DebugLog(event,data={}){if(!GPS_B2_DEBUG_ENABLED)return;gpsB2DebugEnsurePanel();const current=gpsB2DebugState.current||{};Object.assign(current,{lastEvent:event,...data});gpsB2DebugState.current=current;if(current.sessionId){let row=gpsB2DebugState.sessions.find(item=>item.sessionId===current.sessionId);if(!row){row={sessionId:current.sessionId};gpsB2DebugState.sessions.push(row)}Object.assign(row,current)}try{console.debug(`[GPS-B2] ${event}`,data)}catch{}gpsB2DebugRender()}
function gpsB2DebugRender(){const panel=gpsB2DebugState.panel;if(!panel)return;const s=gpsB2DebugState.current||{},fmt=(v)=>v===undefined||v===null||v===''?'—':String(v);const fields=[['Session ID',s.sessionId],['Rôle',s.role],['Contexte',s.context],['Watcher local',s.localWatch],['Watcher registre',s.registryWatch],['Session registre',s.registrySession],['watchPosition appelé',s.watchCalled?'oui':'non'],['Callback success',s.success?'oui':'non'],['Erreur',s.errorCode?`${s.errorCode} ${s.errorMessage||''}`:'—'],['Own position',s.ownPosition?'oui':'non'],['GPS ready',s.gpsReady?'oui':'non'],['Marqueur',s.marker?'oui':'non'],['Recentrage',s.recenter?'oui':'non'],['Dernier événement',s.lastEvent]];const rows=fields.map(([label,value])=>`<div><b>${label}</b> : ${fmt(value)}</div>`).join('');const compare=gpsB2DebugState.sessions.slice(-2).map(row=>`<tr><td>${fmt(row.sessionId)}</td><td>${fmt(row.localWatch)}</td><td>${fmt(row.registryWatch)}</td><td>${row.watchCalled?'✓':'—'}</td><td>${row.success?'✓':'—'}</td><td>${row.errorCode||'—'}</td><td>${row.ownPosition?'✓':'—'}</td><td>${row.gpsReady?'✓':'—'}</td><td>${row.marker?'✓':'—'}</td><td>${row.recenter?'✓':'—'}</td></tr>`).join('');panel.innerHTML=`<strong>DEBUG GPS BLOC 2</strong>${rows}<hr style="border:0;border-top:1px solid #665d39"><div><b>Comparaison A/B</b></div><table style="width:100%;font-size:10px"><thead><tr><th>Session</th><th>local</th><th>reg</th><th>watch</th><th>ok</th><th>err</th><th>own</th><th>ready</th><th>mark</th><th>center</th></tr></thead><tbody>${compare}</tbody></table>`}
const gpsWatchRegistry=(()=>{const watches=new Map();const start=(context,onSuccess,onError,options,sessionId=null)=>{const existing=watches.get(context);typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('start-requested',{context,sessionId,registryWatch:existing?.id??null,registrySession:existing?.sessionId??null});if(existing&&existing.id!=null&&existing.sessionId===sessionId)return existing.id;if(existing)stop(context);void gpsPermission.getState();if(!globalThis?.navigator?.geolocation){onError?.({code:0,message:'Géolocalisation non disponible.'});return null}let id;try{typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('watchPosition-called',{context,sessionId,watchCalled:true});id=globalThis.navigator.geolocation.watchPosition(onSuccess,onError,options)}catch(error){onError?.(error);return null}if(id==null){onError?.({code:0,message:'Watcher GPS invalide.'});return null}watches.set(context,{id,sessionId});typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('watcher-id',{context,sessionId,registryWatch:id,registrySession:sessionId});return id};const stop=context=>{const existing=watches.get(context);if(!existing)return false;try{globalThis.navigator.geolocation.clearWatch(existing.id)}catch{}watches.delete(context);return true};const stopMany=contexts=>contexts.reduce((count,context)=>count+(stop(context)?1:0),0);const has=context=>watches.has(context)&&watches.get(context)?.id!=null;const get=context=>watches.get(context)?.id??null;const session=context=>watches.get(context)?.sessionId??null;return{start,stop,stopMany,has,get,session}})();
...
async function requestCoachingPreviewLocation(){
 ...
 typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('start-requested',{sessionId:previewSessionId,role:myCoachingRole(activeCoachingSession),context:'coaching_preview',localWatch:coachingPreviewWatch,registryWatch:gpsWatchRegistry.get('coaching_preview'),registrySession:gpsWatchRegistry.session('coaching_preview')});
 ...
 coachingPreviewWatch=gpsWatchRegistry.start('coaching_preview',pos=>{
  typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:id,context:'coaching_preview',success:true,accuracy:Number(pos?.coords?.accuracy)||null});
  if(activeCoachingSession?.id!==id||coachingDeferredNoPreviewV1045(activeCoachingSession))return;
  coachingPreviewPosition={lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:Number(pos.coords.accuracy)||0,recorded_at:new Date(pos.timestamp||Date.now()).toISOString()};typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:id,context:'coaching_preview',success:true,accuracy:coachingPreviewPosition.accuracy_m,ownPosition:true});
 ...
 },err=>{if(activeCoachingSession?.id!==id)return;typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('error',{sessionId:id,context:'coaching_preview',errorCode:err?.code??0,errorMessage:err?.message||''});coachingGpsError=err?.code===1?'autorisation de géolocalisation refusée':'position GPS indisponible';updateCoachingTerrainStatus()},{enableHighAccuracy:true,maximumAge:5000,timeout:15000},id);typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('registry-after',{sessionId:id,context:'coaching_preview',localWatch:coachingPreviewWatch,registryWatch:gpsWatchRegistry.get('coaching_preview'),registrySession:gpsWatchRegistry.session('coaching_preview')});return coachingPreviewWatch!==null;
}
...
function startCoachingPresence(){
 ...
 typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('start-requested',{sessionId:trackedSessionId,role:coachingGpsRole(activeCoachingSession),context:'coaching_presence',localWatch:coachingPresenceWatch,registryWatch:gpsWatchRegistry.get('coaching_presence'),registrySession:gpsWatchRegistry.session('coaching_presence')});
 ...
 void gpsPermission.getState();
 coachingPresenceWatch=gpsWatchRegistry.start('coaching_presence',pos=>{typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:trackedSessionId,context:'coaching_presence',success:true,accuracy:Number(pos?.coords?.accuracy)||null});if(coachingPresenceWatch===null||activeCoachingSession?.id!==trackedSessionId||(Number(activeCoachingSession.workflow_version)>=2&&coachingPhase(activeCoachingSession)!=='driver_running'))return;if(Date.now()-coachingPresenceLastPointAt<5000||pos.coords.accuracy>55)return;coachingPresenceLastPointAt=Date.now();const speed=Number.isFinite(Number(pos.coords.speed))?Number(pos.coords.speed):null,heading=resolveCoachingHeading(pos.coords.heading,speed),p={session_id:activeCoachingSession.id,owner_id:session.user.id,lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:pos.coords.accuracy,heading_deg:heading,speed_mps:speed,recorded_at:new Date(pos.timestamp).toISOString()};coachingOwnPosition=p;coachingGpsReady=true;typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:trackedSessionId,context:'coaching_presence',success:true,accuracy:p.accuracy_m,ownPosition:true,gpsReady:true});updateCoachingDeparture();updateFakeLock();updateCoachingPrimaryActions();supabase.from('coaching_live_points').insert(p).then(({error})=>{$('coachingGpsState').textContent=error?'Position non envoyée : '+error.message:`Position partagée • précision ${Math.round(p.accuracy_m)} m`;if(!error)scheduleCoachingMapRender()}).catch(()=>{$('coachingGpsState').textContent='Hors réseau — position en attente'})},err=>{typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('error',{sessionId:trackedSessionId,context:'coaching_presence',errorCode:err?.code??0,errorMessage:err?.message||''});coachingGpsError=err.message||'autorisation de géolocalisation refusée';stopCoachingPresence()},{enableHighAccuracy:true,maximumAge:1000,timeout:15000},trackedSessionId);typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('registry-after',{sessionId:trackedSessionId,context:'coaching_presence',localWatch:coachingPresenceWatch,registryWatch:gpsWatchRegistry.get('coaching_presence'),registrySession:gpsWatchRegistry.session('coaching_presence')});updateCoachingPrimaryActions()
}
...
async function startTraceurTracking(){const s=activeCoachingSession,allowed=s&&isCurrentUserLayingActor(s)&&coachingPhase(s)==='laying'&&s.status==='live';if(traceurWatch!==null&&gpsWatchRegistry.session('coaching_traceur')!==s?.id){gpsWatchRegistry.stop('coaching_traceur');traceurWatch=null}if(traceurWatch!==null)return;if(!allowed||!navigator.geolocation){coachingGpsError='La pose GPS est disponible uniquement pendant la phase de pose.';updateCoachingPrimaryActions();return}typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('start-requested',{sessionId:s?.id||null,role:myCoachingRole(s),context:'coaching_traceur',localWatch:traceurWatch,registryWatch:gpsWatchRegistry.get('coaching_traceur'),registrySession:gpsWatchRegistry.session('coaching_traceur')});coachingGpsError='';coachingGpsReady=false;traceurLastPointAt=0;if($('traceurStatus'))$('traceurStatus').textContent='Pose en cours — acquisition GPS…';void gpsPermission.getState();traceurWatch=gpsWatchRegistry.start('coaching_traceur',pos=>{typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:s?.id||null,context:'coaching_traceur',success:true,accuracy:Number(pos?.coords?.accuracy)||null});if(traceurWatch===null||activeCoachingSession?.id!==s.id||coachingPhase(activeCoachingSession)!=='laying')return;if(Date.now()-traceurLastPointAt<5000||pos.coords.accuracy>45)return;traceurLastPointAt=Date.now();const p={session_id:s.id,owner_id:session.user.id,lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:pos.coords.accuracy,recorded_at:new Date(pos.timestamp).toISOString()};coachingOwnPosition=p;coachingGpsReady=true;typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('success',{sessionId:s.id,context:'coaching_traceur',success:true,accuracy:p.accuracy_m,ownPosition:true,gpsReady:true});updateFakeLock();updateCoachingPrimaryActions();supabase.from('coaching_trace_points').insert(p).then(({error})=>{if(error)$('traceurStatus').textContent='Envoi interrompu : '+error.message;else{$('traceurStatus').textContent=`Pose enregistrée • précision ${Math.round(p.accuracy_m)} m`;scheduleCoachingMapRender()}}).catch(()=>{$('traceurStatus').textContent='Hors réseau : la pose nécessite une connexion.'})},err=>{typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('error',{sessionId:s.id,context:'coaching_traceur',errorCode:err?.code??0,errorMessage:err?.message||''});coachingGpsError=err.message||'autorisation de géolocalisation refusée';stopTraceurTracking()},{enableHighAccuracy:true,maximumAge:1000,timeout:15000},s.id);typeof gpsB2DebugLog==='function'&&gpsB2DebugLog('registry-after',{sessionId:s.id,context:'coaching_traceur',localWatch:traceurWatch,registryWatch:gpsWatchRegistry.get('coaching_traceur'),registrySession:gpsWatchRegistry.session('coaching_traceur')});updateCoachingPrimaryActions()}
