...
let coachingScenarioViewer={open:false,urls:[],index:0,touchStartX:null};
...
async function ensureScenarioPhotoViewer(){let viewer=$('coachingScenarioPhotoViewer');if(viewer)return viewer;viewer=document.createElement('div');viewer.id='coachingScenarioPhotoViewer';viewer.className='coaching-scenario-photo-viewer hidden';viewer.innerHTML='<button type="button" data-scenario-viewer="close" aria-label="Fermer">×</button><button type="button" data-scenario-viewer="prev" aria-label="Photo précédente">‹</button><figure><img alt="Photo du scénario"><figcaption></figcaption></figure><button type="button" data-scenario-viewer="next" aria-label="Photo suivante">›</button>';document.body.append(viewer);viewer.querySelector('[data-scenario-viewer="close"]').onclick=()=>closeScenarioPhotoViewer();viewer.querySelector('[data-scenario-viewer="prev"]').onclick=()=>moveScenarioPhotoViewer(-1);viewer.querySelector('[data-scenario-viewer="next"]').onclick=()=>moveScenarioPhotoViewer(1);viewer.addEventListener('touchstart',event=>{coachingScenarioViewer.touchStartX=event.touches?.[0]?.clientX??null},{passive:true});viewer.addEventListener('touchend',event=>{const start=coachingScenarioViewer.touchStartX,end=event.changedTouches?.[0]?.clientX;if(start!==null&&Number.isFinite(end)&&Math.abs(end-start)>40)moveScenarioPhotoViewer(start-end>0?1:-1);coachingScenarioViewer.touchStartX=null},{passive:true});return viewer}
async function openScenarioPhotoViewer(urls,index=0){const list=(urls||[]).filter(Boolean);if(!list.length)return;const viewer=await ensureScenarioPhotoViewer();coachingScenarioViewer={...coachingScenarioViewer,open:true,urls:list,index:Math.max(0,Math.min(index,list.length-1))};viewer.classList.remove('hidden');preReleaseDebugEvent('SCENARIO_PHOTO_VIEW_OPEN',`${coachingScenarioViewer.index+1}/${list.length}`);renderScenarioPhotoViewer()}
function renderScenarioPhotoViewer(){const viewer=$('coachingScenarioPhotoViewer');if(!viewer||!coachingScenarioViewer.open)return;const {urls,index}=coachingScenarioViewer;viewer.querySelector('img').src=urls[index];viewer.querySelector('figcaption').textContent=`${index+1}/${urls.length}`;viewer.querySelector('[data-scenario-viewer="prev"]').hidden=urls.length<2;viewer.querySelector('[data-scenario-viewer="next"]').hidden=urls.length<2}
function moveScenarioPhotoViewer(delta){if(!coachingScenarioViewer.open)return;coachingScenarioViewer.index=(coachingScenarioViewer.index+delta+coachingScenarioViewer.urls.length)%coachingScenarioViewer.urls.length;renderScenarioPhotoViewer()}
function closeScenarioPhotoViewer(){const viewer=$('coachingScenarioPhotoViewer');if(viewer)viewer.classList.add('hidden');if(coachingScenarioViewer.open)preReleaseDebugEvent('SCENARIO_PHOTO_VIEW_CLOSE');coachingScenarioViewer={...coachingScenarioViewer,open:false,touchStartX:null}}
...
async function markCoachingScenarioRead(){const id=activeCoachingSession?.id;if(!id||coachingScenarioState.status!=='ready')return false;coachingScenarioGate.busy=true;preReleaseDebugEvent('SCENARIO_GATE_READ_START');await renderCoachingScenarioGate();const {data,error}=await supabase.rpc('mark_coaching_scenario_read_v10492',{p_session_id:id});if(error){coachingScenarioGate.busy=false;coachingScenarioRecord('READ_ERROR',error.message);preReleaseDebugEvent('SCENARIO_GATE_READ_ERROR',error.message);coachingToast(`Lecture du scénario impossible : ${error.message}`);await renderCoachingScenarioGate();return false}await loadCoachingScenario(id);const confirmed=coachingScenarioReadByCurrentUser()||!!coachingScenarioState.scenario?.locked_at;if(!confirmed){coachingScenarioGate.busy=false;preReleaseDebugEvent('SCENARIO_GATE_READ_ERROR','confirmation serveur absente');await renderCoachingScenarioGate();return false}coachingScenarioGate.busy=false;coachingScenarioGate.readServer=true;coachingScenarioGate.required=false;coachingScenarioGate.visible=false;preReleaseDebugEvent('SCENARIO_GATE_READ_OK');preReleaseDebugEvent('SCENARIO_GATE_CLOSE');await renderCoachingScenarioGate();return !!data}
