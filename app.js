...
let coachingLiveWeather=null,coachingWeatherTimer=null,coachingWeatherLoading=false,coachingWeatherRequestId=0,coachingTerrainPaused=false,coachingFinishTimer=null,coachingFinishArmed=false,coachingActiveBannerState={activeKm:null};
...
function updateCoachingTerrainStatus(next={}){
 const s=activeCoachingSession;if(!s)return;const startedAt=s.driver_started_at,finishedAt=s.driver_finished_at,started=startedAt?new Date(startedAt).getTime():NaN,finished=finishedAt?new Date(finishedAt).getTime():Date.now(),activeMs=Number.isFinite(started)&&Number.isFinite(finished)?Math.max(0,finished-started):null,trackOrigin=resolveTrackOrigin(s),origin=trackOrigin.quality==='recorded'||trackOrigin.quality==='declared'?new Date(trackOrigin.instant).getTime():NaN,ageEnd=s.driver_finished_at?new Date(s.driver_finished_at).getTime():Date.now(),trackAgeMs=Number.isFinite(origin)&&Number.isFinite(ageEnd)&&ageEnd>=origin?ageEnd-origin:null,hasKm=Object.prototype.hasOwnProperty.call(next,'activeKm');
 if(typeof coachingActiveBannerState==='object'&&hasKm)coachingActiveBannerState.activeKm=next.activeKm;
 const activeKm=hasKm?next.activeKm:(typeof coachingActiveBannerState==='object'?coachingActiveBannerState.activeKm:null),pauseState=s.pause_state||(coachingTerrainPaused?'paused':'running');
...
async function refreshActiveCoachingSession(id){const {data,error}=await supabase.rpc('get_my_coaching_sessions',{p_session_id:id});const fresh=!error&&Array.isArray(data)?data[0]:null;if(!fresh||activeCoachingSession?.id!==id)return false;activeCoachingSession={...activeCoachingSession,...fresh,_received_at:Date.now()};markCoachingSyncV10423();const index=coachingSessions.findIndex(x=>x.id===id);if(index>=0)coachingSessions[index]=activeCoachingSession;else coachingSessions.push(activeCoachingSession);if(typeof applyV1040RoleSurface==='function')applyV1040RoleSurface();return true}
...
function updateCoachingLiveMetrics(groups,trace=[]){const started=activeCoachingSession?.driver_started_at?new Date(activeCoachingSession.driver_started_at).getTime():0,driver=([...groups.entries()].find(([userId])=>['driver','solo'].includes(coachingMemberRole(userId)))?.[1]||[]).filter(p=>!started||new Date(p.recorded_at).getTime()>=started),activeKm=driver.length>1?routeDistance(driver)/1000:null;updateCoachingTerrainStatus({activeKm})}
