...
function preReleaseDebugSanitize(value){let text=value instanceof Error?value.message:String(value??'');return text.replace(/[0-9a-f]{8}-[0-9a-f-]{27,}/gi,'[id]').replace(/[A-Fa-f0-9]{8,}/g,'[id]').replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g,'[email]').replace(/https?:\/\/\S+/gi,'[url]').replace(/(token|secret|password|apikey|access_token)[^\s,;]*/gi,'$1=[redacted]');}
...
async function submitCoachingWizard(){
 ...
 try{
  ...
  const preparation=coachingWizard.trackPreparation,withoutRoute=coachingWizard.sessionType==='solo'||(coachingWizard.mode==='full_blind'&&['coach','driver'].includes(coachingWizard.creatorRole));
