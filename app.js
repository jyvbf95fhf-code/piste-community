...
let wakeLock=null,wakeLockSupported='wakeLock' in navigator,fakeLockPressTimer=null,fakeLockUnlockReady=false,fakeLockBlockUntil=0,fakeLockContext='record';
let plannerPoints=[], plannerWaypoints=[], plannerRedoStack=[], plannerTool='route', coachingSessions=[], activeCoachingSession=null, coachingLastPointAt=0, traceurLastPointAt=0, traceurWatch=null, coachingPresenceWatch=null, coachingPresenceLastPointAt=0, coachingOwnPosition=null, coachingMapRefreshTimer=null, coachingKeepViewport=false, liveMarkerTool='off', coachingAutoMetrics=null, coachingPanel='team', coachingSessionFilter='upcoming';
...
async function requestWakeLock(){
 ...
 try{
  if(!wakeLock){wakeLock=await navigator.wakeLock.request('screen');wakeLock.addEventListener?.('release',()=>{wakeLock=null;if(((gps.start&&!gps.paused)||(activeCoachingSession&&activeCoachingSession.status!=='ended'))&&document.visibilityState==='visible')setTimeout(requestWakeLock,250)},{once:true})}
...
async function openFakeLock(context='record'){if(context==='record'&&(!gps.start||gps.paused))return;if(context==='coaching'&&(!activeCoachingSession||activeCoachingSession.status==='ended'))return;fakeLockContext=context;$('fakeLockScreen').classList.remove('hidden');document.body.classList.add('fake-lock-active');const keptAwake=await requestWakeLock();$('fakeLockWakeStatus').textContent=keptAwake?'Écran maintenu actif':'Maintien d’écran indisponible : ne verrouille pas l’iPhone';$('fakeLockWakeStatus').className=keptAwake?'good':'bad';updateFakeLock()}
...
async function loadCoachingHub(){
 ...
 if(!error){const saved=readActiveCoachingRef(),valid=saved&&coachingSessions.find(s=>s.id===saved.id&&!['ended','cancelled'].includes(s.status));if(saved&&!valid)clearActiveCoachingRef(saved.id);if(valid)activeCoachingSession=valid;else if(activeCoachingSession&&!coachingSessions.some(s=>s.id===activeCoachingSession.id))activeCoachingSession=null}
...
function saveActiveCoachingRef(s){if(!s||['ended','cancelled'].includes(s.status))return clearActiveCoachingRef(s?.id);try{localStorage.setItem(activeCoachingStorageKey(),JSON.stringify({id:s.id,name:s.name||'Session coaching',status:s.status,owner_id:s.owner_id,invite_code:s.invite_code||'',role:isSoloCoaching(s)?'solo':myCoachingRole(s)}))}catch{}}
...
function stopCoachingPresence(){if(coachingPresenceWatch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(coachingPresenceWatch);coachingPresenceWatch=null;coachingPresenceLastPointAt=0;if($('coachingGpsState'))$('coachingGpsState').textContent='Position non partagée'}
...
function coachingPrimaryLabel(role,status){if(status==='ended')return 'Session terminée';if(role==='driver'||role==='solo')return status==='live'?'Ouvrir mon suivi GPS':'Départ avec mon chien';if(role==='traceur')return status==='live'?'Partager ma position':'Départ de piste';if(role==='coach')return status==='live'?'Partager ma position':'Départ de piste';return 'Suivre la session'}
async function openCoachingSession(id){
 clearCoachingRealtime();coachingKeepViewport=false;
 let s=coachingSessions.find(x=>x.id===id);if(!s){const {data}=await supabase.rpc('get_my_coaching_sessions',{p_session_id:id});s=Array.isArray(data)?data[0]:null}if(!s){clearActiveCoachingRef(id);activeCoachingSession=null;refreshActiveSessionShortcut();return}
 activeCoachingSession=s;coachingAutoMetrics=null;const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=s.owner_id===session.user.id;saveActiveCoachingRef(s);refreshActiveSessionShortcut();
 ...
 $('startCoachingLive').textContent=coachingPrimaryLabel(role,s.status);$('startCoachingLive').classList.toggle('hidden',role==='observer'||s.status==='ended');$('endCoachingLive').classList.toggle('hidden',!['coach','driver','solo'].includes(role)||s.status!=='live');$('coachAnnotationTools').classList.toggle('hidden',!['coach','driver','solo'].includes(role)||s.status==='ended');$('traceurTools').classList.toggle('hidden',role!=='traceur'||s.status==='ended');$('calculateCoachingDebrief').classList.toggle('hidden',s.status==='waiting');$('coachingDebriefForm').classList.toggle('hidden',s.status==='waiting'||!['coach','driver','solo'].includes(role));$('coachingAutoDebrief').classList.add('hidden');
 $('endCoachingLive').classList.toggle('hidden',!owner||s.status!=='live');$('cancelCoachingSession').classList.toggle('hidden',!owner||s.status!=='waiting');$('deleteCoachingSession').classList.toggle('hidden',!owner||s.status!=='waiting');$('leaveCoachingSession').classList.toggle('hidden',owner||!['waiting','live'].includes(s.status));
 updateCoachingPreflight();updateCoachingPhase();updateCoachingDeparture();setCoachingPanel('team');setCoachingStage(s.status==='ended'?'debrief':'live');if(s.status!=='ended')startCoachingPresence();await renderCoachingMap();await loadCoachingMessages();
 coachingChannel=supabase.channel(`coaching-${s.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_live_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_trace_points',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_markers',filter:`session_id=eq.${s.id}`},scheduleCoachingMapRender).on('postgres_changes',{event:'INSERT',schema:'public',table:'coaching_messages',filter:`session_id=eq.${s.id}`},()=>loadCoachingMessages()).subscribe();
 setTimeout(()=>$('coachingLivePanel').scrollIntoView({behavior:'smooth'}),100)
}
...
function participantMarker(point,role,label){const colors={driver:'#27b7ff',solo:'#27b7ff',coach:'#a986ff',traceur:'#58d6a4'},letters={driver:'D',solo:'D',coach:'C',traceur:'T'},color=colors[role]||'#91a09d';return L.marker([point.lat,point.lon],{zIndexOffset:900,icon:L.divIcon({className:'participant-map-icon',html:`<span style="--participant-color:${color}">${letters[role]||'•'}</span>`,iconSize:[38,38],iconAnchor:[19,19]})}).addTo(coachingMap).bindTooltip(`${label} • ${new Date(point.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`)}
function updateCoachingParticipants(groups,trace){const el=$('coachingParticipants');if(!el)return;const rows=[];for(const [userId,points] of groups){const role=coachingMemberRole(userId),last=points.at(-1);rows.push(`<span class="participant-chip ${role}"><i></i>${esc(coachingRoleLabel(role))} <small>${last?new Date(last.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}):'en attente'}</small></span>`)}if(trace.length){const last=trace.at(-1);rows.push(`<span class="participant-chip traceur"><i></i>Traceur <small>${new Date(last.recorded_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</small></span>`)}el.innerHTML=rows.join('')||'<span class="participant-chip waiting"><i></i>En attente des positions GPS</span>'}
function updateCoachingLiveMetrics(groups){const started=activeCoachingSession?.started_at?new Date(activeCoachingSession.started_at).getTime():Infinity,driver=([...groups.entries()].find(([userId])=>['driver','solo'].includes(coachingMemberRole(userId)))?.[1]||[]).filter(p=>new Date(p.recorded_at).getTime()>=started),distance=routeDistance(driver)/1000,duration=driver.length>1?Math.max(0,new Date(driver.at(-1).recorded_at)-new Date(driver[0].recorded_at)):0;if($('coachingLiveDistance'))$('coachingLiveDistance').textContent=distance.toFixed(2)+' km';if($('coachingLiveTime'))$('coachingLiveTime').textContent=formatExactDuration(duration)}
...
async function renderCoachingMap(){
 ...
 const id=activeCoachingSession.id,[liveRes,traceRes,markerRes,memberRes]=await Promise.all([supabase.from('coaching_live_points').select('owner_id,lat,lon,recorded_at').eq('session_id',id).order('recorded_at'),supabase.from('coaching_trace_points').select('owner_id,lat,lon,recorded_at').eq('session_id',id).order('recorded_at'),supabase.from('coaching_markers').select('*').eq('session_id',id).order('created_at'),supabase.from('coaching_members').select('role,user_id').eq('session_id',id)]);
 ...
 setCoachingReplayData(trace,points,annotations);updateCoachingParticipants(liveGroups,trace);updateCoachingLiveMetrics(liveGroups);
 const oldView=coachingMap&&coachingKeepViewport?{center:coachingMap.getCenter(),zoom:coachingMap.getZoom(),bearing:coachingMap.getBearing?.()||0}:null;if(coachingMap){coachingMap.remove();coachingMap=null}coachingMap=createPisteMap('coachingMap').setView(oldView?.center||[48.3,7.45],oldView?.zoom||9);if(oldView?.bearing)coachingMap.setBearing?.(oldView.bearing);addCleanBaseLayers(coachingMap);coachingLayers=[];
 ...
 if(coachingLayerVisibility.actual)for(const [userId,userPoints] of liveGroups){const memberRole=coachingMemberRole(userId),color=['driver','solo'].includes(memberRole)?'#2498f0':memberRole==='coach'?'#a986ff':'#91a09d',weight=['driver','solo'].includes(memberRole)?5:3;if(userPoints.length>1)coachingLayers.push(L.polyline(userPoints.map(p=>[p.lat,p.lon]),{color,weight,opacity:.95,lineCap:'round',lineJoin:'round',className:`map-route-line map-route-${memberRole}`}).addTo(coachingMap));if(userPoints.length)coachingLayers.push(participantMarker(userPoints.at(-1),memberRole,coachingRoleLabel(memberRole)))}
 if(coachingLayerVisibility.trace&&trace.length)coachingLayers.push(participantMarker(trace.at(-1),'traceur','Traceur'));
 ...
 const all=[...route,...trace,...allLive];if(all.length&&!oldView)coachingMap.fitBounds(L.latLngBounds(all.map(p=>[p.lat,p.lon])),{padding:[30,30]});
 updateCoachingDeparture();coachingMap.on('dragstart zoomstart rotatestart',()=>{coachingKeepViewport=true});coachingMap.on('click',e=>addLiveCoachingMarker(e.latlng));
}
...
async function ensureCoachingStarted(){if(!activeCoachingSession)return false;if(activeCoachingSession.status==='live')return true;const {error}=await supabase.rpc('start_coaching_session',{p_session_id:activeCoachingSession.id});if(error){alert('Départ impossible : '+error.message);return false}activeCoachingSession.status='live';activeCoachingSession.started_at=new Date().toISOString();saveActiveCoachingRef(activeCoachingSession);refreshActiveSessionShortcut();$('coachingLiveStatus').textContent='En direct • positions GPS synchronisées';return true}
function startCoachingPresence(){
 if(!activeCoachingSession||!navigator.geolocation||coachingPresenceWatch!==null)return;
 coachingPresenceLastPointAt=0;$('coachingGpsState').textContent='Recherche de la position…';
 coachingPresenceWatch=navigator.geolocation.watchPosition(pos=>{if(Date.now()-coachingPresenceLastPointAt<5000||pos.coords.accuracy>55)return;coachingPresenceLastPointAt=Date.now();const p={session_id:activeCoachingSession.id,owner_id:session.user.id,lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:pos.coords.accuracy,recorded_at:new Date(pos.timestamp).toISOString()};coachingOwnPosition=p;updateCoachingDeparture();updateFakeLock();supabase.from('coaching_live_points').insert(p).then(({error})=>{$('coachingGpsState').textContent=error?'Position non envoyée : '+error.message:`Position partagée • précision ${Math.round(p.accuracy_m)} m`;if(!error)scheduleCoachingMapRender()}).catch(()=>{$('coachingGpsState').textContent='Hors réseau — position en attente'})},err=>$('coachingGpsState').textContent='GPS : '+err.message,{enableHighAccuracy:true,maximumAge:1000,timeout:15000})
}
async function startActiveCoaching(){
 if(!activeCoachingSession||!await ensureCoachingStarted())return;const role=isSoloCoaching(activeCoachingSession)?'solo':myCoachingRole(activeCoachingSession);
 updateCoachingPhase();
 if(role==='driver'||role==='solo'){selectedTrainingRoute={id:activeCoachingSession.route_id,name:activeCoachingSession.name,route:activeCoachingSession.planned_route||[],waypoints:activeCoachingSession.planned_markers||[],odor_model:activeCoachingSession.odor_model||{},planned_distance_km:Number((routeDistance(activeCoachingSession.planned_route||[])/1000).toFixed(2))};beginNewPiste('training');setTimeout(()=>applySelectedTrainingRoute(),250);return}
 if(role==='traceur'){startTraceurTracking();return}
 if(role==='coach')startCoachingPresence();
 $('startCoachingLive').textContent=coachingPrimaryLabel(role,'live');$('endCoachingLive').classList.toggle('hidden',activeCoachingSession.owner_id!==session.user.id);
}
async function finishActiveCoaching(){if(!activeCoachingSession||activeCoachingSession.owner_id!==session?.user?.id||!confirm('Mettre fin à cette piste pour toute l’équipe ?'))return;const id=activeCoachingSession.id,{error}=await supabase.rpc('finish_coaching_session',{p_session_id:id});if(error){alert('Fin impossible : '+error.message);return}stopCoachingPresence();stopTraceurTracking();activeCoachingSession.status='ended';clearActiveCoachingRef(id);clearCoachingRealtime();closeFakeLock();refreshActiveSessionShortcut();await loadCoachingHub();await openCoachingSession(id)}
async function cancelActiveCoaching(){if(!activeCoachingSession||activeCoachingSession.owner_id!==session?.user?.id||activeCoachingSession.status!=='waiting'||!confirm('Annuler cette session en attente ? Elle restera visible dans les sessions terminées.'))return;const id=activeCoachingSession.id,{error}=await supabase.from('coaching_sessions').update({status:'cancelled',ended_at:new Date().toISOString()}).eq('id',id).eq('owner_id',session.user.id).eq('status','waiting');if(error){alert('Annulation impossible : '+error.message);return}clearActiveCoachingRef(id);activeCoachingSession=null;clearCoachingRealtime();$('coachingLivePanel').classList.add('hidden');await loadCoachingHub();refreshActiveSessionShortcut()}
async function deleteActiveCoaching(){if(!activeCoachingSession||activeCoachingSession.owner_id!==session?.user?.id||activeCoachingSession.status!=='waiting'||!confirm('Supprimer définitivement cette session en attente ? Cette action est irréversible.'))return;const id=activeCoachingSession.id,{error}=await supabase.from('coaching_sessions').delete().eq('id',id).eq('owner_id',session.user.id).eq('status','waiting');if(error){alert('Suppression impossible : '+error.message);return}clearActiveCoachingRef(id);activeCoachingSession=null;clearCoachingRealtime();$('coachingLivePanel').classList.add('hidden');await loadCoachingHub();refreshActiveSessionShortcut()}
async function leaveActiveCoaching(){if(!activeCoachingSession||activeCoachingSession.owner_id===session?.user?.id||!confirm('Quitter cette session ? La session de l’organisateur ne sera pas supprimée.'))return;const id=activeCoachingSession.id,{error}=await supabase.from('coaching_members').delete().eq('session_id',id).eq('user_id',session.user.id);if(error){alert('Impossible de quitter : '+error.message);return}clearActiveCoachingRef(id);activeCoachingSession=null;clearCoachingRealtime();$('coachingLivePanel').classList.add('hidden');await loadCoachingHub();refreshActiveSessionShortcut()}
async function saveCoachingDebrief(e){e.preventDefault();if(!activeCoachingSession)return;const f=new FormData(e.target),{data:allPoints=[]}=await supabase.from('coaching_live_points').select('owner_id,lat,lon,accuracy_m,recorded_at').eq('session_id',activeCoachingSession.id).order('recorded_at'),points=allPoints.filter(p=>['driver','solo'].includes(coachingMemberRole(p.owner_id)));if(!coachingAutoMetrics)await calculateCoachingDebrief();const payload={session_id:activeCoachingSession.id,owner_id:activeCoachingSession.owner_id,coach_id:myCoachingRole(activeCoachingSession)==='driver'?null:session.user.id,strengths:f.get('strengths')||null,improvement_area:f.get('improvement_area')||null,coach_notes:f.get('coach_notes')||null,actual_track:points,auto_metrics:coachingAutoMetrics||{},updated_at:new Date().toISOString()};const {error}=await supabase.from('coaching_debriefs').upsert(payload);if(error){alert(error.message);return}const endedId=activeCoachingSession.id,{error:endError}=await supabase.rpc('finish_coaching_session',{p_session_id:endedId});if(endError){alert('Débrief enregistré, mais clôture impossible : '+endError.message);return}activeCoachingSession.status='ended';clearActiveCoachingRef(endedId);clearCoachingRealtime();refreshActiveSessionShortcut();alert('Débrief enregistré. Les tracés, événements et mesures automatiques sont conservés.');await loadCoachingHub();await openCoachingSession(endedId);await calculateCoachingDebrief()}
function sendActiveCoachingPoint(p){if(!activeCoachingSession||activeCoachingSession.status!=='live'||!['driver','solo'].includes(myCoachingRole(activeCoachingSession))||!navigator.onLine||Date.now()-coachingLastPointAt<5000)return;coachingLastPointAt=Date.now();supabase.from('coaching_live_points').insert({session_id:activeCoachingSession.id,owner_id:session.user.id,lat:p.lat,lon:p.lon,accuracy_m:p.acc,recorded_at:new Date(p.t).toISOString()}).then(()=>{}).catch(()=>{})}
async function startTraceurTracking(){if(!activeCoachingSession||myCoachingRole(activeCoachingSession)!=='traceur'||!navigator.geolocation||!await ensureCoachingStarted())return;traceurLastPointAt=0;$('startTraceurTrack').disabled=true;$('stopTraceurTrack').disabled=false;$('traceurStatus').textContent='Pose en cours — acquisition GPS…';if(traceurWatch!==null)navigator.geolocation.clearWatch(traceurWatch);traceurWatch=navigator.geolocation.watchPosition(pos=>{if(Date.now()-traceurLastPointAt<5000||pos.coords.accuracy>45)return;traceurLastPointAt=Date.now();const p={session_id:activeCoachingSession.id,owner_id:session.user.id,lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy_m:pos.coords.accuracy,recorded_at:new Date(pos.timestamp).toISOString()};supabase.from('coaching_trace_points').insert(p).then(({error})=>{if(error)$('traceurStatus').textContent='Envoi interrompu : '+error.message;else{$('traceurStatus').textContent=`Pose enregistrée • précision ${Math.round(p.accuracy_m)} m`;scheduleCoachingMapRender()}}).catch(()=>{$('traceurStatus').textContent='Hors réseau : la pose nécessite une connexion.'})},err=>$('traceurStatus').textContent='GPS : '+err.message,{enableHighAccuracy:true,maximumAge:1000,timeout:15000})}
...
function currentActiveShortcut(){const draft=getDraft();if(draft?.start&&draft.user_id===session?.user?.id)return{kind:'record',title:draft.mode==='training'?'Entraînement en cours':'Pistage en cours',info:`${fmt(Number(draft.distance||0)/1000,2)} km • reprendre le GPS`};const saved=readActiveCoachingRef(),coaching=(activeCoachingSession&&!['ended','cancelled'].includes(activeCoachingSession.status)?activeCoachingSession:null)||coachingSessions.find(s=>s.id===saved?.id&&!['ended','cancelled'].includes(s.status))||coachingSessions.find(s=>s.status==='live');if(!coaching&&!saved)return null;const ref=coaching||saved;return{kind:'coaching',id:ref.id,title:ref.name||'Session coaching',info:`${coachingStatusLabel(ref.status)} • ${coachingRoleLabel(coaching?(isSoloCoaching(coaching)?'solo':myCoachingRole(coaching)):ref.role)}`}}
function refreshActiveSessionShortcut(){const active=currentActiveShortcut(),banner=$('activeSessionBanner'),dock=$('activeSessionDock');banner?.classList.toggle('hidden',!active);dock?.classList.toggle('hidden',!active);document.body.classList.toggle('has-active-session',!!active);if(!active)return;$('activeSessionTitle').textContent=active.title;$('activeSessionInfo').textContent=active.info;$('activeSessionDockTitle').textContent=active.title;banner.dataset.kind=dock.dataset.kind=active.kind;banner.dataset.id=dock.dataset.id=active.id||''}
function resumeActiveSession(){const active=currentActiveShortcut();if(!active)return;if(active.kind==='record'){$('resumeDraftBtn').click();return}showPage('coachingPage');openCoachingSession(active.id)}
...
$('resumeActiveSessionBtn').onclick=resumeActiveSession;$('activeSessionDock').onclick=resumeActiveSession;
