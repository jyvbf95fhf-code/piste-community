...
async function openCoachingSession(id,{resume=false}={}){
 ...
 activeCoachingSession=s;coachingSessionEndHandledId=null;coachingAutoMetrics=null;coachingDebriefData=null;coachingParticipantObservations=[];coachingGpsError='';coachingGpsReady=false;if(Number(s.workflow_version)>=2){const refreshed=await refreshActiveCoachingSession(s.id);if(refreshed)s=activeCoachingSession}const globalPhase=coachingGlobalPhase(s);if(globalPhase==='active')requestCoachingPreviewLocation();$('coachingDebriefForm')?.reset();$('coachingDriverFeedbackForm')?.reset();setUiText('coachingDebriefState','Aucun brouillon chargé.');setUiText('coachingDriverFeedbackState','Aucun retour enregistré.');setUiText('coachingObservationState','Chargement des observations…');const role=isSoloCoaching(s)?'solo':myCoachingRole(s),owner=isCoachingOwner(s);if(s.status==='live'&&globalPhase==='active'){verifiedActiveCoachingSession=s;coachingShortcutValidated=true;saveActiveCoachingRef(s)}refreshActiveSessionShortcut();
 $('coachingLivePanel').classList.remove('hidden');$('coachingRole').textContent=coachingCapabilityLabel(membership,s).toUpperCase();$('coachingLiveTitle').textContent=s.name||'Session coachée';$('coachingLiveCode').textContent=s.invite_code||'';$('copyCoachingCode').classList.toggle('hidden',!s.invite_code);const mode=coachingBlindMode(s);$('coachingLiveStatus').textContent=`${coachingStatusLabel(s.status)} • ${mode==='normal'||mode==='all'?'mode normal':mode==='simple_blind'||mode==='progressive'?'simple aveugle':'double aveugle'}`;
...
async function saveParticipantObservation(sessionId,body,version){
 const userId=session?.user?.id;if(!userId)throw new Error('Connexion requise.');const text=String(body??'');if(text.length>2500)throw new Error('Observation limitée à 2500 caractères.');const columns='session_id,user_id,role,body,created_at,updated_at,revision';let response;
...
async function openLibraryItem(type,id){return openMissionDossier(type,id)}
