...
function drawCoachingDriverTrail(points){if(!coachingMap||!coachingLayerVisibility.actual||points.length<2)return null;const line=L.polyline(points.map(point=>[point.lat,point.lon]),{color:'#168de2',weight:7,opacity:1,lineCap:'round',lineJoin:'round',className:'map-route-line map-route-actual coaching-driver-trail',interactive:false}).addTo(coachingMap);line.bringToFront();coachingLayers.push(line);return line}
...
async function renderCoachingMap(){
 ...
 const departure=coachingDeparture();if(departure)coachingLayers.push(L.circleMarker([departure.lat,departure.lon],{radius:7,color:'#fff',fillColor:'#6f9455',fillOpacity:1}).addTo(coachingMap).bindTooltip('Départ'));
...
function renderMissionMap(source){
 ...
 for(const [label,points,color,dash] of [['Tracé prévu',source.planned,'#b7c78d','8 6'],['Tracé Traceur',source.trace,'#3ddb9d','4 7'],[source.type==='coaching'?'Parcours Conducteur':'Trace GPS',source.actual,'#42aaff',null]]){
  ...
  [clean[0],clean.at(-1)].forEach((p,i)=>L.circleMarker([p.lat,p.lon],{radius:6,color}).bindPopup(`${esc(label)} · ${i?'Arrivée':'Départ'}`).addTo(group));group.addTo(missionMap);layers[label]=group;all.push(group);
 }
 for(const [index,track] of (source.gpx||[]).entries()){if(track.points.length<2)continue;const line=L.polyline(track.points.map(p=>[p.lat,p.lon]),{color:'#cba6ff',weight:3,dashArray:'6 4'});if(track.visible!==false)line.addTo(missionMap);layers[`GPX ${index+1} · ${esc(track.name||'Importé')}`]=line;all.push(line)}
 const markers=L.featureGroup();for(const marker of source.markers.filter(valid)){const photo=missionPhotoUrl(marker.photo_data),def=LIVE_MARKERS[marker.type||marker.marker_type]||LIVE_MARKERS.note;L.marker([marker.lat,marker.lon??marker.lng]).bindPopup(`<b>${esc(marker.name||def.label)}</b><p>${esc(marker.comment||marker.note||'')}</p>${photo?`<a href="${photo}" target="_blank" rel="noopener"><img class="mission-marker-photo" src="${photo}" alt="Photo du repère"></a>`:''}`,{maxWidth:240}).addTo(markers)}
 if(markers.getLayers().length){markers.addTo(missionMap);all.push(markers);layers['Repères / Objets']=markers}
