...
let operationalCalls=[],activeOperationalCallId=null,currentOperationalCall=null,operationalCallMap=null,operationalCallLayers=[],operationalCallPoint=null,operationalCallMarkers=[],operationalCallWeather={},operationalCallAnalysis={},operationalCallStep=1; // V10.27_OPERATIONAL_CALL
...
function renderOperationalCallMap(){if(!operationalCallMap)return;operationalCallLayers.forEach(x=>{try{x.remove()}catch{}});operationalCallLayers=[];if(operationalCallPoint){const marker=L.marker([operationalCallPoint.lat,operationalCallPoint.lon],{draggable:true}).addTo(operationalCallMap).bindPopup('<b>Dernier point connu</b><br>'+esc(operationalCallPoint.label||''));marker.on('dragend',e=>{const p=e.target.getLatLng();operationalCallPoint={...operationalCallPoint,lat:p.lat,lon:p.lng};renderOperationalCallSummary()});operationalCallLayers.push(marker);$('openCallNavigationBtn').disabled=false}operationalCallMarkers.forEach((m,i)=>{const d=CALL_MARKERS[m.type]||CALL_MARKERS.note,marker=L.marker([m.lat,m.lon],{icon:L.divIcon({className:'call-map-marker',html:`<span>${d.icon}</span>`,iconSize:[34,34],iconAnchor:[17,17]})}).addTo(operationalCallMap).bindPopup(`<b>${esc(d.label)}</b><br>${esc(m.note||'')}`);operationalCallLayers.push(marker)});const habitual=operationalCallMarkers.filter(x=>x.type==='route');if(habitual.length>1)operationalCallLayers.push(L.polyline(habitual.map(x=>[x.lat,x.lon]),{color:'#a98be8',weight:3,dashArray:'7 7',opacity:.85}).addTo(operationalCallMap));const all=[...(operationalCallPoint?[operationalCallPoint]:[]),...operationalCallMarkers];if(all.length>1)operationalCallMap.fitBounds(all.map(x=>[x.lat,x.lon]),{padding:[25,25]});else if(all.length===1)operationalCallMap.setView([all[0].lat,all[0].lon],16)}
...
$('newOperationalTerrainBtn').onclick=()=>{activeOperationalCallId=null;beginNewPiste('piste')};
$('receivedCallBtn').onclick=()=>{showPage('operationalCallPage');resetOperationalCall()};
$('detachOperationalCall').onclick=()=>{activeOperationalCallId=null;$('operationalCallBanner').classList.add('hidden')};
