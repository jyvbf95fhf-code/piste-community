...
function hideLegacyCoachingPrimaryActions(){['startCoachingLive','toggleCoachingGps','coachingFakeLockBtn','endCoachingLive'].forEach(id=>{const button=$(id);if(button){button.classList.add('hidden');button.disabled=true;button.setAttribute('aria-hidden','true')}})}
...
async function beginCoachingLaying(){const ok=await coachingTransitionV1040('start_coaching_laying');if(ok&&activeCoachingSession)await startTraceurTracking();return ok}
...
function bindV1040Hold(id,action){const button=$(id);if(!button)return;['pointerdown','touchstart'].forEach(type=>button.addEventListener(type,event=>{event.preventDefault();layingHoldStart(button,action)},{passive:false}));['pointerup','pointercancel','pointerleave','touchend','touchcancel'].forEach(type=>button.addEventListener(type,layingHoldCancel));}
